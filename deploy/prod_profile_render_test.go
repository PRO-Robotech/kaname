...
//
// Условие «в CI отсутствие helm — отказ» покрасило бы обычную job юнитов по
// причине, к дереву отношения не имеющей, а условие «везде пропуск» сделало бы
// гейт инертным именно там, где он гейтит мёрж.
//
// Поэтому вердикт производит ОБЪЯВИВШАЯ СЕБЯ полоса — deploy/render-guard.sh,
// который ставит HELM_RENDER_GUARD_LANE=1:
//
...
// а сам ci.yml объявляет отказ от установки helm решением. Значит рендер чарта
// в конвейере не проверяется, и звать полосу надо рукой:
//
//	make helm-render-guard
package deploy_test
...
// остаток, который снимает линия дебрендинга. Первую редакцию этого имени
// покраснил сводный держатель платформы, судивший тогда дерево службы; с выносом
// он его не обходит, и в этом дереве ось держит перепись осей имени платформы
// (`internal/supplyhygiene`, `TestEveryPlatformNameAxisHasAHolder`, ось «ручки»).
