...
// standardKeyDomains — домены, чьи ключи объявляет не продукт, а сама
// платформа контейнеров. Ведомость, и она истекает сама: запись, которой нечего
// разрешать, находкой не становится, но и не прощает ничего сверх своего домена.
var standardKeyDomains = []string{"kubernetes.io", "k8s.io"}
