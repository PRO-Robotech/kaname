...
//
// ЛИТЕРАЛА В БАЗОВЫХ ЗНАЧЕНИЯХ ЭТА ПРОБА НЕ ЧИТАЕТ, и это граница, а не
// недосмотр: она судит ТЕКСТ шаблонов, а `values.yaml` сливается с накладкой
// оператора уже у `helm` и в шаблоне выглядит честным сквозным проходом. Эту
// ось судит соседка `boot_guard_base_values_test.go`. До неё литерал базовых
// значений проходил мимо обеих проб с переписью «подставляют своё 0» — так
// срок ключа подписи выбирался за оператора при снятых умолчаниях процесса
// (#321).
package deploy_test
