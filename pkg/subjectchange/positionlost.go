...
// подменить, поэтому оно и живёт в `internal/`. Внешний потребитель этого
...
// Error — ЗАПИСЬ О ТОМ, ПОЧЕМУ ЭТО ИМЯ ЖИВО (kacho#2504).
//
// `deadcode -test ./...` называет метод недостижимым, и это верно по букве:
// ни одно место дерева его не форматирует. Снимать его всё равно НЕЛЬЗЯ, и
// причина не в осторожности — в контракте: он и есть то, чем `*PositionLostError`
// удовлетворяет интерфейсу `error`. Тип экспортируем и живёт в `pkg/`, то есть
// импортируем извне модуля; снятие метода — ломающее изменение у всякого
// потребителя, который присваивает значение переменной типа `error`.
//
// Недостижимость по обходу и принадлежность контракту — РАЗНЫЕ свойства, и
// инструмент меряет первое.
//
// ЗАПИСЬ ИСТЕКАЕТ САМА: появится в дереве место, форматирующее эту ошибку, —
// инструмент перестанет называть метод, и запись останется без предмета.
// Предикат: `deadcode -test ./... | grep -c PositionLostError.Error` → 1 пока
// запись нужна, 0 когда её пора снять.
