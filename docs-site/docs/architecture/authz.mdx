...
Видимость резолвится **прямым вопросом про объекты страницы**, а не перечислением всего, что
доступно субъекту. Перечисление в хранилище отношений ограничено серверным потолком и не имеет
продолжения, поэтому на долгоживущем сторе объект сверх потолка стал бы невидим владельцу при
живых правах. Вопросы про объекты страницы группируются в батчи (до 50 проверок на запрос,
партиции — параллельно), поэтому стоимость масштабируется **страницей**, а не населением типа:
`pageSize=1000` укладывается в десятки обращений, а не в тысячу. Ошибка любого вопроса прерывает
резолюцию целиком (fail-closed) — частично отфильтрованная страница не отдаётся никогда.
...
Условие — предикат, который обязан быть истинным, чтобы кортеж считался действующим. Условия
объявлены в модели авторизации и вычисляются OpenFGA по контексту запроса:
