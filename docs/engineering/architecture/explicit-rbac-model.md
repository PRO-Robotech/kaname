...
Раньше доступ выводился каскадом в графе отношений: «есть отношение на родителя
...
Модель прав в этом решении **плоская**: прямые отношения на объектах, без
вычисляемых иерархических каскадов доступа. Единственное исключение — cluster
super-admin (один флаг + short-circuit, см. ниже).
...
(+ tier `admin` для admin-набора). Это набор БОЛЬШИНСТВА типов, а не платформенная
константа: набор — атрибут ТИПА (см. ниже про `nlb_target_group`), и он бывает шире
(`registry_registry` несёт `v_create`) и уже (`iam_user` несёт только `v_get`/`v_list`:
распоряжение строкой личности выражено именованными отношениями — `record_writer`,
`identity_suspender`, `identity_remover`, — а глаголы `v_update` и `v_delete` сняты
как не имеющие читателя, #1128 и #1189). `v_create` в наборе большинства НЕТ: глагольное отношение
...
- [`29-relational-verdict.md`](../components/29-relational-verdict.md) — как складывается вердикт.
