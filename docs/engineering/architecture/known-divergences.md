...
**Замер на ревизии `PRO-Robotech/kacho@74eb331234`** — в МОНОРЕПО, той ревизией,
которой этот раздел приехал в линию; предикаты ниже называют пути `services/iam/**`,
которых в этом дереве нет, поэтому история названа ВМЕСТЕ с ревизией — и врезка ниже
рассуждает о предковости в ТОЙ истории, а не в этой (единица счёта названа, потому что
три единицы дают три разных числа):
...
>
> **ПРЕДИКАТ НАЗЫВАЕТ ДЕРЕВО, В КОТОРОМ ЕГО ГОНЯТ.** Обе ревизии принадлежат
> истории предшественника: в ЭТОМ репозитории их нет как объектов, и обе команды
> отвечают кодом **128** (`Not a valid object name`) — то есть прежняя редакция
> обещала читателю `0` и `1` там, где он получает отказ, и учила предикату,
> дающему **обратное** обещанному. Гонять их надо в дереве предшественника:
>
> ```sh
> # В КЛОНЕ PRO-Robotech/kacho — там, где эта история живёт:
> git -C <клон kacho> cat-file -t ee2370e18                # commit — «да», и это ни о чём
> git -C <клон kacho> merge-base --is-ancestor ee2370e18 HEAD; echo $?  # 1 — НЕ предок
> git -C <клон kacho> merge-base --is-ancestor 74eb331234 HEAD; echo $? # 0 — предок
>
> # В ЭТОМ репозитории (PRO-Robotech/kaname) обе дают 128: объекта нет вовсе.
> git cat-file -t ee2370e18; echo $?                       # 128
> ```
>
> Отсюда и форма записи датировки в корпусе: замер, снятый в чужой истории,
> называет её ВМЕСТЕ С РЕВИЗИЕЙ — `PRO-Robotech/kacho@<хеш>`, — иначе читатель
> уходит гонять предикат в дереве, которое о ревизии ничего не знает.
>
...
**Замер на ревизии `PRO-Robotech/kacho@74eb331234`** — в МОНОРЕПО (предикат выше
называет путь `services/iam/**`, которого в этом дереве нет): рукописных копий
**ноль**. Предикат даёт три
...
## 21. Негодная форма фикстурного id ВНЕ охвата гейта — считается и не судится (задача #1809)

**Состояние:** решение записано по задаче #1809. Половина её предмета —
остаток В ОХВАТЕ — закрыта правкой, а не записью: ведомость `fixtureFormLedger`
опустела. Здесь записана ВТОРАЯ половина, для которой задача сама называла
два исхода и оставляла выбор.

**Норма конвенции.** `e2e-flow.md` §5: фикстура не снисходительнее продукта.
Гейт `TestFixtureIdentifiersPassTheProductFormCheck`
(`internal/apps/kaname/shared/ids_fixture_form_test.go`) держит это для формы
СОБСТВЕННОГО идентификатора: литерал в позиции id обязан проходить
`shared.ValidateResourceID` — ту самую функцию, которую зовёт прод-код.

**Расхождение.** Гейт судит только пакеты, чей прод-код сам зовёт строгую
проверку. Литералы остальных пакетов он **считает и печатает, но не обвиняет**.
Замер на `HEAD` этой линии (единица счёта — строковый литерал в позиции
собственного идентификатора, заявляющий объявленный префикс и отвергаемый
`shared.ValidateResourceID`):

```sh
go test ./internal/apps/kaname/shared/ -run TestFixtureIdentifiersPassTheProductFormCheck -count=1 -v
# строка переписи: «негодных В ОХВАТЕ 3 (из них помечено намеренно 3) ·
#                  негодных ВНЕ охвата 617»
```

Пакетов в охвате **10**, вне охвата остаток лежит в **33** пакетах. Перечень
здесь НЕ выписывается — он выводится той же переписью и устарел бы с первым же
пакетом, вошедшим в охват; для ориентира крупнейшие доли:
`api/access_binding/reconcile` 97 · `repo/kaname/pg/relverdict` 78 · `domain` 70.

### Почему это не дефект, а граница доказуемости

В пакете вне охвата прод-код **не судит форму своего идентификатора вовсе**.
Значит утверждение «фикстура снисходительнее продукта» там не имеет референта:
снисходительнее ЧЕГО, если продукт в этом месте не отвергает ничего. Доказуемым
расхождение становится ровно тогда, когда пакет начинает судить, — и тогда же
гейт начинает его обвинять.

Класс, ради которого гейт заведён (#1791: ужесточение проверки в ОДНОМ месте
уронило 39 проб, ни одна из которых про формат не была), приходит именно в этот
момент — и приходит **видимым**, а не молча.

### Самовооружение ИЗМЕРЕНО опытом, а не выведено из устройства

Утверждение «охват растёт сам» — свойство чужого механизма, и опереться на него
по назначению было бы гипотезой. Опыт (2026-09-14, одно-фактный: единственная
правка — прод-вызов `shared.ValidateResourceID` во временном файле пакета
`internal/apps/kaname/api/cluster`, больше ничего):

| величина | до | после |
|---|---:|---:|
| пакетов в охвате | 10 | **11** |
| негодных В ОХВАТЕ | 3 | **7** |
| негодных ВНЕ охвата | 617 | **613** |
| вердикт гейта | PASS | **FAIL, четыре находки с координатами** |

Литералы `cluster` переехали из непросуженных в обвиняемые **в том же прогоне**,
без единой правки самого гейта и без записи в ведомость. Временный файл снят,
вердикт вернулся к PASS.

### Рассмотрено и ОТВЕРГНУТО: заморозить остаток точным числом

Ведомость с точным числом на каждый из 33 пакетов вне охвата рассматривалась —
по образцу той, что держала остаток в охвате, — и отвергнута.

Точное число краснеет **в обе стороны** (иначе оно потолок и не истекает). Значит
всякая обычная проба, заведённая в одном из 33 пакетов с обычным фикстурным
`usr_a`, роняла бы гейт — в пакете, где продукт форму не проверяет и где
приведение к длине 20 не покупает продукту ничего. Автору осталось бы дополнить
литерал ради числа, то есть исполнить обряд. Проверка, краснеющая на верной
работе, хуже отсутствующей: её отключают первой, а вместе с ней перестают читать
и настоящую находку.

Отвергнуто и «привести все 617 к форме вперёд охвата»: работа затрагивает 33
пакета, наборы newman, сценарий k6 и прод-код, а её единственный выигрыш —
ноль в строке переписи.

### Предикат пересмотра — ВНЕШНИЙ, а не «когда решим»

Запись пересматривается, когда пакет вне охвата начинает звать
`shared.ValidateResourceID` на СВОЁМ идентификаторе. Тогда он входит в охват
автоматически, его литералы становятся находками с координатами, и приведение их
к форме — часть того изменения, а не отдельная задача.

Не является предикатом пересмотра: рост числа вне охвата, появление нового
пакета, закрытие задачи #1809.

**Запись истекает сама:** строка переписи «негодных ВНЕ охвата 0» означает, что
у записи не осталось предмета и её надо снять.

### Чем держится

Не гейтом на само расхождение — его здесь нет намеренно (см. отвергнутое выше), —
а двумя свойствами гейта формы, каждое из которых проверяемо:

- **охват ВЫВОДИТСЯ из прод-кода**, а не объявляется списком: пакет, начавший
  судить форму, приводит с собой требование к своим фикстурам. Доказано опытом
  выше;
- **перепись печатает ОБЕ величины** — «негодных В ОХВАТЕ» и «негодных ВНЕ
  охвата». Величина, которую никто не судит, не перестаёт быть названной, и «ноль
  находок» остаётся отличимым от «ноль прочитанного».

Строгость самой проверки и её единственное послабление (закрытый перечень
посеянного) — предмет записи 19, здесь они не пересказываются.
