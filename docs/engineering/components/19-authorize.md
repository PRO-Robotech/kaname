...
> **Не путать с эндпоинтом авторизации церемонии OAuth.** Этот компонент — REST-действия
> `/iam/v1/authorize:<действие>` на крае. `GET /iam/v1/authorize` без суффикса действия — эндпоинт
> авторизации церемонии `authorization_code` на поверхности выдачи токенов службы
> (`internal/handler/ceremonyhttp`, LINE-A-1); путь там зарегистрирован точной парой «метод + путь»,
> и координата с суффиксом действия на поверхности выдачи не резолвится.
