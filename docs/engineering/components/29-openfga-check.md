...
## Попытка обращения к хранилищу прав: причина отказа и когда её повторяют

Раздел выше описывает отказ хранилища как ОДНО состояние — «OpenFGA недоступен».
Снаружи оно так и выглядело: вызывающий получал `unavailable`, и различить, что
именно произошло, можно было только чтением журнала построчно. Различий,
однако, три, и ведут они себя по-разному.

| причина (`outcome`) | что произошло | повтор |
|---|---|---|
| `ok` | ответ получен и разобран | — |
| `store_rejected` | 400: у типа нет такого отношения, объект — подстановочный знак. Терминальный отказ ВОПРОСА | нет: наружу идёт `PermissionDenied`, а не `Unavailable` |
| `store_error` | 5xx | да |
| `store_unreachable` | соединение установить не удалось | нет: условие не изменится в пределах бюджета |
| `pooled_conn_dropped` | соединение взято ИЗ ПУЛА и не дало ни байта | **да, на свежем соединении** |
| `conn_dropped` | свежее соединение оборвалось после отправки запроса | да |
| `store_timeout` | свежее соединение, срок вышел, ответа нет | нет: закрытый отказ обязан укладываться в объявленный бюджет |
| `decode_failed` | ответ 200, тело не разбирается | нет |

**Почему `pooled_conn_dropped` — отдельная причина, а не «таймаут».**
Пересоздание пода хранилища меняет адрес назначения; соединения, которые процесс
держит в пуле, указывают на экземпляр, которого больше нет. Пакеты по ним уходят
в никуда, ответа не будет никогда, и попытка упирается в собственный срок — то
есть по ФОРМЕ отказ неотличим от «хранилище молчит», хотя хранилище живо и
отвечает соседним запросам за миллисекунды. Различает их ровно один признак:
свежее соединение доказывает, что адресат жив, переиспользованное — не
доказывает ничего. Отсюда и правило повтора, и отдельная метка.

**Бюджет.** У каждой попытки ЧТЕНИЯ свой срок `CheckTimeout` (умолчание 200 мс),
попыток не больше двух. Общий срок на обе попытки сделал бы повтор мёртворождённым:
первая попытка на мёртвом соединении съедает бюджет целиком. Удвоение времени
наступает ТОЛЬКО на повторяемых причинах; при настоящей недоступности хранилища
пул пуст, соединение свежее, повтора нет и время остаётся прежним.

**Наблюдаемость.** Счётчик `kacho_iam_authz_store_attempts_total{op,outcome,reused}`
— каждая попытка, включая успешные: «ноль отказов за всю жизнь» обязано быть
отличимо от «никто не смотрел». В журнал идёт РОВНО один исход,
`pooled_conn_dropped`: он редкий, чинится повтором и был невидим. Недоступность и
молчание в журнал не идут — под настоящим отказом это строка на каждый запрос.

Код: `services/iam/internal/clients/openfga_attempt.go`; провязка наблюдателя —
`services/iam/cmd/kacho-iam/env.go` (`newAuthzStoreObserver`). Свойство «каждое
обращение к транспорту хранилища объявляет причину» держит гейт
`internal/repohygiene` `TestAuthzStoreCallsClassifyTheirOutcome`.
