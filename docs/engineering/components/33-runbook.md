...
prod-пути, ничего ею не подписывается и не верифицируется. **Таблица снята
миграцией 0065** (перед этим на рабочем стенде в ней было ноль строк), CronJob-
ротатор снят ранее как вестигиальный. Её отсутствие — норма, НЕ причина инцидента.
`GetJWKSStatus` снят с обслуживания (отдавал пустой набор и никем не звался) —
диагностического сигнала по JWKS в gRPC-поверхности iam нет вовсе: смотрите
Hydra и прокси `:9097`.
