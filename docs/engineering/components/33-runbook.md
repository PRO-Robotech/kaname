...
7. **Ключ подписи утёк либо подлежит выводу** → команда оператора тем же
   ...
   когда подпись перешла. Коды: `0` сделано (повтор — `already-done`), `1`
   отказ по существу, `3` вердикта нет. `outcome=partial` у `compromise`
   значит: ключ снят, подписывающего нет и замена не заведена, служба не
   подписывает — повторите команду, повтор довершает замену.
   `outcome=signer-unknown` (код `3`) значит: ключ снят, а подписывает ли
   служба, не установлено (чтение подписывающего отказало либо вызов кончился
   посреди замены) — повторите команду.
