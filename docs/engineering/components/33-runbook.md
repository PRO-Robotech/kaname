...
7. **Ключ подписи утёк либо подлежит выводу** → команда оператора тем же
   ...
   отказ по существу, `3` не исполнялось. `outcome=partial` у `compromise`
   значит: ключ снят, замены нет и служба не подписывает — повторите команду,
   повтор довершает замену.
