...
7. **Ключ подписи утёк либо подлежит выводу** → команда оператора тем же
   ...
   `outcome=partial` у `compromise` значит: ключ снят, подписывающего нет и
   замена не заведена, служба не подписывает — повторите команду, повтор
   довершает замену и называет её. `outcome=signer-unknown` (код `3`) значит:
   ключ снят, а подписывает ли служба, не установлено (чтение подписывающего
   отказало либо истёк предел посреди замены) — повторите команду. Вся команда
