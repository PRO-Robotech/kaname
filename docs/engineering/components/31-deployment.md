...
Историческая таблица `kacho_iam.oidc_jwks_keys` **снята миграцией 0065**. Она
осталась от дизайна ДО перехода на проксирование: приватная половина писалась
зашифрованной и **ни разу не расшифровывалась ни в одном prod-пути**, а в Hydra
публиковалась только **публичная** половина, которой подписать ничего нельзя;
на полностью рабочем стенде таблица содержала **ноль строк**. Ранее вместе с ней
был снят CronJob `kacho-iam-jwks-rotator` (он к тому же звал несуществующий
...
RPC отчёта о состоянии JWKS **снят с контракта целиком** — и объявление тоже.
Он сообщал содержимое таблицы ключей, дропнутой миграцией `0065_drop_oidc_jwks_keys.sql`,
после чего мог отвечать только пустым набором — навсегда; вызывающих у него не было ни
одного (ни сервис, ни шлюз, ни консоль, ни регрессионный кейс). В
`internal_iam_service.proto` на его месте стоит комментарий, объясняющий снятие: и метод,
и его сообщения удалены.

Резервировать при этом было нечего: protobuf знает `reserved` только для номеров и имён
**полей**, но не для методов сервиса и не для имён сообщений верхнего уровня. От повторного
занятия имени защищает `buf breaking` и объявление ломающего изменения в сообщении коммита —
не запись в схеме.

Верификация подписи живёт в JWKS-проксировании iam (`:9097`), а не в отчёте о ключах.
