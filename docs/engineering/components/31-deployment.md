...
(`internal/ceremonyport.AccessTokens`) стоит над **тем же** подписантом, что чеканит
все токены службы, и над тем же публикуемым набором — второго подписного
материала, который пришлось бы хранить, вращать и беречь, нет (задача #396).

**Где работает подписант: в процессе службы.** Адаптер принимает подписанта
значением процесса, а не адресом, поэтому церемония, собранная на нём, и подписант
исполняются в одном процессе, и отметка выпуска `iat` берётся с тех же часов, что
границы церемонии. Класс отказа «часы подписанта отстают от часов церемонии на
секунду и больше — обмен отвергнут» при такой сборке не возникает по построению,
и счётчика у него нет. Подписант вне процесса — другое развёртывание: вместе с ним
заводится объявленный счётчик отказов обмена по часам подписанта и проба, которая
его судит.

Токен доступа церемонии несёт **ключ семейства** — идентификатор гранта, общий у
всех выпусков одной выдачи. Отзыв семейства (повтор кода, повтор токена
обновления, снятие сессии) кладёт отсечку по этому ключу в ту же таблицу отсечек,
что читает авторитет отзыва на пути запроса, и токен отозванного семейства
отвергается при предъявлении — при любой отметке выпуска.
