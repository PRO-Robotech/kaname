...
- `github.com/PRO-Robotech/corelib` — общий ФУНДАМЕНТ, от которого зависят и служба,
  ...
  Модуля ПЛАТФОРМЫ (`github.com/PRO-Robotech/kacho`) среди зависимостей службы
  **нет ни одного**, и это единственное ребро продукта, которое у неё есть.

**Ребро `kaname → kacho` снято целиком — ступень S0a, kacho#2617, исход C.**
Прежде здесь стоял ОСТАТОК из трёх путей: `pkg/api/kaname/cloud/iam/v1` (свои
доменные заглушки, публиковавшиеся модулем платформы), `pkg/ownerregister` и
`pkg/subjectchange` (производители, отнесённые приёмкой K3-1 к классу `kaname`).
Все три сняты ОДНИМ изменением, и одновременность была замером, а не
осторожностью: два последних импортировали платформенные заглушки своим
не-тестовым кодом, поэтому двоичное получало ВТОРУЮ копию дескриптора
транзитивно — `go build` давал 0, запуск давал панику регистрации.

Что сделано и чем проверено:

- 262 узла импорта переключены на своё — 251 на заглушки службы
  (`proto/kaname/` 41 контракт, `pkg/api/kaname/` 87 порождённых файлов,
  `make proto-gen` исполняется без дерева платформы), 11 на перенесённые сюда
  `pkg/ownerregister` (4 файла) и `pkg/subjectchange` (13 файлов);
- `require` платформы и обе строки `go.sum` убраны `go mod tidy`;
- пакетов платформы в `go list -test -deps ./...` — ноль из 1087.

Держит это гейт `internal/supplyhygiene` двумя предикатами:
`TestNoImportNodeNamesThePlatformModule` судит РАЗОБРАННЫЕ узлы импорта,
`TestModuleGraphCarriesNoEdgeToThePlatform` — `go.mod` (require и replace),
`go.sum` и граф сборки. Четвёртый источник несущий: он находит транзитивное
ребро, ни одним узлом импорта не названное, и заменяет собой прежний гейт
`internal/contracthome` `TestBothCopiesOfAContractNeverReachOneBinary`, чей
предикат обходил исходники платформы в кэше модулей и существовал ровно пока
модуль платформы оставался зависимостью.

**Чего это не закрывает.** Те же 87 заглушек и оба пакета всё ещё лежат в модуле
платформы, и их изъятие — ступень S0b (kacho#2616, kacho#2614). Пока изъятия нет,
один и тот же контракт публикуют два модуля; двоичным СЛУЖБЫ это безвредно (в её
графе сборки модуля платформы нет), а половина класса, что живёт у платформы,
судится в её дереве.
