...
- `github.com/PRO-Robotech/kacho` — ОСТАТОК зависимости от платформы, и он сходится
  ...
  собственные доменные proto-stubs службы; `pkg/ownerregister` и `pkg/subjectchange`
  — производители, отнесённые приёмкой K3-1 к классу `kaname`, до переезда которых
  платформа остаётся их домом. Чего в остатке нет, гейт объявляет находкой.

  **Три пути снимаются ОДНОВРЕМЕННО, и это замер.** Контракты службы уже уехали в
  этот репозиторий (решение владельца 2026-09-13, kacho#2616): исходники в
  `proto/kaname/` (41 файл), заглушки в `pkg/api/kaname/` (87 файлов),
  `make proto-gen` исполняется без дерева платформы. Переключить 251 узел импорта
  на них отдельно нельзя: `pkg/ownerregister/ownerregister.go` и
  `pkg/subjectchange/reader.go` импортируют платформенные заглушки своим
  не-тестовым кодом, поэтому двоичное получает ВТОРУЮ копию дескриптора
