...
| `kaname_client_token_outcomes_total`          | counter   | outcome                             | Исходы обращений к токен-эндпоинту платформы. Набор значений — закрытый словарь `clientassertion.Outcomes()`; печатаются ВСЕ объявленные, включая нулевые, поэтому «ноль отказов» отличимо от «полоса не исполнялась ни разу». Наружу все отказы АУТЕНТИФИКАЦИИ отдают один и тот же ответ — различимость для оператора живёт только в этой метке; свои коды наружу несут лишь отказы формы запроса и два отказа по темпу (`in-flight-ceiling-reached`, `client-pace-exceeded` — 429 со сроком ожидания, kaname#315). |
| `kaname_basic_credential_outcomes_total`      | counter   | verb, outcome                       | Исходы полосы базового секрета внутреннего слушателя по глаголу (`resolve`/`liveness`) и исходу. Причины отказа — закрытый словарь `domain.BasicCredentialRefusalReasons()`, клетки — `internal_iam.DeclaredBasicCredentialCells()`; печатаются ВСЕ объявленные, включая нулевые. Наружу любая причина — один и тот же отказ; различима она только здесь и в журнале (`basic credential refused`, поля `verb`, `outcome`). |
