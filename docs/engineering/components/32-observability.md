...
| `kaname_authorization_ceremony_outcomes_total` | counter | outcome                             | Исходы церемонии `authorization_code` (LINE-A-1): выдача кода (`authorize.*`), обмен и ротация (`exchange.*`). Набор — закрытый словарь `ceremony.Outcomes()`, печатаются все объявленные, включая нулевые. Наружу отказы неразличимы (Р10, 04/05) — различимость живёт в этой метке; `*-replayed` означает отзыв семейства авторизации. |
