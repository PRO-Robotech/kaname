...
запроса; отношение — по глаголу: мутации (`Issue`, `Revoke`) требуют `v_update`, чтение
(`List`) — `v_list`. Step-up (`required_acr_min=2`) стоит на **мутациях**: выпуск и отзыв
креденшала меняют посадку безопасности; `List` отдаёт только метаданные без приватной части
и требует обычной аутентификации (`required_acr_min=1`).
...
Отзывает токен (снимает OAuth-клиента в Hydra). Требует `v_update` на `iam_user` из `user_id`
