...
Эта страница описывает **конвенции**, общие для всего API Kaname: ресурсы, REST-пути,
...
Источник истины — Protocol Buffers в `proto/` (домен `kaname.cloud.iam.v1`). REST-поверхность
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Membership</strong></td><td>Принадлежность человека аккаунту; их у него может быть несколько. Заводится приглашением (<code>Create</code>), читается по аккаунту; правки нет — ни одного поля вызывающий не назначает сам</td><td>[Membership](/api/membership)</td></tr>
...
Административного ресурса **Limit** (`lim`) у службы больше нет: авторитет величин выпилен
из неё целиком. Свои три потолка она по-прежнему считает, но их величину объявляет посадка
установки, а не внешний глагол; арендатор читает свой потолок на странице
[Квоты](/api/quotas).
...
Ключевая конвенция Kaname: **чтение синхронно, мутации асинхронны**.
...
`Operation` (`corelib.operation`) несёт `id` (префикс `iop`), `description`, `createdAt`,
...
Все внешние запросы проходят через **`api-gateway`**, который проверяет удостоверение до
того, как запрос дойдёт до IAM. Удостоверений два вида:

- **JWT доступа** в заголовке `Authorization: Bearer <token>` — для машин и терминала.
  Выпускает его платформа своим подписантом там, где объявлен её токен-эндпоинт
  (`client_credentials` либо `jwt-bearer`), а на установке `external` без него — внешний
  OAuth-сервер. Край принимает **оба** издателя, и какой из них выпустил токен, записано в самом
  токене;
- **печенье сессии человека** — для консоли. Имя печенья зависит от посадки личности
  установки: `ory_kratos_session` под `external` (вход держит поставщик) и `kaname_session`
  под `own` (вход держит служба). Что верно под какой посадкой —
  [Идентичность и токены](/architecture/identity).

Отсутствующее или непринятое удостоверение → `UNAUTHENTICATED` (HTTP `401`); причин у отказа
четыре, и ответ на все один — см. таблицу ниже. Транспорт service→service — **mTLS** (verified
client-cert).
...
flowchart LR
  ...
  agw -->|"verify JWT"| agw
  agw -->|"iam.v1.* + subject"| iam["kaname"]
  iam -->|"Check(subject, relation, object)"| pg[("PostgreSQL<br/>kaname")]
  pg -->|"allow / deny"| iam
```
...
схеме `kaname`, в которой лежат выдачи. Отдельного хранилища, до которого право должно
...
`message` — текст в стабильной формулировке Kaname, `details[]` — массив структурированных
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>FAILED_PRECONDITION</code></td><td>400</td><td>Состояние не позволяет (удаление аккаунта с проектами; удаление непустого проекта — в операции, с перечнем видов и чисел, см. <a href="/api/project">Project</a>; system-роль; deletion_protection); потолок квоты на вид не назван ни на одной области видимости (признак <code>QUOTA_NOT_PROVISIONED</code>)</td></tr>
    ...
    <tr><td><code>UNAVAILABLE</code></td><td>503</td><td>База IAM или сосед на пути запроса (внешний поставщик удостоверений, домен величин) недоступны — fail-closed для мутаций</td></tr>
    <tr><td><code>UNAUTHENTICATED</code></td><td>401</td><td>Удостоверение не принято, и причин четыре, неразличимых в ответе намеренно: (1) удостоверения нет либо оно неизвестно — опечатка, чужой издатель, анонимный запрос; (2) срок истёк; (3) отозвано — выход, смена пароля, блокировка личности, отзыв токена; (4) <strong>недостаточный уровень аутентификации</strong> — глагол несёт порог <code>2</code>, а сессия или токен объявляют <code>1</code> (см. <a href="/architecture/authz#step-up">step-up</a>). Различимый отказ был бы оракулом для предъявителя</td></tr>
...
Отдельно стоит **якорь кластера** — `cluster_root`. Это не сгенерированный
...
это конвенция Kaname. БД хранит микросекунды, но клиент видит только секундную точность
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>UserService</code></td><td>Get / List / Invite / ResendInvite / Update / RemoveFromAccount / Block / Unblock / ResetSecondFactor / Delete / ListOperations</td><td>[User](/api/user)</td></tr>
    <tr><td><code>MembershipService</code></td><td>Create / Get / List</td><td>[Membership](/api/membership)</td></tr>
