...
:::note Подписки на операции нет; журнала подписки у IAM тоже пока нет
...
владельцами журнала объявлены другие домены; ресурсы IAM в него сегодня не попадают. Исход
операции поток не заменяет ни у одного домена: у провалившейся мутации события нет вовсе.
Устройство подписки — в документации api-gateway, страница «Подписка на изменения».
:::
...
- **step-up floor** `required_acr_min=2` несёт узкий набор RPC, меняющих posture безопасности
  (выдача учётных данных, выдача и отзыв прав, блокировка, необратимое удаление) — 24 записи
  каталога прав из 289, а не пользовательская поверхность целиком.
...
flowchart LR
  ...
  iam -->|"Check(subject, relation, object)"| pg[("PostgreSQL kacho_iam")]
  fga -->|"allow / deny"| iam
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>FAILED_PRECONDITION</code></td><td>400</td><td>Состояние не позволяет (удаление аккаунта с проектами; system-роль; deletion_protection)</td></tr>
