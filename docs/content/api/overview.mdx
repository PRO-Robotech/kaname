...
`Authorization: Bearer <token>`. Токен выдаёт слой идентичности: человеку — интерактивный вход
через Ory Kratos → Hydra, машине — `client_credentials` либо `jwt-bearer` на токен-эндпоинте
платформы, а на установке без него у внешнего OAuth-сервера (см.
[Идентичность и токены](/architecture/identity)). Край принимает **оба** издателя, и какой из них
выпустил токен, записано в самом токене. Отсутствующий или невалидный токен →
