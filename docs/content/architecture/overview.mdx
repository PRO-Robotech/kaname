...
## Идентичность — Ory Kratos + Ory Hydra, плюс своя чеканка

Личность человека и его интерактивный вход вынесены в Ory-стек: **Kratos** хранит личности и
обслуживает вход (пароль / passkey), **Hydra** — OAuth 2.0 authorization server. IAM интегрируется
с ними через hook-listener (активация User при первом входе) и клиент Hydra (регистрация
OAuth-клиентов). User — проекция Kratos-identity.
...
публикуемого набора ключей рядом с зеркалом набора провайдера. Где своя чеканка не объявлена, ключ
и токен по-прежнему зеркалятся у Hydra, и издателем выступает она.

**Интерактивный вход человека держится на провайдере на любой посадке**: вида выдачи
`authorization_code` наш токен-эндпоинт не принимает вовсе — перечень видов у него закрыт и состоит
из `client_credentials` и `jwt-bearer`. Это остаток перехода, а не решение: пока он не снят,
провайдер нужен каждой установке, где входит человек.
