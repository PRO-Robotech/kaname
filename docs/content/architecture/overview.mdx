...
```mermaid
flowchart TD
  handler["handler<br/>(тонкий transport: parse → use-case → format)"] --> uc["use-case<br/>(apps/kaname/api/{account,project,user,…})"]
...
## Идентичность — по посадке: внешний поставщик либо свой вход

Кто проверяет человека, решает ключ `authn.identity-provider`, и посадок две. Под **`external`**
личность и интерактивный вход вынесены в Ory-стек: **Kratos** хранит личности и обслуживает
вход (пароль / passkey), **Hydra** — OAuth 2.0 authorization server; IAM интегрируется с ними
через hook-listener (активация User при первом входе) и клиент Hydra (регистрация
OAuth-клиентов), а User — проекция Kratos-identity. Под **`own`** внешнего поставщика нет:
способы входа, сессия (`kaname_session`) и второй фактор — записи самой службы, форма входа
приходит на её слушатель полосы входа от края.
...
публикуемого набора ключей. Под `external` без своей чеканки ключ и токен по-прежнему зеркалятся
у Hydra, и издателем выступает она.

Вида выдачи `authorization_code` наш токен-эндпоинт не принимает ни на одной посадке — перечень
видов у него закрыт и состоит из `client_credentials` и `jwt-bearer`: человек входит либо у
поставщика, либо на полосе входа службы, а токен для терминала выпускает уже сессией.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>:9092</code></td><td>hooks</td><td>Ory Kratos (mTLS) — посадка <code>external</code></td><td>Identity-хуки (активация User при первом входе, завершение восстановления)</td></tr>
    <tr><td><code>:9095</code></td><td>metrics</td><td>Prometheus (cluster-internal)</td><td><code>/metrics</code></td></tr>
    <tr><td><code>:9100</code> (адрес объявляет профиль)</td><td>login lane</td><td>ровно край платформы (взаимный TLS, допуск по SAN) — посадка <code>own</code></td><td>Формы входа: регистрация, вход, выход, смена пароля, восстановление, второй фактор</td></tr>
  </tbody>
</table>
...
```mermaid
flowchart LR
  vpc["kacho-vpc"] -->|"Check + RegisterResource"| iam["kaname"]
  cmp["kacho-compute"] -->|"Check + RegisterResource"| iam
  geo["kacho-geo"] -->|"Check"| iam
  nlb["kacho-nlb"] -->|"Check"| iam
  iam -->|"вердикт запросом"| pg[("PostgreSQL kaname")]
  iam -->|"identity / OAuth — посадка external"| ory["Ory Kratos + Hydra"]
```
...
рёбра IAM — к инфраструктуре (Postgres; под `external` ещё Kratos и Hydra), не к доменным
сервисам.
:::
