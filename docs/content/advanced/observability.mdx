...
Kaname — центральный авторизатор платформы, поэтому его наблюдаемость критична: сбой или
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Authz Check</strong></td><td>Латентность <code>Check</code> (бюджет ≤ 30 ms p95); доля deny; отказы вердикта</td></tr>
    <tr><td><strong>Сброс кэша края</strong></td><td>Темп записи в <code>subject_change_outbox</code> и рейт <code>PollSubjectChanges</code>. Отставание живёт <strong>у читателя</strong>: журнал читает сам api-gateway курсором, и «докуда дочитали» со стороны iam не видно. Глубина журнала отставанием <strong>не является</strong>: строки снимает фоновая уборка по сроку, поэтому число выходит на полку, а его падение — норма</td></tr>
...
- отказы `PollSubjectChanges` — край не смог прочитать журнал (его кэш решений держит старый
  срез прав дольше обычного).
...
```mermaid
flowchart TD
  check["Check p95 &gt; 30ms или рост deny"] --> a["база под нагрузкой / раздутая цепь областей / план вывода"]
  outbox["PollSubjectChanges: отказы или тишина"] --> b["край не читает журнал → его кэш держит старый срез прав"]
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Ресурс создан, но не виден в authz-filtered List сразу</td><td>Отстал кэш прав на api-gateway: право уже действует, срез края — нет. Проверить, спросив iam мимо края (poll-retry, не мгновенный assert)</td></tr>
    <tr><td>Отношения владения отвергаются, ресурсы невидимы</td><td>Несовпадение домена объекта: object-префикс ≠ имя сервиса — видно только под production-strict mTLS</td></tr>
