...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Сброс кэша края</strong></td><td>Глубина <code>subject_change_outbox</code>, лаг дренажа, retry / отравленные строки (при отставании край дольше отвечает по старому срезу прав)</td></tr>
...
- ошибки дренажа сброса кэша края (несовпадение домена объекта, отравленные строки).
...
flowchart TD
  ...
  outbox["subject_change_outbox глубина растёт"] --> b["дренаж отстаёт → кэш края держит старый срез прав"]
