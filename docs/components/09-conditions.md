...
> проверено по коду (2026-07-27, обновлено 2026-07-29):
>
> - `condition_id` и `builtin_condition` **сняты с контракта**
>   `CreateAccessBindingRequest` (теги 6/7 и имена зарезервированы, 2026-07-29):
>   их никто не читал, строку под `condition_id` нельзя создать ни одним методом,
>   а встроенному варианту негде храниться — поле обещало ограничение, которого
>   нет. Привязки условия «в момент Create» не существует и на уровне контракта.
>   `Update` условие тоже не меняет.
> - `expires_at` — **исключение: реализован** (2026-07-29). Запрос доносит срок
>   до объекта, развёрточный проход отзывает истёкшую выдачу (`ExpireBinding`),
>   значение меньше 5 минут вперёд отвергается синхронно по имени поля. То есть
>   TTL — единственный уровень «условия», который сегодня действительно
>   действует, и действует он не через ABAC-overlay, а через отзыв выдачи.
