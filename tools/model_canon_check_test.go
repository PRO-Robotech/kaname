...
func TestModelCanonCheckOnTheRealTreeComparesEveryOwnedBlock(t *testing.T) {
	...
	// Обход сверки за корень дерева НЕ ВЫХОДИТ: выборка платформы, лежащая вне
	// его, не будет найдена, и сверка объявила бы находками пять модулей, чьи
	// манифесты на месте. Поэтому предпосылок ДВЕ, и вторая — про расположение.
	tree := platformtree.RequireNamedPlatformTree(t)
	root := serviceRoot(t)
	if !strings.HasPrefix(tree, root) {
		t.Skipf("УСЛОВИЕ СОЗДАНО НЕВЕРНО (у предпосылки есть производитель): "+
			"%s=%q лежит ВНЕ корня обхода %q. Обход за корень не выходит, поэтому "+
			"манифесты оттуда не найдутся, и сверка объявила бы находками модули, "+
			"чьи манифесты на месте", platformtree.PlatformTreeEnv, tree, root)
	}
