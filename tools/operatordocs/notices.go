...
//	`./cmd/kaname` + `./cmd/migrator` (поставляем)  — 40 модулей
//
...
var ShippedPackages = []string{"./cmd/kaname", "./cmd/migrator"}
...
` + "`kaname-migrator`" + `, и под какой лицензией оно распространяется. Перечень нужен
