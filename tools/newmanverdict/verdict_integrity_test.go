...
// verdictScript — координата прогонщика ОТ КОРНЯ ДЕРЕВА ПЛАТФОРМЫ.
//
// Файл лежит ВНУТРИ модуля, поэтому детектор приводит координату в ОБЕИХ
// посадках: в монорепо — как записана, в самостоятельном клоне — со снятой
// приставкой модуля. Пропуска здесь не бывает.
