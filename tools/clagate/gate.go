...
// HistoryFloorCommits — сколько коммитов области отличают ИСТОРИЮ ДОМЕНА от
// дерева, которое её не несёт.
//
// Число здесь ПОРОГ, а не замер: на день заведения обход области давал 1132
// коммита, и порог выбран заведомо ниже — чтобы не краснеть на росте истории и
// не требовать правки при каждом её пополнении. Он ОДИН на оба исхода
// `Classify`: второй экземпляр разошёлся бы с первым молча, и тогда «слепой
// обход» и «условие не создано» перестали бы быть двумя сторонами одной оси.
const HistoryFloorCommits = 500

// Outcome — исход боевого прогона. Их ТРИ, и третий не вычитается из вердикта и
// не зачитывается в успех.
type Outcome int
...
// Classify разводит три исхода боевого прогона по ОДНОМУ дискриминатору —
// посадке судимого дерева.
//
// ─────────────────────────────────────────────────────────────────────────────
// ЗАЧЕМ ОТДЕЛЬНАЯ ФУНКЦИЯ, А НЕ ВЕТВЛЕНИЕ В ПРОБЕ
//
// Ради инъекции. Проба идёт в том дереве, в котором её запустили: ни посадку, ни
// глубину истории она себе подать не может, поэтому ветвление внутри неё
// доказывается ровно одной стороной — той, что случилась. Здесь обе величины
// приходят аргументами, и каждая ось проверяется в обе стороны на месте.
//
// ─────────────────────────────────────────────────────────────────────────────
...
//
// Отсюда несущее свойство: **в дереве платформы пропуск НЕДОСТИЖИМ**. Не «его
// там не бывает», а недостижим — ветвь одна, и с `inPlatformTree=true` в неё не
// заходят ни при каком отчёте. Без этого гейт снимался бы поломкой ведомости:
// достаточно объявить область мимо дерева, чтобы он молча пропустился, — то есть
// маска, неотличимая от исправной работы.
//
// ПОМЕТКА ИСТЕКАЕТ САМА. Настоящий клон арендатора историю несёт, и в нём гейт
// выносит вердикт как в монорепо; пропуск достаётся ровно тем деревьям, у
// которых спрашивать не о чем. Приедет история — пропуск прекратится, и снимать
// для этого ничего не нужно.
//
// ЧЕГО ЗДЕСЬ НЕТ: суждения о самих находках. `Classify` отвечает, ЕСТЬ ЛИ у
// прогона предмет; что найдено в предмете — дело вызывающего.
func Classify(rep Report, inPlatformTree bool) (Outcome, string) {
	var why string
	switch {
	case len(rep.PremiseFailures) > 0:
		why = fmt.Sprintf("основание гейта не построено: %s", strings.Join(rep.PremiseFailures, "; "))
	case rep.CommitsExamined <= HistoryFloorCommits:
		why = fmt.Sprintf(
			"обход области %v в диапазоне %s дал коммитов %d при пороге %d — это не история домена",
			rep.Scope, rep.RevRange, rep.CommitsExamined, HistoryFloorCommits)
	default:
		return OutcomeJudge, ""
	}

	if inPlatformTree {
		return OutcomeBlindWalk, why + ".\n" +
			"Посадка — ДЕРЕВО ПЛАТФОРМЫ: историю домена оно несёт by construction, значит слеп " +
			"САМ ОБХОД (усечён диапазон либо область объявлена мимо дерева). Это находка о гейте, " +
			"а не свойство поставки"
	}
	return OutcomeUnmetPremise, why + ".\n" +
		"Посадка — САМОСТОЯТЕЛЬНАЯ (дерева платформы рядом нет): распакованный состав коммита, " +
		"свежий `git init`, срез глубины. Истории домена в таком дереве может не быть by " +
		"construction, и вердикта о соглашении прогон не выносит НИ В ОДНУ сторону"
}

// ClassifyStale — вердикт о ЗАПИСЯХ ВЕДОМОСТИ, которым больше нечего покрывать.
//
// ПОЧЕМУ ЭТО ОТДЕЛЬНЫЙ ВОПРОС, А НЕ ЧАСТЬ Classify. Та отвечает, есть ли у
// гейта предмет ВООБЩЕ (несёт ли дерево историю домена). Здесь предмет есть, и
// спрашивается другое: авторитетна ли эта история для суждения о том, чему
// запись ещё нужна.
//
// ВЕДОМОСТЬ ЕДЕТ С МОДУЛЕМ, А ЕЁ ПРЕДМЕТЫ — НЕТ. Область объявлена как `.`
// именно затем, чтобы файл не правился при выносе. Но запись покрывает КОММИТ,
// а история артефакта — цепочка ПЕРЕПУБЛИКАЦИЙ: коммит дерева платформы в неё
// не попадает ни при каком построении. Значит запись, законная и не истекающая
// в стволе платформы (история не переписывается, коммит остаётся там навсегда),
// в самостоятельном клоне «нечего покрывать» ВСЕГДА — и краснела бы КАЖДОЙ
// поставкой, сообщая о посадке, а не о продукте.
//
// САМОИСТЕЧЕНИЕ ПОСЛАБЛЕНИЯ ПРИ ЭТОМ НЕ СНЯТО. Оно остаётся строгим там, где
// история авторитетна, — в дереве платформы; ветвь пропуска туда недостижима
// by construction, ровно как у Classify. В клоне вердикт НЕ ВЫНОСИТСЯ, но
// причина и ЧИСЛО записей называются: «ноль находок» обязано быть отличимо от
// «ноль прочитанного», и молчаливый пропуск здесь дал бы ровно второе.
func ClassifyStale(rep Report, inPlatformTree bool) (Outcome, string) {
	if len(rep.UnusedEntries) == 0 || inPlatformTree {
		return OutcomeJudge, ""
	}
	return OutcomeUnmetPremise, fmt.Sprintf(
		"записей ведомости без предмета в этом дереве: %d.\n"+
			"Посадка — САМОСТОЯТЕЛЬНАЯ (дерева платформы рядом нет): её история есть "+
			"перепубликация модуля, и коммиты дерева платформы в неё не попадают by "+
			"construction. Запись, покрывающая такой коммит, здесь без предмета ВСЕГДА, "+
			"поэтому вердикта о её нужности прогон не выносит НИ В ОДНУ сторону — судит "+
			"его дерево платформы, где история авторитетна",
		len(rep.UnusedEntries))
}
