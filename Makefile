...
# ─── ХУКИ ОТПРАВКИ ──────────────────────────────────────────────────────────
#
# До 2026-09-14 `git push` в этом дереве не проверял НИЧЕГО, и установлено это
# было наблюдением, а не чтением: линия ушла на origin молча, а находки
# статического анализа нашёл конвейер — то есть первым читателем кода оказался
# ранер, отвечающий через десятки минут и занимающий место в очереди, которого
# мало. Цели ниже ставят между правкой и стволом локальный прогон.
#
# ПРОВЯЗКА — ПЕРЕХОДНИК, А НЕ `core.hooksPath`. Разбор цены — в шапке самого
# скрипта; здесь коротко: `core.hooksPath` молча выключает всё, что уже лежит в
# `.git/hooks`, а лежать там может страж личности автора коммита.
#
# `check-hooks` НУЖЕН ОТДЕЛЬНО ОТ `install-hooks`, И ВОТ ПОЧЕМУ. Переходник может
# стоять и звать скрипт, которого в рабочей копии нет; тогда он печатает
# «проверок НЕ БЫЛО» и выходит УСПЕХОМ — по исходу `git push` это состояние
# неотличимо от исправной провязки. Различает их только `check-hooks`, отказом.
#
# Способность обеих целей отказать доказана инъекцией в обе стороны:
# `bash scripts/hooks/prepush-inject.sh` — 13 утверждений, у каждого дефекта
# законный близнец, на котором проверка молчит.
#
# Вызов: `make install-hooks` / `make check-hooks`
.PHONY: install-hooks check-hooks
## install-hooks — провязать хуки этого дерева в клон (переходником, не core.hooksPath)
install-hooks:
	@./scripts/hooks/install.sh install

## check-hooks — провязаны ли хуки в ЭТОМ клоне (непровязанный — отказ, а не тишина)
check-hooks:
	@./scripts/hooks/install.sh check
