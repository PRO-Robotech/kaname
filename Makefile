...
# КОПИЯ КРАЯ ДОСТУПНА И САМОСТОЯТЕЛЬНОМУ КЛОНУ — это измерено, а не выведено:
# край приезжает сюда МОДУЛЕМ-ПИНОМ платформы (`go.mod`), и его архив несёт
# `gateway/internal/middleware/embed/permission_catalog.json` (102 865 байт на
# пине v0.1.1-0.20260912212223-96c5c6e10b99; предикат — `ls` по каталогу из
# `go list -m -f '{{.Dir}}'`). Поэтому сверке не нужен полный чекаут монорепо,
# только скачанный модуль, и конвейер судит её в ОБЕИХ посадках.
#
# ВТОРОЙ ОПЕРАНД — ИМЕННО ПИН, и это названо прямо: сверяется копия края той
# ревизии, ПРОТИВ КОТОРОЙ СЛУЖБА СОБИРАЕТСЯ, а не текущая голова шлюза.
# Уедет пин вперёд с изменившимся каталогом — сверка краснеет, и синхронизацию
# зовёт человек. Расхождение между пином и головой шлюза эта цель НЕ видит
# by construction: у неё нет второго дерева.
PLATFORM_MODULE  := github.com/PRO-Robotech/kacho
EDGE_CATALOG_REL := gateway/internal/middleware/embed/permission_catalog.json
.PHONY: check-permission-catalog
## check-permission-catalog — копия каталога прав СОВПАДАЕТ с копией края (не пишет)
check-permission-catalog:
	@own="$(IAM_CATALOG_EMBED)"; \
	edge_dir="$$(go list -m -f '{{.Dir}}' $(PLATFORM_MODULE) 2>/dev/null)"; \
	if [ -z "$$edge_dir" ] || [ ! -d "$$edge_dir" ]; then \
	  echo "УСЛОВИЕ НЕ СОЗДАНО: каталог модуля-пина $(PLATFORM_MODULE) не разрешён."; \
	  echo "Сверка НЕ ИСПОЛНЯЛАСЬ — это не «копии совпали». Нужно: go mod download"; \
	  exit 2; \
	fi; \
	edge="$$edge_dir/$(EDGE_CATALOG_REL)"; \
	if [ ! -f "$$edge" ]; then \
	  echo "УСЛОВИЕ НЕ СОЗДАНО: в модуле-пине нет копии края: $$edge"; \
