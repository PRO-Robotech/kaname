...
# а переходник новой редакции на ненайденном адресате ОТКАЗЫВАЕТ — в рабочей
# копии, заявившей провязку. Каталог хуков один на все копии клона, поэтому
# заявление живёт в служебном каталоге копии, и провязка одной копии соседнюю не
# останавливает (разбор — шапка `scripts/hooks/install.sh`). Что переходник
...
## install-hooks — провязать хуки git из scripts/hooks в клон и заявить их за эту рабочую копию (проверить: make check-hooks)
install-hooks:
	@bash scripts/hooks/install.sh install

## check-hooks — провязаны ли хуки этой рабочей копии; непровязанная копия — третий исход, чужой хук и переходник прежней редакции — находки
check-hooks:
