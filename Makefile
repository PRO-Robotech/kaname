...
define require_tree
	@test -n "$(MONOREPO_ROOT)" || { \
	  echo "УСЛОВИЕ НЕ СОЗДАНО: цель $(1) судит ДЕРЕВО, а не один модуль, и рядом его нет."; \
	  echo "Модуль стоит"; \
	  echo "самостоятельным клоном ($(CURDIR)), соседей по монорепо у него не будет."; \
...
# ПОМЕТКА `[монорепо]` И `require_tree` СНЯТЫ ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ.
#
...
#
# Отличается от require_tree ровно предметом вопроса: не «лежит ли модуль внутри
# монорепо», а «достижим ли второй операнд». Проверяются ТРИ вещи, и третья —
# несущая: выборка, лежащая вне корня обхода, обходом не видна вовсе, и сверка
# молча объявила бы находками пять модулей без манифеста. Условие, созданное
# неверно, обязано быть отличимо от несозданного.
#
