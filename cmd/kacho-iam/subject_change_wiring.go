...
func buildSubjectChangeDrainer(
	...
	d, err := drainer.New[clients.SubjectChangeEvent](
		...
		drainer.Config{
			...
			// Постоянный отказ применения НЕ травится (kacho#455). Предмет потери
			// тут тише всего остального: недоставленная строка означает, что край
			// продолжает отвечать по УСТАРЕВШЕМУ вердикту о доступе — то есть ровно
			// тот эффект, ради предотвращения которого очередь и заведена, — а
			// снаружи «доставлено» и «отвечает по старому» неотличимы.
			//
			// Причина постоянного отказа здесь — наша же настройка: отказ в правах
			// на внутреннем листенере края (наш сертификат не в круге доверенных
			// отправителей) либо отвергнутая форма запроса. Оба снимаются перекатом,
			// то есть событием, которого дренаж не видит; травление сделало бы
			// временную причину бессрочной потерей. Повтор с отступом стоит одного
			// вызова раз в BackoffMax и виден счётчиком незавершённых строк.
			//
			// Отказ разбора травится по-прежнему — и с миграции 0097 записать
			// строку, на которой он откажет, НЕЛЬЗЯ: payload обязан быть непустым
			// объектом jsonb с непустым subject_id. Проверяется пробой
