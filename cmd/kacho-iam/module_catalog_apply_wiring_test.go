...
// # Предмет
//
// Применитель каталога — единственный писатель строк `kacho_iam.catalog_*` в
...
const moduleCatalogPkgPath = "github.com/PRO-Robotech/kacho/services/iam/internal/apps/kacho/modulecatalog"
...
func TestIAM1034_CatalogApplierBuiltByTheRootIsAlsoCalledByIt(t *testing.T) {
	...
	if census.Built == 0 {
		t.Fatalf("композиционный корень НЕ СТРОИТ применителя каталога вовсе "+
			"(разобрано %d файлов) — строки `kacho_iam.catalog_*` наполняет посев миграции, "+
