...
func runServe(cfg config.Config) error {
	...
	// с курсором: `sent_at`/`attempt_count`/`last_error` больше не пишет никто, и
	// читатель на них не смотрит by construction (выборка идёт по `id > $1`).
	...
	// потребитель читает журнал курсором и `sent_at` не трогает.
	//
