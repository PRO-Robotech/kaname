...
// Коды выхода — то, ЧЕМ накатчик называет класс своего отказа читателю-машине.
//
// Читатель исхода наката — шаг конвейера и init-контейнер, а не человек: он
// обязан отличить «условие не создано» (базы нет, вердикта о дереве НЕТ НИ
// ОДНОГО) от «находка о дереве» (накат отвергнут по существу). Названный КОД —
// единственная форма этого различия, которая не зависит от того, какие слова
// попали в текст отказа: текст накатчика вкладывает в себя обеззараженную строку
// подключения, то есть ЭХО ВВОДА, и читатель, ищущий в нём слова, судил бы
// собственную посадку, а не причину отказа.
const (
	// exitFinding — НАХОДКА: отказ есть утверждение о дереве. Неверная посадка,
	// негодная миграция, отвергнутое предусловие, ответивший сервер, который
	// назвал негодный пароль или отсутствующую базу.
	exitFinding = 1
	// exitUnmet — УСЛОВИЕ НЕ СОЗДАНО: базы на месте нет, и о миграциях не
	// известно НИЧЕГО. Величина совпадает с той, которой этот класс называют
	// прочие шаги конвейера, — иначе читателю пришлось бы держать вторую таблицу.
	exitUnmet = 75
)
...
// exitCodeFor — класс отказа наката, названный ТЕМ ЖЕ предикатом, которым накат
// решает, имеет ли смысл ждать.
//
// СЛОВАРЬ ОДИН, И ЭТО НЕСУЩЕЕ СВОЙСТВО. Класс «база ещё не принимает соединения»
// уже объявлен в дереве — `dbready.IsNotReady`, — и объявлен узко: транспорт
// (dial/DNS/обрыв), класс SQLSTATE 08 и явные «сервер стартует / выключается /
// перегружен». Негодный пароль, отсутствующая база, отказ привилегий и любой
// отказ самих миграций в него НЕ входят и входить не должны: их ожидание не
// лечит. Своя редакция того же класса здесь разошлась бы с барьером готовности
// молча — накат ждал бы две минуты и объявлял бы исход по другому правилу, чем
// то, по которому ждал.
//
// ЧЕГО ЗДЕСЬ НЕТ И НЕ БУДЕТ — разбора текста отказа. Сообщение наката вкладывает
// в себя обеззараженную строку подключения, то есть ЭХО ВВОДА, и поиск слов в
// нём отвечает на вопрос «что оператор подставил», а не «почему отказано».
//
// Класс доезжает до читателя ЧЕРЕЗ ОБЁРТКИ: открытие базы оборачивает барьер
// готовности, барьер — последнюю причину, и обе обёртки через `%w`. Поэтому
// предикат зовётся на цепочке целиком, а не на её верхнем звене.
