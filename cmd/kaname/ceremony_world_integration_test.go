...
// отпечаток b755886f…, решение PRO-Robotech/kacho#2721, задача этой службы
// #423): церемония OAuth 2.1 `authorization_code` нашими силами.
//
// ─────────────────────────────────────────────────────────────────────────────
// ОСНОВА — ВЕТКА ЭПИКА `357`, А НЕ СВОЙ ДВИЖОК
//
// Движок церемонии — фундамента (`corelib/oauthceremony`): его словарь
...
//
// Шаг посева, выразимый только ВМЕСТЕ с предметом (перенос срока записи кода:
// записи кода рождает только испытуемый), исполняется после ступени 2 и
// отказывает словом «НЕ-ВЫПОЛНИЛОСЬ(фикстура)» — за отсутствие возможности он
// себя выдать не может: к нему доходит только проба, у которой возможность есть.
//
// ─────────────────────────────────────────────────────────────────────────────
...
const (
	...
	// Виды выдачи — словарь движка фундамента, а не выписанные слова.
	grantAuthorizationCode = string(oauthceremony.GrantAuthorizationCode)
	grantRefreshToken      = string(oauthceremony.GrantRefreshToken)

	// lineA1ClientAuth — способ аутентификации конфиденциального клиента
	// (приёмка Р3): секрет в заголовке Basic, словом движка. Схема `357`
	// требует способ у КАЖДОГО клиента и допускает проверочное значение
	// секрета только при способе секретом
	// (`interactive_clients_secret_verifier_method_ck`).
	lineA1ClientAuth = string(oauthceremony.ClientAuthBasic)

	// lineA1CodeStore — хранилище записей кода волны-2 (миграция
	// `20260920175117`). Имя выписано, а его единственность по признаку
	// приёмки (`code_challenge`, Р5) сверяется в мире: второе хранилище кода
	// разделило бы записи, и счёт записей ослеп бы молча.
	lineA1CodeStore = "kaname.authorization_codes"
...
func (w *ceremonyWorld) requireStorePremise() {
	w.t.Helper()
	for _, rel := range []string{"kaname.users", "kaname.accounts", "kaname.human_sessions", "kaname.interactive_clients", lineA1CodeStore} {
		var present bool
	...
	w.requireCodeStoreIsTheOnlyOne()
}

// requireCodeStoreIsTheOnlyOne — хранилище кода по признаку приёмки (запись
// связывает `code_challenge`, Р5) ровно одно, и это lineA1CodeStore; у него
// есть колонки, которыми мир переносит срок. Иначе счёт записей кода у проб
// смотрел бы не туда, куда пишет испытуемый.
func (w *ceremonyWorld) requireCodeStoreIsTheOnlyOne() {
	w.t.Helper()
	...
	for rows.Next() {
		...
		names = append(names, "kaname."+n)
	}
	rows.Close()
	if err := rows.Err(); err != nil {
		w.fixture("каталог о хранилище кода: %v", err)
	}
	if len(names) != 1 || names[0] != lineA1CodeStore {
		w.fixture("хранилище кода по признаку `code_challenge` — %v, ожидалось ровно [%s]", names, lineA1CodeStore)
	}
	for _, col := range []string{"issued_at", "expires_at"} {
		var present bool
		if err := w.pool.QueryRow(w.ctx, `
			SELECT EXISTS (SELECT 1 FROM information_schema.columns
			WHERE table_schema = 'kaname' AND table_name = $1 AND column_name = $2)`,
			strings.TrimPrefix(lineA1CodeStore, "kaname."), col).Scan(&present); err != nil {
			w.fixture("каталог о сроке кода: %v", err)
		}
		if !present {
			w.fixture("у %s нет колонки %s — срок кода мир перенести не сможет", lineA1CodeStore, col)
		}
	}
}
...
func (w *ceremonyWorld) buildSurface() {
	...
	mux, err := registrytokenwire.Build(w.pool, registrytokenwire.BuildConfig{
		...
		// Предел одного обращения авторитета о базовом секрете — ТОТ ЖЕ, что
		// у корня (`serve.go`): величина корня, а не своя.
...
func (w *ceremonyWorld) codeRecords() []string {
	w.t.Helper()
	rows, err := w.pool.Query(w.ctx, `SELECT row_to_json(c)::text FROM `+lineA1CodeStore+` c`)
	if err != nil {
...
func (w *ceremonyWorld) ageCodes() {
	w.t.Helper()
	// Срок записи стоит строго после её выдачи
	// (`authorization_codes_expiry_after_issue_ck`): переносятся оба момента.
