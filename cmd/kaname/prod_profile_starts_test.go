...
func TestProductionProfileSatisfiesTheStartupGuards(t *testing.T) {
	...
	// ЧТО ЭТА ПЕРЕПИСЬ УТВЕРЖДАЕТ, А ЧТО НЕТ — сказано прямо (#2641).
	//
	// Оба числа приходят ИЗ ОДНОГО перечня, поэтому их равенство не может
	// покраснеть ни при каком составе дерева: утверждается здесь ровно то, что
	// боевой профиль объявил АДРЕС каждому ребру, которое перечень называет, —
	// и ничего о ПОЛНОТЕ самого перечня.
	//
	// Полноту держит отдельный гейт, и там числа приходят из РАЗНЫХ источников:
	// `TestIAM2641_EveryNonGRPCSurfaceIsJudgedByATransportGuard` сверяет длину
	// перечня с числом поверхностей, ВЫВЕДЕННЫМ разбором корня, и ведёт
	// самоистекающую ведомость поверхностей, судимых отдельным стражем. До него
	// эта строка читалась как утверждение о покрытии и была формой без
	// содержания: перечень сверялся сам с собой.
