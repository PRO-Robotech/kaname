...
import mc "github.com/PRO-Robotech/kaname/internal/apps/kaname/modulecatalog"
...
func TestIAM1034_InjectionRedsTheUncalledApplierAndKeepsQuietOnTheCalledOne(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			files: []wiringFile{{"cmd/kaname/serve.go", applierBuiltAndCalledDirectly}},
		},
		{
			name:  "передан применяющей провязке — молчание (живая раскладка)",
			files: []wiringFile{{"cmd/kaname/serve.go", applierHandedToAnApplyingHelper}},
		},
		{
			name:  "построен и НЕ позван — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", applierBuiltNeverCalled}},
			finds: true,
		},
		{
			name:  "под псевдонимом и НЕ позван — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", applierAliasedNeverCalled}},
			finds: true,
		},
		{
			name:  "передан посреднику-ПУСТЫШКЕ — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", applierHandedToAHollowHelper}},
			finds: true,
		},
		{
			name:  "посредник применяет ЧУЖОЙ тип — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", applierHandedToAHelperOfAnotherType}},
			finds: true,
		},
		{
			name:  "применитель отброшен связыванием — молчание",
			files: []wiringFile{{"cmd/kaname/probe.go", applierDiscarded}},
		},
		{
			name: "построен в одном файле, позван в другом — молчание",
			files: []wiringFile{
				{"cmd/kaname/serve.go", applierBuiltNeverCalled},
				{"cmd/kaname/apply.go", applierCalledElsewhere},
			},
		},
		{
			name:  "проба, строящая применитель, предметом не является — молчание",
			files: []wiringFile{{"cmd/kaname/serve_test.go", applierBuiltNeverCalled}},
		},
	}
