...
func TestIAM2010_InjectionRedsTheUncalledRolesApplierAndKeepsQuietOnTheCalledOne(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierBuiltAndCalledDirectly}},
		},
		{
			name:  "передан применяющей провязке — молчание (живая раскладка)",
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierHandedToAnApplyingHelper}},
		},
		{
			name:  "построен и НЕ позван — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierBuiltNeverCalled}},
			finds: true,
		},
		{
			name:  "под псевдонимом и НЕ позван — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierAliasedNeverCalled}},
			finds: true,
		},
		{
			name:  "передан посреднику-ПУСТЫШКЕ — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierHandedToAHollowHelper}},
			finds: true,
		},
		{
			name:  "посредник применяет ЧУЖОЙ тип (каталожный) — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", rolesApplierHandedToAHelperOfAnotherType}},
			finds: true,
		},
		{
			name:  "применитель отброшен связыванием — молчание",
			files: []wiringFile{{"cmd/kaname/probe.go", rolesApplierDiscarded}},
		},
		{
			name: "построен в одном файле, позван в другом — молчание",
			files: []wiringFile{
				{"cmd/kaname/serve.go", rolesApplierBuiltNeverCalled},
				{"cmd/kaname/roles.go", rolesApplierCalledElsewhere},
			},
		},
		{
			name:  "проба, строящая применитель, предметом не является — молчание",
			files: []wiringFile{{"cmd/kaname/serve_test.go", rolesApplierBuiltNeverCalled}},
		},
	}
