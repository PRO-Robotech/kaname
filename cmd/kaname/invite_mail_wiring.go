...
//
// Основание: приёмка доставки писем ID-MAIL-1, Р23 и Р25;
...
// попытки не наблюдался бы ни в одном исходе.
//
// ОБЪЯВЛЕНА ОДНАЖДЫ НА ОБЕ ПОЛОСЫ (kacho#2490). Её применяет и дренаж
// компенсаций (`provider_compensation_wiring.go`), где терпение выводится из
// предела административного клиента. Второй запас об одном предмете разошёлся
// бы с этим молча — тем же способом, каким разошлись 5 с терпения и 10 с
// предела клиента до того, как вывод стал выводом.
