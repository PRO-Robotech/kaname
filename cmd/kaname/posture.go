...
// gRPC-слушателей у процесса два, но рядом с ними живут не-gRPC
// поверхности (число выводится, а не выписывается — см. врезку ниже), своя
// дверь решения о доступе (iam — владелец модели, а не её
...
// не «ровно два», как знает дескриптор, а два gRPC плюс не-gRPC поверхности со
// своими профилями ([servicecontract.Surface]), и объявить их парой полей
// значило бы описать форму, которой у процесса нет. Ось названа остатком, а не
// закрыта молча.
//
// Числа не-gRPC поверхностей здесь НЕТ намеренно: оно выводится из дерева
// (`httpSurfaces` в `runServe`) и растёт молча, а у числа в комментарии нет
// владельца — прежняя редакция говорила «четыре» при шести.
func describePosture(cfg config.Config, logger *slog.Logger) (servicecontract.Descriptor, error) {
	...
	return servicecontract.New(servicecontract.Spec{
		...
		OwnContour: "контур входящего пути kaname собран в его композиционном корне: " +
			"два gRPC-слушателя плюс не-gRPC поверхности со своими профилями, " +
