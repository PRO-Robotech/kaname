...
import (
	"github.com/PRO-Robotech/kaname/internal/apps/kaname/modulecatalog"
	otherapp "github.com/PRO-Robotech/kaname/internal/apps/kaname/api/cluster"
)
...
import (
	ma "github.com/PRO-Robotech/kaname/internal/apps/kaname/api/module"
	mc "github.com/PRO-Robotech/kaname/internal/apps/kaname/modulecatalog"
)
...
import "github.com/PRO-Robotech/kaname/internal/apps/kaname/modulecatalog"
...
func TestIAM1034_VerbInjectionRedsTheUnhandedApplierAndKeepsQuietOnTheHandedOne(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			files: []wiringFile{{"cmd/kaname/wiring.go", verbApplierHandedToTheConsumer}},
		},
		{
			name:  "построен и НЕ передан — находка",
			files: []wiringFile{{"cmd/kaname/wiring.go", verbApplierBuiltNeverHandedOff}},
			finds: true,
		},
		{
			name:  "передан ЧУЖОМУ пакету — находка",
			files: []wiringFile{{"cmd/kaname/wiring.go", verbApplierHandedToAnotherPackage}},
			finds: true,
		},
		{
			name:  "под псевдонимом и НЕ передан — находка",
			files: []wiringFile{{"cmd/kaname/wiring.go", verbApplierUnderAnAlias}},
			finds: true,
		},
		{
			name:  "под псевдонимом и передан — молчание",
			files: []wiringFile{{"cmd/kaname/wiring.go", verbApplierUnderAnAliasHandedOff}},
		},
		{
			name:  "отброшен связыванием — молчание",
			files: []wiringFile{{"cmd/kaname/probe.go", verbApplierDiscarded}},
		},
		{
			name:  "проба, строящая применитель, предметом не является — молчание",
			files: []wiringFile{{"cmd/kaname/wiring_test.go", verbApplierBuiltNeverHandedOff}},
		},
	}
