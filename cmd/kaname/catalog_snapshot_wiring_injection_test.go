...
import cat "github.com/PRO-Robotech/kaname/internal/catalog"
...
func TestIAM1945_InjectionRedsTheUnstartedSnapshotAndKeepsQuietOnTheStartedOne(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			files: []wiringFile{{"cmd/kaname/serve.go", snapshotBuiltAndStarted}},
		},
		{
			name:  "построен и НЕ запущен — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", snapshotBuiltNotStarted}},
			finds: true,
		},
		{
			name:  "под псевдонимом и НЕ запущен — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", snapshotAliasedNotStarted}},
			finds: true,
		},
		{
			name:  "запущен СОСЕД, снимок нет — находка",
			files: []wiringFile{{"cmd/kaname/serve.go", snapshotNotStartedButNeighbourIs}},
			finds: true,
		},
		{
			name:  "снимок отброшен связыванием — молчание",
			files: []wiringFile{{"cmd/kaname/probe.go", snapshotDiscarded}},
		},
		{
			name: "построен в одном файле, запущен в другом — молчание",
			files: []wiringFile{
				{"cmd/kaname/serve.go", snapshotBuiltNotStarted},
				{"cmd/kaname/refresh.go", startedElsewhere},
			},
		},
		{
			name:  "проба, строящая снимок, предметом не является — молчание",
			files: []wiringFile{{"cmd/kaname/serve_test.go", snapshotBuiltNotStarted}},
		},
	}
