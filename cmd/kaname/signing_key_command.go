...
// служба: та же настройка, тот же страж своей чеканки и тот же страж посадки до
// первого соединения с базой, та же ключница с проверкой ключа обёртки — и
// меняет ТУ ЖЕ строку, которую читают публикатор и подписант каждой реплики.
// Своей копии набора у реплик нет, поэтому снятый ключ уходит из ответа
// публикатора со следующим запросом.
//
...
//	3  вердикта нет — вызов не разобран, настройка своей чеканки или посадка
//	   не приняты стражем (в том числе незаданный срок ключа), база или
...
func runSigningKeyCommandWithin(
	...
	// Настройку своей чеканки судит ТОТ ЖЕ страж, что у службы
	// (config.Config.Validate зовёт его же), и ДО первого соединения. Срок ключа
	// подписи не подставляется нигде (#321): незаданный, он иначе открыл бы
	// базу и получил отказ построителя ключницы его словами, а не словами
	// стража, которые оператор видит у службы. В процессе main зовёт страж
	// раньше; здесь он держит свойство самой команды, а не порядок вызовов.
	if err := cfg.AuthN.TokenSigning.Validate(); err != nil {
		_, _ = fmt.Fprintf(out, "настройка своей чеканки: %v\n", err)
		return signingKeyExitNotRun
	}
