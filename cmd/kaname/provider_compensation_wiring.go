...
//
// НУЛЕВАЯ ЗАДАЧА ПРИ НУЛЕВОЙ ОШИБКЕ — ЗАКОННЫЙ ТРЕТИЙ ИСХОД, а не забытая
// ветвь: на посадке без внешнего поставщика дренировать нечего и некуда (разбор
// — в теле). Вызывающий обязан проверить задачу на nil прежде, чем ставить её в
// очередь задач; «ошибки нет» тут не означает «задача есть».
//
...
func buildProviderCompensationDrainer(
	...
	// `external` и пережившие её перевод на `own`. Исполнить их нечем — адреса
