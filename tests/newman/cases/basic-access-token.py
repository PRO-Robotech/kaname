        ...
        Step(
            ...
            test_script=[
                ...
                *assert_status(200),
                ...
                # ФОРМА ОБЪЯВЛЕНА `pkg/credsecret`: `kacho_<id>_<26+6 крокфорда>`,
                # а идентификатор чеканит `ids.NewID` — префикс из ТРЁХ знаков и
                # тело СЛИТНО, без разделителя (`uocg44dzhw23p0bzwgeb`). Здесь
                # стояло `kacho_uoc_…`, с лишним подчёркиванием: утверждение
                # отвергало ровно то, что чеканит продукт, и совпасть не могло ни
                # при каком ответе. Первым его закрывал отказ в правах, поэтому
                # дефект не был виден. Контракт допускает и форму `uoc_<…>`
                # (pattern поля `id`), но её никто не производит — утверждается
                # производимое, иначе проба разрешает больше, чем существует.
                "pm.test('ответ несёт непустой секрет объявленной формы', () => {",
                "  pm.expect(_r.secret, 'response.secret').to.be.a('string')",
                "    .and.to.match(/^kacho_uoc[0-9a-hjkmnp-tv-z]{17}_[0-9a-hjkmnp-tv-z]{32}$/);",
                "});",
