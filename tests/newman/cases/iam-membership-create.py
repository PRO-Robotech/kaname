...
---------------------------------
  ...
  * разграничение переехало вместе с полем: чужой аккаунт — отказ ПРАВ, authz-first
    (край отвечает раньше сервиса), и он неотличим от отказа по несуществующему
    аккаунту; пустой аккаунт — тоже отказ прав, потому что вызов без объекта
    fail-closed by construction (обе двери: край и собственная дверь службы);
...
край, — 403/7, — а не то, что производил бы сервис за снятой дверью.
