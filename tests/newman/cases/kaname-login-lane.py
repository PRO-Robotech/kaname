...
ГДЕ ТРИ УСЛОВИЯ СОЗДАЮТСЯ. Стенд чарта посадки `own`
(`KANAME_STAND_IDENTITY_PROVIDER=own .github/scripts/stand-chart.sh up`, задание
`chart-own` процесса `e2e-newman.yml`) поднимает посадку накладкой оператора
поверх боевого профиля и выписывает клиентский лист с именем края; подкоманда
...
ТРЕТЬЯ КАТЕГОРИЯ НАЗВАНА ВСЛУХ. Остальные стенды дерева стоят на посадке
`external`: `.github/scripts/stand-own.sh` экспортирует
`KANAME_AUTHN__IDENTITY_PROVIDER=external` (его предмет — служба без края), а
`stand-chart.sh` без ручки ставит поставляемый профиль, и тот объявляет
`external`. Там `loginLaneBaseUrl` не задан, и КАЖДЫЙ шаг ниже уходит в «условие
не создано» помеченным утверждением, а не в зелёное и не в красное.
...
передаётся ДРУГОЙ лист — `_leaf edge` стенда посадки `own` с SAN края.
