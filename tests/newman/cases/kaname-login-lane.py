...
ТРЕТЬЯ КАТЕГОРИЯ НАЗВАНА ВСЛУХ. Стенды дерева стоят на посадке `external`:
`.github/scripts/stand-own.sh` экспортирует
`KANAME_AUTHN__IDENTITY_PROVIDER=external`, а `.github/scripts/stand-chart.sh`
ставит поставляемый профиль, и тот объявляет `external`. Сама посадка `own`
стартует — kaname#21 закрыт замером: чарт с накладкой
`authn.identityProvider: own`, слушатель полосы отвечает. Стенда, который
поднимал бы её, выписывал лист края и писал три переменные выше, в дереве нет —
это предмет kaname#183. Значит `loginLaneBaseUrl` не задан, и КАЖДЫЙ шаг ниже
уходит в «условие не создано» помеченным утверждением, а не в зелёное и не в
красное.
...
Клиентские листы обоих стендов выписаны на имя самой службы
(`stand-own.sh` — `v3_srv`, `stand-chart.sh` — `_leaf cli`), а не края. Слушатель
формы такому листу отвечает 403 `permission denied`, поэтому лист с SAN края —
второе условие, которое стенд посадки `own` обязан создать.
