...
# ТЕЛО ЗАХВАТЫВАЕТСЯ И СВЕРЯЕТСЯ СЫРЫМ — НА ОБЕИХ СТОРОНАХ (#2268).
# Здесь с обеих сторон стояло `JSON.stringify(pm.response.json())`: тело
# РАЗБИРАЛОСЬ и печаталось заново, то есть сравнивалась не пришедшая по проводу
# строка, а её пересборка. Форма была ОДНА на обеих сторонах, поэтому гейт
# `scripts/body_capture_form_test.py` молчал by construction: он требовал
# СОГЛАСИЯ форм и прямо объявлял, что КАКУЮ именно взять — решение автора.
#
# Для утверждения об АНТИ-ОРАКУЛЕ это решение неверно, и вот чем. Оба тела
...
# ПРОДУКТ, то есть это и есть настоящий оракул существования, и проба обязана на
# нём краснеть. Пересборка ровно эти различия НОРМАЛИЗУЕТ: `JSON.parse` +
# `JSON.stringify` печатают своё, а не то, что пришло. Утверждение было СЛАБЕЕ
# собственного заголовка: «тела совпадают» означало «совпадают их пересказы».
#
# Перепись дерева на день правки: захватов целого тела 16, сырых 15,
# пересобранный 1 — этот. Набор давно сошёлся на сырой форме, исключением был
# только он. Норма и замер, из которого она выведена (`protojson` + `detrand`:
# 74 и 76 байт на соседних ревизиях при побайтово одинаковых скриптах), — в
# `cases/kaname-own-rest-front.py` §«ТЕЛО ДЛЯ ПОБАЙТОВОГО СРАВНЕНИЯ»; здесь они
# не пересказываются. Впредь держит вторая ось того же гейта: захват ЦЕЛОГО тела
# обязан быть сырым, и пересобранный — находка независимо от согласия форм.
#
# ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ НЕПУСТОТЫ. «Тела совпадают» истинно ТРИВИАЛЬНО там, где
# поверхность отвечает одинаково на всё: равенство двух одинаково пустых ответов
# не свидетельствует ни о чём, а когда поверхность начнёт различать — утверждение
# перевернётся МОЛЧА. Поэтому рядом стоит наблюдение, различимое на ТОЙ ЖЕ
# поверхности ТЕМ ЖЕ предъявителем, и стоит оно ВНУТРИ того же `pm.test`:
# соседним `pm.test` контроль не работает — упавшее утверждение не мешает
# следующему пройти.
# ===========================================================================
...
CASES.append(Case(
    ...
    title="IAM-AB-SIA-10/11: ?accountId=<чужой> и ?accountId=<несуществующий, годной формы> отвечают "
          "200 с пустым перечнем и ТЕЛА совпадают ПОБАЙТОВО — различимый ответ здесь есть оракул "
          "существования аккаунта (сравниваются сырые тела, а не их пересборка, и не только коды)",
    classes=["NEG", "CONF"],
    priority="P0",
    steps=[
        Step(
            ...
            test_script=[
                *assert_status(200),
                ...
                "});",
                "pm.environment.set('siaForeignBody', pm.response.text());",
            ],
        ),
        # НАБЛЮДЕНИЕ ДЛЯ КОНТРОЛЯ НЕПУСТОТЫ: та же поверхность, тот же
        # предъявитель, негодная форма идентификатора — и ДРУГОЕ тело. Тем
        # самым доказано, что ответ зависит от входа, и равенство двух тел ниже
        # перестаёт быть истинным тривиально. Что производитель отказа здесь
        # ВЛАДЕЛЕЦ и текст его, закреплено порознь кейсами IAM-AB-SIA-12 и
        # IAM-AB-SIA-14; тут утверждается только различимость, поэтому текст не
        # дублируется — два места об одном предмете разошлись бы молча.
        Step(
            name="sia-discriminating-refusal",
            method="GET",
            ...
            test_script=[
                ...
                *assert_grpc_code(3, "INVALID_ARGUMENT"),
                "pm.environment.set('siaDiscriminatingBody', pm.response.text());",
            ],
        ),
        Step(
            ...
            test_script=[
                *assert_status(200),
                "pm.test('тела совпадают ПОБАЙТОВО: существование аккаунта по ответу не читается', () => {",
                "  const foreign = pm.environment.get('siaForeignBody');",
                "  pm.expect(foreign, 'тело ответа на чужой аккаунт не захвачено — сравнивать не с чем')"
                ".to.be.a('string').and.not.empty;",
                "  // >>> контроль непустоты множества, на котором проверяется отрицание",
                "  const disc = pm.environment.get('siaDiscriminatingBody');",
                "  pm.expect(disc, 'отказ формы не захвачен — контроля непустоты нет, и равенство ниже "
                "ни о чём не свидетельствует').to.be.a('string').and.not.empty;",
                "  pm.expect(foreign, 'поверхность отвечает ОДИНАКОВО и на годный вход, и на негодную "
                "форму: различать нечего, и утверждение ниже истинно тривиально — на пустом множестве')"
                ".to.not.eql(disc);",
                "  // <<< контроль непустоты",
                "  pm.expect(pm.response.text()).to.eql(foreign);",
                "});",
            ],
        ),
    ],
