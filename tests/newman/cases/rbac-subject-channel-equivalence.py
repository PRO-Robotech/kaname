...
def poll_op(op_var, out_id_var=None, auth="jwtAccountAdminA", allow_already_exists=False):
    ...
    return Step(
        ...
        test_script=[
            ...
            "  const _ipd1 = Date.now(); while (Date.now() - _ipd1 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */",
    ...
    return [
        ...
        Step(
            ...
            test_script=[
                ...
                f"if (!j.done && pc < {POLL_CAP}) {{ pm.environment.set('_{tag}DelCount', String(pc + 1)); const _ipd2 = Date.now(); while (Date.now() - _ipd2 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */ pm.execution.setNextRequest(pm.info.requestName); return; }}",
...
def poll_until_status(name, method, path, test_script, auth="jwtAccountAdminA",
    ...
    return Step(
        ...
        test_script=[
            ...
            f"if ((_retryCode || _retryPred) && _pc < {POLL_CAP}) {{ pm.environment.set('{counter_var}', String(_pc + 1)); const _ipd3 = Date.now(); while (Date.now() - _ipd3 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */ pm.execution.setNextRequest(pm.info.requestName); return; }}",
...
def check_until_deny(name, fga_subject, relation, obj, claim):
    ...
    return Step(
        ...
        test_script=[
            ...
            "  const _ipd4 = Date.now(); while (Date.now() - _ipd4 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */",
            "  pm.execution.setNextRequest(pm.info.requestName);",
            "  return;",
            "}",
    ...
    return [
        ...
        Step(
            ...
            test_script=[
                ...
                f"if (j && !j.done && c < {POLL_CAP}) {{ pm.environment.set('_{rev_op_var}Count', String(c + 1)); const _ipd6 = Date.now(); while (Date.now() - _ipd6 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */ pm.execution.setNextRequest(pm.info.requestName); return; }}",
    ...
    return [
        Step(
            ...
            test_script=[
                ...
                f"if (pm.response.code === 403 && _mc < {POLL_CAP}) {{ pm.environment.set('_{op_var}MCount', String(_mc + 1)); const _ipd7 = Date.now(); while (Date.now() - _ipd7 < 100) void 0; /* real inter-poll delay: cap 300 x 100ms ~= 30s budget (testing.md) */ pm.execution.setNextRequest(pm.info.requestName); return; }}",
