...
**Что этот замер НЕ закрывает.** Стенд поднят полосой развёртывания, а не сценарием
дерева: `.github/scripts/stand-own.sh` экспортирует
`KANAME_AUTHN__IDENTITY_PROVIDER=external`, и ни он, ни `stand-chart.sh` не
выписывают листа края и трёх переменных. В конвейере набор поэтому по-прежнему
уходит в третью категорию. Числовая половина предиката #183 выполнена этим замером
трижды; половина о стенде дерева — нет. Поэтому #183 остаётся открытой: её
оставшийся предмет — сценарий стенда посадки `own`, который выписывает лист края и
пишет три переменные, и числовой вердикт набора в конвейере на нём.
