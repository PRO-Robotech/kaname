...
**vpc (7) + compute (1) — `AUTHZ-*-LS-OWN-AAB` / `get-abs<N>`: LEAK CANARIES, not lag**

Correcting the attribution the removed entry itself carried: these folders are **not** in the
iam suite. They are generated by `services/vpc/tests/newman/cases/authz-deny.py` (NETWORK,
SUBNET, ADDRESS, ROUTE-TABLE, SECURITY-GROUP, GATEWAY, NIC) and
`services/compute/tests/newman/cases/authz-deny.py` (INSTANCE). The entry sat in this file
because the gate is shared, and the misfiling is worth naming: it is why the family read as
an iam timing quirk for as long as it did.
...
empty, scope-filtered page. `200` with any row is a **cross-account data leak**. These are
P1 negatives, and a name-keyed deduction was removing them.

Not lag, and the root cause is on record: the iam `rbac-subjects` suite creates a group in
the **shared** account-A, adds `userAABId` to it, and binds `ROLE_VIEW` on the account —
so while that grant is live, account→project containment makes project-A1's children
legitimately `v_list`-visible to AAB and the canary fails deterministically. Teardown for
both granting cases now exists in-tree, so residual red means teardown failed or a run
overlapped the live window — either of which is a finding, not a lag.

The `-abs<N>` suffix is `retry_until_absent` (budget 25 × ~500 ms ≈ 12.5 s, then the real
assertion runs once on the terminal response). So the reason-keyed mechanism was **already**
applied here; the deduction was a second, name-keyed layer on top of it, and it is the one
that could hide a persistent leak.
