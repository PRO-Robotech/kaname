...
## Запись прощения ОБЪЯВЛЯЕТ СЕБЯ — иначе её срок годности не проверяется ничем

Запись, объявляющая набор или кейс красным, пока открыт названный ею дефект, обязана
нести маркер отдельной строкой перед собой:

    <!-- known-failing: #<номер> — что именно прощается -->

Маркер читает гейт `tools/known_failing_expiry_test.go`: он спрашивает трекер про
состояние каждой названной задачи и **краснеет на закрытой**. Цена измерена (#2546):
запись про `#2191` пережила закрытие своего дефекта и продолжала объявлять набор
красным по причине, которая снята, — то есть утверждала о продукте неправду, и
утверждала долговечно.

Сверка сетевая и потому идёт за ручкой `KANAME_ISSUE_TRACKER_CHECK=1`; её состояние
называется на КАЖДОМ прогоне, поэтому «сверено 0» никогда не выглядит как «сверено».
Рядом стоит локальная проверка: абзац, объявляющий условие живым и не объявивший себя
маркером, — тоже находка, иначе формат остался бы тем, о чём никто не знает.

**Ноль записей прощения — ЦЕЛЬ, а не поломка**: на пустом перечне гейт проходит и
печатает перепись. Падение здесь подталкивало бы держать запись ради зелёного.

## Полоса входа паролем: стенд создаёт условие кодом дерева (kaname#183, 2026-09-23)

Три условия набора `kaname-login-lane` — посадку `own`, клиентский лист с именем
края и человека со способом входа паролем — теперь создаёт код дерева, а не
полоса развёртывания руками:

- `KANAME_STAND_IDENTITY_PROVIDER=own .github/scripts/stand-chart.sh up` ставит
  накладку оператора `own` поверх `deploy/values.prod.yaml` и выписывает лист края
  (Secret стенда `kaname-edge-client-tls`); `assert` сверяет ревизию в работающем
  контейнере и посадку по самоотчёту процесса (седьмая ось
  `identity_provider=own`) и по `pg_stat_ssl`;
- `stand-chart.sh seed-login-lane` выносит лист из Secret, поднимает переадресацию
  полосы на `127.0.0.1` и зовёт `tests/authz-fixtures/seed_login_lane.py`: человек
  заводится регистрацией на полосе, посев утверждает, что он **входит** паролем, и
  пишет `loginLaneBaseUrl`, `loginLaneEmail`, `loginLanePassword`. Учётные данные
  человека живут Secret'ом стенда `kaname-login-lane-human`, в дерево не попадают;
- задание `chart-own` процесса `e2e-newman.yml` исполняет эти шаги и гоняет набор.

**Воспроизводимость проверена на том же стенде, где набор мерили руками.** На kind
`kacho-own-2816` снято созданное руками — релиз `kaname` и пространство имён
`kaname` (соседнее пространство имён платформы не тронуто: 21 под `Running` до и
после); локальный файл окружения набора удалён, и посев создал его из шаблона.
Затем стенд поднят кодом ветки `183` @ `fd2133c51bf2771ef53ab84ae27255fc04d21ae3`,
по одному вызову на шаг:

| шаг | rc | что показал |
|---|---:|---|
| `up` | 0 | кластер чужой — контекст выписан в свой файл стенда; четыре листа, шесть секретов; образ собран с ревизией `fd2133c51`; выкат дошёл до готовности |
| `assert` | 0 | провенанс: файл ревизии в контейнере = ожидаемой, **сходится**; семь осей посадки сходятся, включая `identity_provider=own`; база: шифрованных соединений службы 17, открытым текстом 0 |
| `seed-login-lane` | 0 | человек заведён и входит паролем через полосу; записано 3 ключа |
| `run.sh --service kaname-login-lane` | 0 | вердикт ниже |

| прогон | коллекций | запросов | без ответа | утверждений | упавших | УСЛОВИЕ НЕ СОЗДАНО | исполнено |
|---|---:|---:|---:|---:|---:|---:|---:|
| 1 | 1/1 | 9 | 0 | 32 | 0 | 0 | 9/9 |

Строка вердиктного слоя: `kaname-login-lane: ran 9 request(s) / 32 assertion(s) — 0
failed (из них 0 УСЛОВИЕ НЕ СОЗДАНО, находок 0), 0 SCRIPT FAILED, 0 UNANSWERED`.

После `fd2133c51` ветка меняет в `stand-chart.sh` самопроверку (ось сноса чужого
кластера; признак полосы судится в УМОЛЧАНИЯХ координат, а не в фактических
координатах, которые задание `chart-own` называет само), строку сводки `up` и
выносит умолчания кластера, образа и каталога в именованные величины
`DEFAULT_CLUSTER`, `DEFAULT_IMAGE`, `DEFAULT_WORK`. Сами координаты от этого не
меняются: пролог скрипта до и после правки разрешает их в те же строки — и без
названных снаружи, и с координатами задания `chart-own`. Шаги `up`, `assert` и
`seed-login-lane` стоят на тех же координатах, что мерили выше. Предикат:
`git diff fd2133c51 -- .github/scripts/stand-chart.sh`.

**Область зелёного.** Одна коллекция, один прогон, дерево `fd2133c51` (продуктовый
код равен голове волны `cd8476112`: разница — только `.github/`, `tests/`,
`docs/`), стенд `kacho-own-2816`, пространство имён `kaname`. Задание `chart-own`
в конвейере на этой голове ещё не исполнялось — его вердикт снимается отдельно;
`kaname-recovery-lane`, `kaname-second-factor` и остальные коллекции этим прогоном
не исполнялись, их исход «не выполнилось».

## Полоса входа паролем: числовой вердикт на стенде посадки `own` (kaname#183, 2026-09-23)

Набор `kaname-login-lane` впервые исполнен против живого слушателя формы, а не в
третьей категории. Три прогона подряд, одно дерево, один стенд.

**Против чего гоняли — провенанс снят ДО прогона.**

- стенд: kind `kacho-own-2816`, служба отдельным релизом в пространстве имён
  `kaname`, посадка `authn.identityProvider: own`, свой домен доверия стенда;
  образ `kaname:stand-183-cd8476112eb4`;
- `/etc/kacho/image-revision` в работающем контейнере —
  `cd8476112eb48a8f0042fa4b56bc0a9921405565`, метка образа
  `org.opencontainers.image.revision` — та же. Исход сверки: **сходится**;
- дерево прогона — ветка `183` @ `24fa6edc9d8bded6afa3e91341e195d4d4f4d53f` (в неё
  влита голова `365`, та же ревизия `cd8476112`). От ревизии образа оно отличается
  только шапкой `cases/kaname-login-lane.py`; `gen.py` даёт ноль разницы в коллекциях.

**Три условия предиката и как они доехали до прогона.**

| условие | чем создано | чем проверено до прогона |
|---|---|---|
| клиентский лист с именем края | выписан УЦ стенда, лежит секретом стенда | лист края → `200` и печенье контекста; лист самой службы → `403` code 7 `permission denied`; без листа → обрыв рукопожатия (`certificate required`) |
| `loginLaneBaseUrl` | переадресация порта полосы на `127.0.0.1` | тот же запрос через ту же переадресацию |
| `loginLaneEmail`, `loginLanePassword` | посев стенда (секрет): человек со способом входа `password` | строка человека и его способа входа в базе службы |

Переменные вписаны в `environments/local.postman_environment.json` (файл в
`.gitignore`, в дерево не попадает), лист передан флагами прогонщика:

    scripts/run.sh --service kaname-login-lane \
      --ssl-client-cert <лист края> --ssl-client-key <его ключ> \
      --ssl-extra-ca-certs <УЦ стенда>

**Вердикт в числах** (newman 6.2.2; сводка `scripts/run.sh`, счёт исполненного и
категории — `scripts/assert-suites-green.sh` по тем же отчётам):

| прогон | коллекций | запросов | без ответа | утверждений | упавших | УСЛОВИЕ НЕ СОЗДАНО | исполнено | rc |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 1/1 | 9 | 0 | 32 | 0 | 0 | 9/9 | 0 |
| 2 | 1/1 | 9 | 0 | 32 | 0 | 0 | 9/9 | 0 |
| 3 | 1/1 | 9 | 0 | 32 | 0 | 0 | 9/9 | 0 |

Строка вердиктного слоя, одна и та же у всех трёх: `kaname-login-lane: ran 9
request(s) / 32 assertion(s) — 0 failed (из них 0 УСЛОВИЕ НЕ СОЗДАНО, находок 0), 0
SCRIPT FAILED, 0 UNANSWERED`. Сам гейт выходит кодом 1: у остальных 46 коллекций в
этом прогоне отчёта нет (гонялась одна) — это «не выполнилось» о них, а не вердикт.

**Область зелёного.** Зелёное выше — ровно о коллекции `kaname-login-lane` (фильтр
`--service kaname-login-lane`), о дереве `24fa6edc9` и о стенде с провенансом выше.
Остальные 46 коллекций дерева в этих прогонах не исполнялись: о них исход «не
выполнилось», не зелёный и не красный. Этим замером не проверены стенд самого
дерева и конвейер, а также `kaname-recovery-lane` и `kaname-second-factor`, которые
ждут того же условия стенда.

Коды по шагам, одинаковые у трёх прогонов: признак формы `200` · повтор `200` ·
неверный пароль `401` · вход `200` · признак выхода `200` · выход `200` · признак
смены пароля `200` · сохранённый носитель после выхода `401` · форма без признака `400`.

**Способность упасть: та же коллекция с листом самой службы вместо листа края.**
Исполнено 9/9, красных **26 из 32**, «условие не создано» 0 — отказ пришёл от
слушателя (`403`), а не от харнесса. Шесть утверждений под этой инъекцией остались
зелёными: три «носитель сессии НЕ выдан», два «печенье контекста не переставлено» и
«признак выхода отличается от признака входа». Каждое стоит в шаге, чьё утверждение
статуса покраснело, — зелёным не остался ни один шаг.

Под той же инъекцией шаги `logout` и `represent-ended-session` отправлены по 16 раз
(запросов 39 вместо 9): их оборачивает ограниченный повтор генератора по `403/404`
(окно материализации прав, 15 повторов). На трёх зелёных прогонах он не сработал ни
разу — 9 запросов на 9 шагов. Второй из этих шагов отрицательный (ждёт `401`), а
повтор на отрицании правило наборов запрещает. Это находка класса, а не этого
набора: по порождённым коллекциям дерева так обёрнуты 33 отрицательных шага в 12
коллекциях из 47 — предмет kaname#393. Этим замером обёртка не менялась.

**Недействительная попытка до трёх прогонов.** Первая переадресация встала только
на `[::1]`, а `127.0.0.1` того же порта в момент пробы обслуживал чужой процесс;
прогон по ней дал `ECONNREFUSED` на всех девяти запросах — «не выполнилось» о
харнессе, а не вердикт. Переадресация пересоздана с явным `--address 127.0.0.1` на
свободном порту, проба стенда повторена через неё, и три прогона выше идут по ней.

**Чего этот замер не закрывал.** Стенд этого замера поднят полосой развёртывания, а
не сценарием дерева: на дереве `24fa6edc9` ни `stand-own.sh`, ни `stand-chart.sh`
не выписывали листа края и не писали трёх переменных, и в конвейере набор уходил в
третью категорию. Эта половина снята кодом — раздел «стенд создаёт условие кодом
дерева» выше.

## Собственный REST-фронт: 7 утверждений перестали зеленеть на пустом множестве (2026-09-07)

**Прощение набора СНЯТО 2026-09-10 вместе со своим предметом (#2546).** Здесь стояла
запись, объявлявшая набор `kaname-own-rest-front` красным целиком до закрытия `#2191`.
Её условие выполнено — **`#2191` закрыт 2026-09-08** (`gh issue view 2191 --json state`
→ `CLOSED`), — и запись пережила свой предмет: она продолжала объявлять набор красным по
причине, которая снята. Всякий механизм исключения обязан истекать сам, поэтому запись
снята, а не подновлена.

Прежний оборот живого условия здесь намеренно НЕ воспроизводится дословно: цитата
снятого прощения читается и человеком, и распознавателем как действующее прощение —
тот же класс, ради которого запись и снимается.

**Что при этом НЕ утверждается — и это сказано прямо.** Снятие записи не означает
«набор зелен». Оно означает ровно то, что названная причина быть красным больше не
действует. Вердикт о наборе на развёрнутой посадке даёт `scripts/assert-suites-green.sh`
в джобе `e2e-newman`, и он **ничего не вычитает**: этот документ никогда не был маской
и на его вердикт не влияет.

**Состояние набора после фикса — третья категория, «не выполнилось», а не красное и не
зелёное.** Автор `#2191` записал это своими словами при закрытии: пара «страж старта»
проверена на живом стенде, а прогон самого набора там был третьей категорией (шаблон
пода старше двоичного файла), и вердикт откладывался до прогона накопительной линии в
ствол. Значит замера после фикса нет ни у кого; появится он с ближайшим прогоном линии.
Если набор окажется красным, это **новый предмет с новым номером и своим предикатом**,
а не продление снятой записи.

**Числа ниже относятся к прогону ДО фикса** (`#2191` открыт): из **55** утверждений
набора зелёными были **20**.
...
Семь красных были по предмету `#2191`.

**Предсказание так и осталось предсказанием** (перемерено 2026-09-10, #2546): замера
после закрытия `#2191` в этом документе нет, и число **13** ждёт своего прогона. Оно
названо здесь предсказанием намеренно — чтобы следующий читатель не принял его за
результат.
