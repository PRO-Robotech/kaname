...
После `fd2133c51` ветка меняет в `stand-chart.sh` только самопроверку (ось сноса
чужого кластера) и строку сводки `up`; шаги `up`, `assert` и `seed-login-lane`
исполняются тем же кодом, что мерили выше. Предикат:
