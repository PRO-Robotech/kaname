...
#
# СКОЛЬКО ЭТО ИДЁТ — ЧАСЫ, И ЭТО ИЗМЕРЕНО, А НЕ ВЗЯТО ИЗ КОНСТАНТЫ.
# Волна идёт столько, сколько живёт предъявитель. Замер на текущей посадке (2026-08-04)
# даёт 14400 с: `exp - iat` четырёх разных выпущенных токенов, настройка выдающего
# (`ttl.access_token: 4h`) и отсутствие per-client override у сервиса
# (`KACHO_IAM_SAKEY_ACCESSTOKENTTL` не задан) — три независимых источника, сходятся.
# В дереве при этом два места объявляли срок равным 900 с; число было ложным и
# ИСПРАВЛЕНО вместе с этой волной. Разница существенна для того, кто ставит расписание:
# «переждать 15 минут» и «переждать 4 часа» — разные решения.
#
# ПОЭТОМУ ВОЛНА НЕ ВХОДИТ В ОБЩИЙ ПАРАЛЛЕЛЬНЫЙ ПРОГОН. Она вызывается явно и
# заслуживает собственного расписания (ночной прогон / отдельная job). «Здесь его не
# запустить в общем прогоне» — факт расписания, а не вердикт: пока волна не отработала,
# её утверждения остаются ОТКРЫТЫМ ДОЛГОМ С ЧИСЛОМ, который печатает
# `tests/authz-fixtures/ceremony_credentials.py --debt`, а не зелёной суитой.
#
...
#   [SETUP_NS=kacho] [SKEW_S=30] [DELAY=…] ./scripts/run-expired-bearer.sh
