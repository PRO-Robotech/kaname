...
#
# СКОЛЬКО ЭТО ИДЁТ — ДЕСЯТКИ СЕКУНД, И ЭТО ИЗМЕРЕНО (2026-08-04).
# Волна идёт столько, сколько живёт предъявитель, — а живёт он ровно столько, сколько
# заказано КЛИЕНТУ ЭТОЙ ПРОБЫ. Замер: 29 с по стенным часам (срок 10 с + запас 20 с),
# положительный контроль 200 до ожидания, 401 после.
#
# ЗДЕСЬ СТОЯЛО «ЧАСЫ», И ЭТО ОПРОВЕРГНУТО ВЫЗОВОМ. Прежняя редакция объявляла срок
# равным 14400 с и выводила отсюда, что волна не может входить в общий прогон и требует
# собственного расписания. Замер умолчания был верен; ВЫВОД — нет: у выдающего есть
# per-client `client_credentials_grant_access_token_lifespan`, он действует, и клиенту,
# которого iam заводит под эту пробу штатным выпуском ключа, ставится срок в секунды.
# Общая настройка (`ttl.access_token` выдающего, `KACHO_IAM_SAKEY_ACCESSTOKENTTL`
# сервиса) НЕ трогается — менять её значило бы менять посадку стенда всем остальным,
# и этот запрет остаётся в силе.
#
