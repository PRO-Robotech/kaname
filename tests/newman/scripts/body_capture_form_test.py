...
Ось (а)/(б) НЕ требует конкретной формы: она требует, чтобы форма была ОДНА.

ОСЬ (в): ЗАХВАТ ЦЕЛОГО ТЕЛА ОБЯЗАН БЫТЬ СЫРЫМ — И ЭТО НЕ РЕШЕНИЕ АВТОРА
----------------------------------------------------------------------
Прежняя редакция объявляла форму решением автора: «набор вправе сравнивать хоть
сырое, хоть пересобранное, лишь бы одинаково». Для СОГЛАСИЯ форм это верно и
остаётся в силе. Для самой формы — неверно, и вот чем.

Целое тело захватывают ровно затем, чтобы сверить его с телом другого шага. Оба
приходят от ОДНОГО процесса в ОДНОМ прогоне, поэтому всякое различие их байтов —
порядок ключей, пробел после запятой, форма числа, экранирование — произвёл
ПРОДУКТ. Для утверждения об анти-оракуле («чужое и несуществующее отвечают
одинаково») это различие и есть предмет: оно и есть оракул, и проба обязана на
нём краснеть. Пересборка ровно эти различия НОРМАЛИЗУЕТ: `JSON.parse` +
`JSON.stringify` печатают своё, а не то, что пришло по проводу.

Значит выбора между двумя формами нет: пересобранная СТРОГО СЛАБЕЕ сырой —
она не видит ничего сверх неё и прячет часть того, что видит сырая. Утверждение
на ней зелено по совпадению, а его заголовок обещает больше, чем оно проверяет.

ЗАМЕР НА ДЕНЬ ЗАВЕДЕНИЯ ОСИ: захватов целого тела 16, сырых 15, пересобранный 1
(`siaForeignBody`, `iam-access-binding-account-scope`, предмет `kacho#2268`).
Набор сошёлся на сырой форме сам; ось закрепляет сошедшееся, а не вводит новое.

ВЕДОМОСТИ ИСКЛЮЧЕНИЙ У ОСИ (в) НЕТ — НАМЕРЕННО. Пустая ведомость есть механизм
без предмета, а непустую пришлось бы заводить вместе с первым законным случаем и
его причиной. Понадобится такой случай — правится сам гейт, и решение видно в
диффе, а не прячется строкой в перечне прощённых.
        ...
        for step, src in _scripts(doc):
            ...
            for m in _SET.finditer(src):
                ...
                if form:
                    ...
                    # (в) захват ЦЕЛОГО тела обязан быть СЫРЫМ. Пересобранная
                    # форма строго слабее: различие байтов двух ответов ОДНОГО
                    # процесса произвёл продукт, и она его нормализует.
                    if form == SERIALIZED:
                        findings.append(
                            f"{path}: '{var}' захвачено ПЕРЕСОБРАННЫМ телом на шаге "
                            f"«{step}» — `JSON.stringify(pm.response.json())` печатает своё, "
                            f"а не пришедшее по проводу, и нормализует ровно те различия, "
                            f"ради которых тело и сверяют; захватывать `pm.response.text()`")
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        # ИНЪЕКЦИЯ ОБЯЗАНА РОНЯТЬ ТОЛЬКО ПРОВЕРЯЕМОЕ. Поэтому дефект оси (а)
        # вносится в СРАВНЕНИЕ при сыром захвате, а дефект оси (в) — в захват при
        # согласных формах. Прежняя редакция (a) захватывала пересобранным и
        # роняла бы ОБЕ оси сразу: красное пришло бы от соседа, и вакуумность
        # одной из них ничем бы не показалась.
        print("(a) ось (а): захват сырой, сравнение пересобранное — НАХОДКА, и только эта")
        rc, out = _run(_collection("pm.response.text()", "JSON.stringify(pm.response.json())"), tmp)
        ...
        check("ось (в) молчит — захват сырой", "ПЕРЕСОБРАННЫМ телом" not in out, out)
        ...
        print("(b') ось (в): обе стороны ПЕРЕСОБРАННЫЕ — НАХОДКА, и только эта")
        # Прежде этот мир объявлялся законным («форма не навязывается, лишь бы
        # одна») и был единственным, в котором дефект #2268 оставался невидим:
        # согласие форм держалось, а сравнивались пересказы тел. Решение
        # отозвано осью (в), и отозвано ЗДЕСЬ, а не молча.
        rc, out = _run(_collection("JSON.stringify(j)", "JSON.stringify(pm.response.json())"), tmp)
        check("гейт краснеет", rc == 1, out)
        check("называет ось (в), а не смешение форм", "ПЕРЕСОБРАННЫМ телом" in out, out)
        check("называет переменную и шаг захвата", "'probeBody'" in out and "step-a" in out, out)
        check("ось (а) молчит — формы согласны",
              "а сравнивается формой" not in out, out)
        ...
        doc2 = _collection("pm.response.text()", "JSON.stringify(pm.response.json())")
        ...
        doc = {
            ...
            "item": [
                {"name": "step-a", "event": [{"listen": "test", "script": {"exec": [
                    "pm.environment.set('probeBody', pm.response.text());"]}}]},
                {"name": "step-b", "event": [{"listen": "test", "script": {"exec": [
                    "pm.expect(pm.response.text(), JSON.stringify(pm.response.json()))"
                    "  .to.eql(pm.environment.get('probeBody'));"]}}]},
            ],
        }
        # Стороны здесь ЗЕРКАЛЬНЫ прежней редакции: захват сырой, а пересобранное
        # тело стоит СООБЩЕНИЕМ. Прежде захват был пересобранным, и после
        # заведения оси (в) этот мир краснел бы по ней — то есть проба про
        # «сообщение не сравниваемое» доказывала бы чужое свойство.
        rc, out = _run(doc, tmp)
        check("пересобранное тело в СООБЩЕНИИ находкой не считается", rc == 0, out)
        doc["item"][1]["event"][0]["script"]["exec"] = [
            "pm.expect(JSON.stringify(pm.response.json()), 'msg')"
            "  .to.eql(pm.environment.get('probeBody'));"]
        rc, out = _run(doc, tmp)
        check("пересобранное тело в ЗНАЧЕНИИ по-прежнему находка", rc == 1, out)
