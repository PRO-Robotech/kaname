...
def prose_of_python(path: pathlib.Path) -> list[tuple[int, str]]:
    ...
    инъекции, чья причина пропуска — дословное отрицание приезда посева. Это
    фикстура гипотетического случая, а не утверждение о дереве, и запрет по
    тексту файла краснел бы на ней — то есть требовал бы переписать законную
    пробу.

    ОБОРОТ ЗДЕСЬ НАЗВАН ОПИСАНИЕМ, А НЕ ПРОЦИТИРОВАН, и это не осторожность.
    Прежняя редакция цитировала его дословно — и стала НАХОДКОЙ держателя
    (`pipeline_claims_test.py`) в тот момент, когда этот файл попал в индекс git:
    до того обход его не видел, потому что состав берётся из индекса. Текст,
    переехавший из файла, который держатель исключает по `__file__`, в файл,
    который он судит, меняет не смысл, а СУДЬЮ. Предикат по УЗЛУ различает
    прозу и литерал построением: проза — это
