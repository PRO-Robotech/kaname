...
def test_authz_fixture_seeds_paginate_list_by_scope():
    ...
    """
    # ПРЕМИССА — ОБХОД ФАЙЛОВ ПОСЕВА, А НЕ «НАШЛИСЬ ЧТЕНИЯ». Предмет этой пробы —
    # СКРИПТЫ ПОСЕВА, а не код службы, и посев в дереве службы ЕСТЬ:
    # `git ls-files -- tests/authz-fixtures | wc -l` → 2, из них скрипт посева
    # ОДИН — `seed_own_stand.py`; второй файл (`principal_pairings.py`) посевом
    # не является и в обход не идёт, см. `_AUTHORITY_ONLY` выше. Значит ветка
    # ниже описывает НЕ сегодняшнее дерево, а состояние, в котором обход не нашёл
    # ни одного скрипта: каталог унесли, переименовали, либо в нём остался один
    # авторитет.
    #
    # ЗДЕСЬ СТОЯЛО ОБРАТНОЕ — отрицание посева целиком, с предикатом снятия по
    # ИСТОРИИ каталога фикстур (счёт коммитов, достижимых из рефов копии). Такой
    # счёт не есть свойство дерева: одна и та же команда в ОДНОЙ копии отвечает
    # от нуля на местном `main` до пяти по всем рефам, — значит верного числа у
    # неё нет и утверждение на ней неопровержимо. Дословно снятое утверждение
    # здесь НЕ ПЕРЕСКАЗАНО: держатель
    # `pipeline_claims_test.py::test_seed_arrived_and_nobody_claims_it_absent`
    # читает прозу ВСЕГО дерева разбором, и цитата краснела бы как утверждение.
    #
    ...
    if not sources:
        # ТЕКСТ ГОВОРИТ О СВОЁМ ЗАМЕРЕ, А НЕ О РЕПОЗИТОРИИ. Прежняя редакция
        # выводила из пустого обхода, что посев в репозиторий не приезжал, —
        # вывод не следует из замера: каталог может нести один авторитет, и
        # обход пуст при живом дереве фикстур.
        pytest.skip(
            f"{PRECONDITION_MARK}: обход {AUTHZ_FIXTURE_DIR} не нашёл ни одного "
            f"скрипта посева (каталог "
            f"{'есть' if AUTHZ_FIXTURE_DIR.is_dir() else 'отсутствует'}); правило "
