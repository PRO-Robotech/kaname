...
def build(root: pathlib.Path, total: int, marked: int, plain: int) -> None:
    """Синтетический прогон: одна коллекция, один ИСПОЛНЕННЫЙ запрос, N утверждений."""
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print("ось 5 — без производителя метки гейт ОТКАЗЫВАЕТ, а не берёт умолчание")
        e = tmp / "e"; build(e, 5, 3, 0)
        fake = tmp / "fake"; fake.mkdir()
        shutil.copy(GATE, fake / GATE.name)
        shutil.copy(SCRIPTS / "exec-coverage.py", fake / "exec-coverage.py")
        rc, out = run_gate(e, fake / GATE.name)
        check("код 1 и отказ назван", 1, rc, out, "метку третьего исхода не прочитать")

    print()
    if FAILURES:
        print(f"ОТКАЗ: провалено утверждений {len(FAILURES)} из 7", file=sys.stderr)
        for x in FAILURES:
            print("  " + x, file=sys.stderr)
        return 1
    print("ЧИСТО: 7 утверждений — третий исход отличим кодом возврата, "
