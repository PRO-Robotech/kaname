...
# ПРИЗНАК ШАГА, УЖЕ АДРЕСОВАННОГО ИМЕНОВАННОМУ ФРОНТУ. Его ставит
# `require_env_url` первой строкой своего блока; по нему помощник ниже узнаёт
# шаг, чей фронт объявлен САМИМ кейсом (внутренний, выдачи, соседний), и не
# перебивает это объявление вторым присваиванием адреса.
_ENV_URL_MARK = "// HARNESS-CONFIG GUARD — "

# Адрес собственного ПУБЛИЧНОГО фронта службы на автономном стенде.
OWN_FRONT_VAR = "ownRestBaseUrl"


def address_own_front(cases: List["Case"], why: str) -> List["Case"]:
    """Переадресовать КАЖДЫЙ шаг модуля на собственный публичный фронт службы.

    ПРЕДМЕТ. Решение владельца (e2e-flow.md §7а): сущности службы проверяются в
    её репозитории на её собственном фронте, без края платформы. Генератор
    приписывает `{{baseUrl}}` каждому шагу автоматически, а шаги модуля делают не
    только сами кейсы, но и помощники (опрос операции, чтение до исчезновения,
    ожидание видимости) — их путь автор кейса не пишет и переадресовать поимённо
    не может. Поэтому переадресация делается ОДНИМ проходом по готовым шагам.

    ПОЧЕМУ ЗДЕСЬ, А НЕ КОПИЕЙ В МОДУЛЕ. Проход одинаков у каждого переведённого
    модуля; копия в каждом разошлась бы с остальными молча — ровно тот класс,
    которым набор уже платил (`_UNRESOLVED_VAR_GUARD`, #1474).

    ЧЕГО ПОМОЩНИК НЕ ДЕЛАЕТ — И ЭТО РЕШЕНИЕ, А НЕ НЕДОДЕЛКА:

      * шаг, уже адресованный кейсом (`require_env_url` в его пред-скрипте —
        внутренний фронт, выдача токенов, соседняя поверхность), НЕ трогается:
        второе присваивание перебило бы объявление автора, и шаг ушёл бы не туда
        с зелёным видом. Такой шаг автор переадресует САМ, своей переменной;
      * переменная края `baseUrl` не переопределяется (гейт
        `scripts/own_front_env_test.py`): край и собственный фронт — две
        поверхности, и одно значение на обе сделало бы вердикт функцией того,
        чей стенд поднят.

    Пропавший адрес — отказ с меткой «условие не создано» и именем переменной
    (`require_env_url`), а не молчаливый пропуск. Возвращает те же `cases`, чтобы
    модуль мог написать `CASES = address_own_front(CASES, …)` одной строкой.
    """
    if not why:
        raise ValueError("address_own_front: причина переадресации обязательна — "
                         "без неё отказ «адреса нет» не называет, ЧТО потеряно")
    for case in cases:
        for step in case.steps:
            if any(line.startswith(_ENV_URL_MARK) for line in step.pre_script):
                continue
            step.pre_script = [*require_env_url(OWN_FRONT_VAR, step.path, why),
                               *step.pre_script]
    return cases
...
_INJECTED = {
    ...
    "address_own_front": address_own_front,
