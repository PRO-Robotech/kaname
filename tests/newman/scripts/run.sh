...
#   ./scripts/run.sh --service a --service b  # несколько коллекций, ОДИН вердикт
...
#
# ТРЕБУЕТ СТЕНДА, И ДЛЯ БОЛЬШИНСТВА КОЛЛЕКЦИЙ ЭТОТ СТЕНД — ПЛАТФОРМЕННЫЙ.
# Перепись адресатов печатает `scripts/own_front_env_test.py` («адресуются к
# ownRestBaseUrl N»): к собственным фронтам службы адресуется меньшинство модулей
# кейсов, остальные идут к краю платформы — через умолчание `baseUrl` либо через
# `internalBaseUrl`/`externalBaseUrl`. Число здесь не выписано намеренно: оно
# меняется с каждой переадресованной коллекцией, а у выписанного нет держателя.
# Край не транспорт: он
...
#          покрытия; контракты iam — ИЗ СВОЕГО ДЕРЕВА (`proto/kaname/`, приехали
#          сюда ступенью S0a, kacho#2617: прежде их брали у модуля-пина платформы,
#          и это требовало `go mod download`); и стенд по адресам из env: край
#          платформы для большинства модулей кейсов, собственные фронты службы
#          (:9098/:9099) для остальных (перепись — см. выше).
...
# SERVICES — коллекции, названные `--service`. Флаг повторяем: несколько коллекций
# гоняются ОДНИМ вызовом и дают ОДИН вердикт с ОДНОЙ сводкой.
#
# ПОЧЕМУ ОДИН ВЫЗОВ, А НЕ ВЫЗОВ НА КОЛЛЕКЦИЮ. Каждый вызов начинает с чистого
# `out/` (см. ниже — иначе устаревший отчёт воскресает фантомной суитой). Два
# вызова подряд поэтому СТИРАЮТ отчёт первого до того, как его прочтёт кто-то
# после: задание автономного стенда вычищает и выкладывает `out/` ОДНИМ шагом
# после прогона, и выложило бы только последнюю коллекцию — молча, при двух
# зелёных шагах.
SERVICES=()
BAIL=""
DELAY="100"
...
#
# ГРАНИЦА ШАПКИ ВЫВОДИТСЯ ИЗ ФАЙЛА, А НЕ ВЫПИСЫВАЕТСЯ ДИАПАЗОНОМ СТРОК. Здесь
# стояло `sed -n '6,48p'` при шапке, кончавшейся на строке 50: две последние её
# строки — про адресатов стенда — в справку не попадали, и всякая правка шапки
# сдвигала обрез ещё раз. Шапка — это сплошной блок комментария от строки 6 до
# первой строки, которая комментарием не является.
_usage() {
  awk 'NR < 6 { next } /^#/ { sub(/^# ?/, ""); print; next } { exit }' "$0"
  echo
  echo "Флаги: --service <stem> (можно повторить) | --bail | --delay <мс> | --jobs <n> (принимается и игнорируется) | --help"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --service)
      # Повтор одного имени — ОТКАЗ, а не молчаливое склеивание: коллекция прошла
      # бы дважды, её строка дважды вошла бы в сводку, и итог сосчитал бы один
      # прогон за два.
      for _s in ${SERVICES[@]+"${SERVICES[@]}"}; do
        if [[ "$_s" == "$2" ]]; then
          echo "ОТКАЗ: --service $2 назван дважды — строка сводки удвоила бы итог" >&2
          exit 2
        fi
      done
      SERVICES+=("$2"); shift 2 ;;
    ...
    --jobs)    shift 2 ;;
    # ВЗАИМНЫЙ TLS СОБСТВЕННЫХ ФРОНТОВ. Флаги по-прежнему уезжают в `newman`
    # (`EXTRA`), но предполёту нужны СВОИ: он спрашивает тот же фронт до первой
    # коллекции, и без клиентского листа его вопрос умер бы на рукопожатии —
    # то есть «условие не создано» пришло бы по неверной причине.
    --ssl-client-cert)    SSL_CLIENT_CERT="$2"; EXTRA+=("$1" "$2"); shift 2 ;;
    --ssl-client-key)     SSL_CLIENT_KEY="$2";  EXTRA+=("$1" "$2"); shift 2 ;;
    --ssl-extra-ca-certs) SSL_EXTRA_CA="$2";    EXTRA+=("$1" "$2"); shift 2 ;;
    *)         EXTRA+=("$1"); shift ;;
  esac
done
...
# КОРЕНЬ ДЕРЕВА — ВВЕРХ ДО ПРИЗНАКА, А НЕ ЧЕТЫРЕ СЕГМЕНТА ВВЕРХ. Здесь стояло
# `cd ../../../..`, верное ровно пока набор лежал на `services/iam/tests/newman`.
# После выноса службы набор лежит на `tests/newman`, и те же четыре уровня
# уезжают ДВУМЯ выше корня — наружу репозитория. Признак корня — `go.mod`
# (модуль у репозитория один); искать вверх сам каталог фикстур нельзя, иначе
# «корня нет» и «объявления нет» слились бы в один исход. Бутстрап тот же, что у
# `_stems_lib` выше, и неустраним по той же причине.
_tree_root() {
  local d="$1"
  while [[ "$d" != "/" && -n "$d" ]]; do
    if [[ -f "$d/go.mod" ]]; then
  ...
  echo "корень дерева не найден: вверх от '$1' нет ни одного go.mod" >&2
  echo "Это ОТКАЗ, а не пропуск: без корня пути фикстур были бы выведены наугад." >&2
  return 1
}
_CEREMONY_ROOT="$(_tree_root "$NEWMAN_DIR")" || exit 2
_CEREMONY_DECL="$_CEREMONY_ROOT/tests/authz-fixtures/ceremony_credentials.py"

# ОТСУТСТВИЕ ОБЪЯВЛЕНИЯ — ИСХОД, А НЕ ПРОПУСК. Прежде у условия не было ветви
# `else`, и признаком «волна не активирована» служило НЕПЕЧАТАНИЕ строки, — а
# отсутствующую строку не ищет никто. Тогда «волна не активирована» было
# неотличимо от «волна активирована и пуста», и коллекции, которым нужен
# человеческий предъявитель, молча уходили в ОБЩУЮ волну под машинным.
#
# ПЕРЕЧЕНЬ ТАКИХ КОЛЛЕКЦИЙ ЗДЕСЬ НЕ НАЗЫВАЕТСЯ, и это честно, а не лениво: его
# единственный производитель — то самое объявление, которого нет. Второй,
# выписанный здесь перечень был бы вторым местом об одном предмете и разошёлся
# бы с объявлением молча. Печатается ПРИЧИНА и координата, по которой предмет
# станет доступен.
if [[ ! -f "$_CEREMONY_DECL" ]]; then
  echo "[ceremony] УСЛОВИЕ НЕ СОЗДАНО: волна церемонии НЕ активирована —"
  echo "[ceremony]   нет объявления $_CEREMONY_DECL"
  echo "[ceremony]   коллекции, которым нужен человеческий предъявитель, идут в ОБЩЕЙ"
  echo "[ceremony]   волне под машинным; их перечень назвать нечем — его единственный"
  echo "[ceremony]   производитель и есть отсутствующее объявление."
elif ! command -v python3 >/dev/null 2>&1; then
  echo "[ceremony] УСЛОВИЕ НЕ СОЗДАНО: волна церемонии НЕ активирована — нет python3"
...
# ── ПРЕДПОЛЁТ: ГОДЕН ЛИ ПОСЕВ, КОТОРЫМ СОБИРАЕМСЯ ГОНЯТЬ ────────────────────
#
# Посев автономного стенда пишет предъявителей и `runId` ОДИН раз, и прогонщик
# их не обновляет. Отсюда два отказа, ни один из которых не есть вердикт о
# продукте, и оба по тексту неотличимы от дефекта: истёкший предъявитель даёт
# СПЛОШНОЙ 401 `credential is not accepted` (срок посева — 15 минут, замер на
# живом стенде), а повтор на израсходованном посеве — `ALREADY_EXISTS` от
# ресурсов прошлого прогона, чьё имя выведено из `runId`.
#
# ПРОВЕРКА СТОИТ ДО ПЕРВОЙ КОЛЛЕКЦИИ, а не после первого падения: причина,
# названная потом, стоит прогона. Логика вынесена в скрипт и доказана своей
# инъекцией (`scripts/seed_preflight.py --self-test`) — в объявлении здесь ни
# одного условия, которое нечем прогнать.
#
# ТРИ ИСХОДА, И КОД ЧИТАЕТСЯ КАК ДАННЫЕ, а не как условие продолжения: `set -e`
# оборвал бы прогонщик молча, до развилки.
_preflight_rc=0
python3 "$NEWMAN_DIR/scripts/seed_preflight.py" \
  --env "$ENV" \
  ${SERVICES[@]+"${SERVICES[@]/#/--stem=}"} \
  ${SSL_EXTRA_CA:+--ssl-extra-ca-certs "$SSL_EXTRA_CA"} \
  ${SSL_CLIENT_CERT:+--ssl-client-cert "$SSL_CLIENT_CERT"} \
  ${SSL_CLIENT_KEY:+--ssl-client-key "$SSL_CLIENT_KEY"} || _preflight_rc=$?
case "$_preflight_rc" in
  0) : ;;
  75)
    echo "ПРОГОН НЕ СОСТОЯЛСЯ: условие не создано (см. причину выше). Ни одна" >&2
    echo "коллекция не гонялась, вердикта о дереве нет НИ ОДНОГО." >&2
    exit 75 ;;
  *)
    echo "ПРЕДПОЛЁТ НЕ ИСПОЛНЕН (код $_preflight_rc) — о годности посева не" >&2
    echo "известно ничего; гоню дальше, но причину отказа читайте с этим в виду." >&2 ;;
esac
...
if [ -z "$PROTO_GLOB" ]; then
  ...
  #
  # КОРЕНЬ ДЕРЕВА НЕ УГАДЫВАЕТСЯ ЧИСЛОМ `../`, А ПРОВЕРЯЕТСЯ ПРИЗНАКОМ.
  #
  # Первая редакция этой правки поднималась на ТРИ уровня от `$NEWMAN_DIR`, взяв
  # его значение из головы, — а он равен `tests/newman` (рецепт делает
  # `cd "$(dirname "$0")/.."`), то есть три уровня выводят ВЫШЕ корня клона.
  # Ошибка дала не отказ пути, а третий исход покрытия («НЕ ИЗМЕРЕНО») в
  # конвейере: наблюдалось на прогоне задания «автономный стенд». Поэтому корень
  # опознаётся по `go.mod`, и предпосылка НАЗЫВАЕТСЯ, а не подразумевается.
  _tree_root="$NEWMAN_DIR/../.."
  if [ ! -f "$_tree_root/go.mod" ]; then
    echo "ОТКАЗ: корень дерева не опознан — в $_tree_root нет go.mod." >&2
    echo "        Покрытие НЕ ИЗМЕРЕНО: это не «ноль RPC», предмета у гейта нет." >&2
    echo "        Либо задайте COVERAGE_PROTO_GLOB явно." >&2
    exit 2
  fi
  for _cand in "$_tree_root/proto/kaname/cloud/iam/v1/*.proto"; do
    [ -n "$_cand" ] || continue
...
if [ -z "$PROTO_GLOB" ]; then
  ...
  echo "        Искали относительно опознанного корня: ${_tree_root:-<не опознан>}/proto/…" >&2
  echo "        Либо задайте COVERAGE_PROTO_GLOB явно." >&2
  exit 2
fi
