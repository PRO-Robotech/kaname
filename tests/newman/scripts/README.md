...
- `assert-suites-green.sh` — читает метку у производителя (не выписывает её) и
...
- `precondition_mark_test.py` (+ `precondition_mark_injection_test.py`) — держат
  единственность производителя: кейс, поставивший метку себе, вывел бы своё
  падение из находок без чьего-либо решения;
- `precondition_outcome_test.py` — гоняет НАСТОЯЩИЙ вердиктный гейт на
