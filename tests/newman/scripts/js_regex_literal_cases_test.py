...
образцу `tests/newman/scripts/*_test.py` (в дереве платформы к нему добавлен
`services/*/tests/newman/scripts/*_test.py`). Проба, положенная
...
# в декларациях. Замер 2026-09-07 на `3abe65f89` + эта правка (было 473, замер
# 2026-08-24 на `8a3e00f18`). Расти нельзя; закрыли место — опустите число тем
# же изменением.
#
# 473 -> 472: три подстановки метки третьего исхода, приехавшие вклейкой, ушли
...
def test_a_value_hostile_to_a_string_literal_does_not_break_the_title():
    ...
    значит покраснеть на чужом предмете, а починить одно место из 472 — «починить
