...
мирами.
  ...
  W_today   — маршрутизатор различает (промах пути → 404 `Not Found`, чужой
              метод → 405), а всё, что доходит до обработчика, отвечает ОДНИМ И
...
# Промах ГЛАГОЛА. Статус сведён с 501 к 405 решением задачи #2493 — обе внешние
# поверхности службы отвечают на него одинаково; ТЕЛО при этом прежнее дословно,
# потому что подменён обработчик ошибок маршрутизации, а не отображение кодов.
METHOD_MISS = (405, _j({"code": 12, "message": "Method Not Allowed", "details": []}))
...
HONEST_IN_W_TODAY = {
    ...
    ("nonsense-path-on-own-front",
     ...
     "владельца назвал бы ресурс и идентификатор"): "то же",
    ("known-path-wrong-method", "status 405"):
    ...
    ("known-path-wrong-method",
     "METHOD-MISMATCH: 405 фронта отличим от 405 выдачи токена — тело несёт код "
     "маршрутизатора, а перечня допустимых глаголов нет"):
        "различитель ДВУХ ПОВЕРХНОСТЕЙ, а не отрицание сведённого статуса: положительная "
        "половина (код маршрутизатора в теле) наблюдаема на этой полосе и в W_today, "
        "поэтому проходить ей положено",
