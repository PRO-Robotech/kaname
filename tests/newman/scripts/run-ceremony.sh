...
# объявлением tests/authz-fixtures/ceremony_credentials.py — по ключу предъявителя,
# которого машинный посев не куёт, тем же предикатом, что у переписи долга
# (.github/scripts/newman-suite-debt.py, `ceremony_need`). Выписанный перечень в
# этом репозитории уже расходился с деревом; выведенный разойтись не может.
#
# ПОЧЕМУ ЭТО ВОЛНА, А НЕ ШАГ ПОСЕВА ПЕРЕД ВСЕМИ. Три причины, все из дерева:
#   1. предъявитель человека живёт ограниченный срок (в дереве платформы этот класс
#      документирует PRO-Robotech/kacho:tests/authz-fixtures/prodrun.sh: матрица
#      старше ~10 минут чеканит токены, истекающие посреди прогона) — церемония
#      идёт непосредственно перед своими коллекциями;
...
# КОРЕНЬ ДЕРЕВА ИЩЕТСЯ ВВЕРХ ДО ПРИЗНАКА, А НЕ СЧИТАЕТСЯ СЕГМЕНТАМИ. Фиксированная
# глубина подъёма есть свойство РАСКЛАДКИ: в монорепо набор лежал на
# `services/iam/tests/newman`, и четыре вверх были корнем; после выноса службы он
# лежит на `tests/newman`, и те же четыре вверх уезжают ДВУМЯ уровнями выше корня —
# наружу репозитория, где читается несуществующий путь и молча выходит «предмета
# нет». Подъём до признака этого свойства не имеет by construction.
#
# ПРИЗНАК — `go.mod`: модуль у репозитория ровно один, поэтому ближайший
# объемлющий `go.mod` и есть корень дерева. Искать вверх САМ каталог фикстур
# нельзя: тогда «корень не найден» и «объявления нет» стали бы ОДНИМ исходом, а
# различать их — предмет этого места.
#
# Бутстрап неустраним и потому повторён у каждого прогонщика: общий слой нельзя
# найти его же средствами — та же причина, что у `_stems_lib` в `run.sh` и у
# `_kacholib_dir()` в `gen.py`. Согласие копий по ПРИЗНАКУ держит не этот
# комментарий, а `scripts/suite_root_ascent_test.py`, ось 3.
_tree_root() {
  local d="$1"
  while [[ "$d" != "/" && -n "$d" ]]; do
    if [[ -f "$d/go.mod" ]]; then
...
ROOT="$(_tree_root "$SUITE_DIR")" || exit 2
...
if [ ! -f "$SEED" ]; then
  ...
  echo >&2
  echo "Отчётов не оставлено НАМЕРЕННО: гейт tests/newman/scripts/assert-suites-green.sh обязан" >&2
...
# ─── 4. вердикт — ЧИСЛАМИ ────────────────────────────────────────────────────
# Форма и сигнатура те же, что у суитных агрегаторов дерева платформы
# (`aggregate_verdict <out_dir> <stem…>` → 0/1); там её судила общая инъекция
# PRO-Robotech/kacho:deploy/scripts/assert-verdict-aggregators-honest.sh. Здесь
# держатель — scripts/ceremony_verdict_test.py: чистый отчёт обязан дать 0, а
# упавшее утверждение / запрос без ответа / немой отчёт / отсутствующий отчёт /
# ненулевой код newman — ненуль, и проба доказывает инъекцией, что умеет упасть.
# Ни одно из пяти непрошедших состояний не вычитается и не объясняется.
aggregate_verdict() {
