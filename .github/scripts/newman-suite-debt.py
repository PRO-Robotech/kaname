...
PRODUCER_LEDGER: dict[str, tuple[str, str]] = {
    ...
    # Восстановление доступа (Ф5, kacho#1271): та же полоса; запись заведена
    # вместе с записью Ф12 — без неё ведомость расходилась с деревом и вердикт
    # о ЛЮБОЙ коллекции службы не выносился (kacho#1281 по дороге).
    "kaname-recovery-lane": ("A", "запрос кода восстановления и его предъявление на собственном слушателе формы службы (Ф5 Р2): один ответ при любом исходе, ключей `jwt…` не читает"),
