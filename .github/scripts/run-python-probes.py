...
образцов состава поэтому два (см. `DEFAULT_PATTERNS` ниже):
  ...
  * `tests/authz-fixtures/*_test.py` — пробы ПОСЕВА, который поднимает фикстуры
    ...
    ПОСЕВ В ЭТОМ РЕПОЗИТОРИИ ЕСТЬ, а проб ПО ЭТОМУ ОБРАЗЦУ у него ноль — и это
    разные утверждения. В дереве `tests/authz-fixtures/` три файла
    (`ls tests/authz-fixtures/*.py | wc -l` → 3): `seed_own_stand.py` — машинный
    посев автономного стенда, `seed_login_lane.py` — посев человека полосы входа
    на стенде посадки `own`, и `principal_pairings.py`. Файлов `*_test.py` среди
    них ноль (`ls tests/authz-fixtures/*_test.py 2>/dev/null | wc -l` → 0), потому
    что посев доказывает себя ИНЪЕКЦИЕЙ в форме `--self-test`, которую зовёт
    отдельный шаг конвейера ПЕРЕД тем, как поверить его вердикту, — та же форма,
    что у гейтов `.github/scripts/`, и она уместна там, где предмет один.

    Образец оставлен НАМЕРЕННО и виден в переписи на КАЖДОМ прогоне («по образцу
    tests/authz-fixtures/*_test.py: 0»), то есть ноль называется числом, а не
    молчанием: придёт проба посева в форме НАБОРА — образец найдёт её сам.

    ПРЕЖНЯЯ РЕДАКЦИЯ УТВЕРЖДАЛА ОБРАТНОЕ И ПЕРЕЖИЛА СВОЙ ПРЕДМЕТ: она отрицала
    наличие посева в дереве целиком и называла предикатом своего снятия ИСТОРИЮ
    каталога фикстур — счёт коммитов, достижимых из рефов копии. Число у такого
    предиката не есть свойство дерева, и это ЗАМЕРЕНО, а не выведено: одна и та
    же команда в ОДНОЙ копии отвечает по-разному в зависимости от того, какой реф
    ей назвать, — от нуля на местном `main` до пяти по всем рефам при пятнадцати
    рефах. Верного и неверного числа у неё поэтому нет, то есть утверждение на
    ней неопровержимо и замером не является. Предикат о ПОСЕВЕ читает ДЕРЕВО:
    `git ls-files -- tests/authz-fixtures | wc -l` → 3, и это то же число, что
    выше. Дословно отрицание здесь НЕ ПЕРЕСКАЗАНО намеренно: предикат держателя
    читает прозу этого файла РАЗБОРОМ, и цитата собственного снятого утверждения
    краснела бы как утверждение — тот же класс, что проверка, считающая своё
    объяснение.
    Держит факт проба `tests/newman/scripts/pipeline_claims_test.py`: она краснеет
    на утверждении об отсутствии посева, когда посев в дереве есть, — и молчит,
    когда его нет, потому что тогда утверждение ВЕРНО.
...
# часть проб набора судит фикстуры БОЕВОЙ посадки, чьего производителя в этом
# репозитории нет — машинный посев дерева поднимает АВТОНОМНЫЙ стенд, а не её.
#
