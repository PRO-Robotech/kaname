...
#     именем края для полосы входа. С kaname#337 одной этой строки мало: чарт
#     не собирает `own` без токен-эндпоинта платформы, и накладка несёт ещё
#     `authn.clientToken.enabled: true` и четыре его величины (INSTALL.md §1);
#     так перемерено 2026-09-23. Стенд посадки `own` со своим листом края —
#     предмет kaname#183.
