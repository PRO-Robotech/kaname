...
stand_env() {
  ...
  # СТАРТОВАТЬ на ней служба сегодня ОТКАЗЫВАЕТСЯ. Причин было ПЯТЬ; две сняты
  # задачей kaname#21 — корень больше не строит административную дорогу к
  # ВНЕШНЕМУ поставщику и не публикует запись зеркала его набора ключей на
  # посадке, где поставщика нет. Осталось ТРИ, и все три держит ОДИН предмет —
  # своего входа человека в этом дереве нет:
  #
  #   · не провязано хранилище способов входа человека  — задача kacho#1268;
  #   · не провязано хранилище его сессии               — задача kacho#1269;
  #   · 311 записей каталога требуют уровней доверия 1 и 2, которых полоса не
  #     предъявляет ни одного, — СЛЕДСТВИЕ первых двух, своего производителя
  #     не имеющее. Здесь стояло 323: число не перемерялось с той ревизии, на
  #     которой записано, а каталог с тех пор менялся. Перемерено 2026-09-14 —
  #     ДВАЖДЫ и разными источниками: пробой полос (команда ниже) и самоотчётом
  #     живого процесса при старте (`identity posture lane wiring`,
  #     `catalog_entries_demanding_a_raised_floor`).
  #
  # Обе задачи ОТКРЫТЫ (перемерено 2026-09-13), обе внутри эпика kacho#1266.
  # Это состояние ПРОДУКТА, а не стенда, и число здесь перемеряется командой, а
  # не помнится:
  #
