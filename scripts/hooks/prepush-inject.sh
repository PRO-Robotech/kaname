...
# ── исход рецепта, СХЛОПНУТЫЙ make, обязан быть восстановлен ─────────────────
#
# Замерено на make 4.4.1: рецепт, вышедший 1, 2 либо 3, даёт у make РОВНО 2.
# Значит без восстановления находка объявлялась бы несозданным условием, и
# читателя посылали бы «создать условие» там, где чинить надо дерево. Ось
# проверяется на СИНТЕТИЧЕСКОМ рецепте: настоящие цели для этого звать не надо,
# а один факт против близнеца виден только на управляемом входе.
# Табуляция стоит в ФОРМАТЕ printf, а не в аргументе: `%s` подстановку не
# разворачивает, и первая редакция писала литеральные два знака. Рецепт тогда не
# разбирался вовсе («missing separator»), то есть проба мерила СВОЮ поломку, а не
# предмет. Поймал это положительный близнец — он обязан был дать зелёное и дал
# красное; без него ось выглядела бы исправной.
mkmake() { # mkmake <каталог> <тело цели lint>
    mkrepo "$1" "$good"
    printf 'lint:\n\t%s\n' "$2" > "$1/Makefile"
    make -C "$1" -n lint >/dev/null 2>&1 ||
        { echo "  ПРОВАЛ фикстура $1: синтетический рецепт не разбирается" >&2
          failed=$((failed + 1)); }
}
mkmake "$base/mk-ok"    '@echo всё хорошо'
mkmake "$base/mk-find"  '@echo "поле такое-то негодно"; exit 1'
mkmake "$base/mk-void"  '@echo "УСЛОВИЕ НЕ СОЗДАНО: инструмента нет"; exit 2'

# (−) положительный близнец: цель прошла.
want 0 "(−) цель рецепта прошла — зелёное" \
    env -C "$base/mk-ok" KANAME_PREPUSH_GROUPS=lint "$hook"

# (+) один факт: цель отказала БЕЗ маркера — это НАХОДКА, а не условие.
want 1 "(+) отказ цели без маркера — КРАСНОЕ (схлопывание make восстановлено)" \
    env -C "$base/mk-find" KANAME_PREPUSH_GROUPS=lint "$hook"

# (+) один факт: та же цель, тот же код у make — но маркер сказан.
want 2 "(+) отказ цели С маркером — НЕ ВЫПОЛНЕНО, а не находка" \
    env -C "$base/mk-void" KANAME_PREPUSH_GROUPS=lint "$hook"

# Предпосылка самой оси: make ДЕЙСТВИТЕЛЬНО схлопывает. Перестанет — проба
# обязана сказать об этом, а не молчать: запрет, чья предпосылка отпала,
# становится ложью (и тогда run_make пора снимать, а не чинить).
probes=$((probes + 1))
mkmake "$base/mk-three" '@exit 3'
mkrc=0; ( cd "$base/mk-three" && LC_ALL=C make lint ) >/dev/null 2>&1 || mkrc=$?
if [ "$mkrc" -eq 2 ]; then
    echo "  ok   (предпосылка) make схлопывает код рецепта 3 → 2 — восстановление нужно"
else
    echo "  ПРОВАЛ предпосылка отпала: make отдал $mkrc, а не 2 — run_make пора пересмотреть" >&2
    failed=$((failed + 1))
fi
