...
# ВЕДОМОСТЬ ОСЕЙ — ЕДИНСТВЕННЫЙ источник и перечня, и числа. Объявленное число
# стоит здесь, исполненное набирается по ходу, и расхождение между ними — само
# по себе исход: ось, тихо не исполнившаяся, не отличалась бы от исполненной.
AXES_DECLARED=2
axes_executed=0
unmet_reasons=()
...
if [ "${#missing[@]}" -gt 0 ]; then
    axis_unmet "ось 2 (сквозной опыт) — нет в PATH: ${missing[*]}; настоящий хук прогнать не на чем"
else
