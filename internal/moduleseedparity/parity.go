...
//
// # Сверяются ВСЕ ЧЕТЫРЕ подраздела, и невыразимого вида строки не осталось
//
// Раздел несёт четыре подраздела — служебные записи, группы, выдачи и
// вступления, — и сверка судит каждый. Здесь стояло «наружу выведена только
// НЕВЫРАЗИМОСТЬ: у выдачи есть ключ `roleId` и нет ни одного ключа для
// ОТНОШЕНИЯ, поэтому выдача отношением необъявима ни при каком написании;
// группа необъявима по следствию» и называлась живой проба предпосылки
// `TestBindingFormStillCannotExpressARelationGrant`. Шапка пережила предмет:
// ключ `grantedRelation` заведён 2026-09-04 (#1936), проба сработала как
// обещала и снята вместе со своим предметом, предикат `ExpressibleByForm` и
// перечень `Inexpressible` — тоже (надгробия ниже). Всякая живая строка модуля
// судится наравне с остальными.
//
// # Число границы считается ПО ВЛАДЕЛЬЦУ, и это не педантизм
//
// Строка без модуля-владельца (`kacho-api-gateway`, `bootstrap-admin`,
...
// объявленного — верно, а не пробел. Перепись поэтому печатает по каждому
// подразделу четыре величины: объявлено · живых · без модуля-владельца · с
// владельцем. Одно число скрывало бы ровно тот случай, ради которого граница
// названа.
//
// # Владелец живой СЛУЖЕБНОЙ ЗАПИСИ выводится ИМЕНЕМ; владелец ГРУППЫ — ОБЪЯВЛЕНИЕМ
//
// Служебная запись модуля названа `kacho-<служба>`; служба переводится в модуль
// закрытого набора платформы. Запись, чьё имя этому не отвечает, манифестом
// модуля невыразима by construction — у неё нет модуля-владельца, — и считается
// отдельно. Иначе её отсутствие среди объявленных читалось бы как неполнота.
//
// Группа так не именуется: обе живые группы платформы носят приставку
// `module-`, а объявляет их манифест САМОЙ службы (приёмка MRW-1, Р5). Поэтому
// группа относится к модулю по объявлению — [GroupOwnership], ключ — пара
// (аккаунт, имя); выдача, чей субъект группа, — по объявлению этой группы. Ключ
// СРАВНЕНИЯ остаётся тройкой с назначением (`keysOfGroup`): относи строку той же
// тройкой — и дрейф назначения ушёл бы в разряд «без владельца», погасив обе
// стороны вердикта разом. Правило имени для групп отвергнуто, потому что не
// называет владельца: обе группы приставку несут, а объявляет их один манифест
// из шести.
...
// GroupOwnership — какой модуль ОБЪЯВИЛ группу: пара (аккаунт, имя) → модуль
// (приёмка MRW-1, Р5).
//
// Ключ — пара, а не имя и не тройка. Не имя: то же имя в другом аккаунте — другая
// группа. Не тройка: назначение принадлежит ключу СРАВНЕНИЯ, и, попав в ключ
// атрибуции, оно отправило бы строку с разошедшимся назначением в разряд «без
// владельца», где обе стороны вердикта молчат.
//
// Написания сравниваются СЫРЫМИ — окна двух написаний аккаунта здесь нет: оно
// существует ради непереведённых чужих манифестов и истекает с их переводом.
type GroupOwnership map[string]string

// Declare записывает объявление: манифест модуля `module` заводит группу
// `name` в аккаунте `account`.
func (o GroupOwnership) Declare(module, account, name string) {
	o[groupPairKey(account, name)] = module
}

// OwnerOf — модуль, объявивший группу; false — не объявил никто.
func (o GroupOwnership) OwnerOf(account, name string) (string, bool) {
	module, ok := o[groupPairKey(account, name)]
	return module, ok
}

func groupPairKey(account, name string) string {
	return strings.Join([]string{account, name}, "\x00")
}
