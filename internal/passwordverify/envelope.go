...
//     процесса (перенос иным процессом), приносит класс, которого перепись
//     старта не видела; первый вход по нему поднимает потолок, и окно оракула
//     равно одному обращению, а не времени до перезапуска. Известный класс —
//     поиск по ключу, без прогона;
//   - при записи — в этом дереве вызывающего НЕТ, и это сказано прямо: все
...
//     construction. Перенос П3 (`kacho#1268`), кладущий чужие классы, обязан
//     звать `Admit` на каждый класс до записи; повод `write` заводится вместе с
//     ним, а не заранее — клетка счётчика, которую нечем поднять, лгала бы о
//     провязке.
//
