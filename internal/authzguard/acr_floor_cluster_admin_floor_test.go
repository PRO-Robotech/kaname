...
//
// ПОЧЕМУ УТВЕРЖДАЕТСЯ ОТНОШЕНИЕ, А НЕ ЧИСЛА. Литерал здесь был бы третьим
...
func TestACRFloorClusterAdminFloorsComeFromTheContract(t *testing.T) {
	...
	if f := clusterAdminFloorFindings(reg); len(f) > 0 {
		t.Fatalf("пол ступени у InternalClusterService разошёлся с решением — %d находк(и):\n  %v\n\n"+
			...
			"(proto/kaname/cloud/iam/v1/internal_cluster_service.proto), и только он: шапка "+
			"`acr_floor.go` их не пересказывает намеренно — пересказ уже разошёлся с "+
			"контрактом однажды (kaname#131).", len(f), f)
	}
}
