...
//  3. производитель, чей домен ещё не взят у объявления, — их ТРИ, и все три
//     названы ведомостью ниже. Третья запись заведена ступенью S0a: производитель
//     приехал в это дерево из модуля платформы, и до переезда его не видел ни один
//     прогон этого репозитория — то есть слепая зона была не «разбор не умеет», а
//     «файла в дереве нет».
//
...
var ledger = []ledgerEntry{
	...
	{
		File: "pkg/subjectchange/positionlost.go",
		Why: "домен здесь — ПРОВОД, и его смена видна арендатору. Файл приехал в это дерево " +
			"ступенью S0a (kacho#2617, исход C), которая переносит дом Go-заглушек и " +
			"НЕ ПРАВИТ провод; до неё производитель жил в модуле платформы, где объявление " +
			"продукта недостижимо, и расхождение `iam.kacho.cloud` против `iam.kaname.cloud` " +
			"не видел ни один прогон. Правка домена — своё изменение со своей приёмкой: " +
			"клиент различает полосы по паре «домен + признак», и что он видит до и после, " +
			"обязано быть названо",
		Until: "grep -c 'iam.kacho.cloud' pkg/subjectchange/positionlost.go даёт 0",
		Owner: "PRO-Robotech/kaname#48",
	},
}
