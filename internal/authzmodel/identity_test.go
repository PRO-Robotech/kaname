...
//
// Без этого гейта у прав два источника, расходящихся молча: кластер применяет
// один текст, вердикт внутри iam выводится из другого. Разойдясь, они дадут не
// отказ, а РАЗНЫЕ решения о доступе — и заметно это станет по чужому доступу, а
// не по красному прогону.
...
func TestEmbeddedModelIsByteIdenticalToCanonical(t *testing.T) {
	...
	if string(canonical) != DSL {
		t.Fatalf("вшитая копия разошлась с %s (канон %d байт, копия %d байт). "+
			...
			"`make -C deploy fga-model-embed`. Пока они разные, кластер применяет один "+
			"текст, а вердикт внутри iam выводится из другого — и это не отказ, "+
