...
// `import` соседнего контракта, а не нами, и переименовать такой путь здесь
// значит не собрать контракты вовсе. Предмет axis 1 — «чем продукт зовёт СВОИ
// каталоги»; чужая координата к нему не относится.
//
// СЕГОДНЯ У ПОЛОСЫ ПРЕДМЕТА В ДЕРЕВЕ НЕТ, и это сказано, а не подразумевается.
// Единственный вход с отставленным сегментом в пути был контракт формы
// операции; он получил имя фундамента и лёг под нейтральный корень
// (`corelib/operation/operation.proto`, kacho#2601), поэтому перепись печатает
// «входных контрактов 0 путей». Полоса оставлена, потому что СВОИХ записей у неё
// нет: она читает ведомость входов, и путь, ушедший из ведомости, послабление
// теряет сам. Способность полосы молчать и находить держит пара проб на
// синтетическом корне в `directory_name_injection_test.go`.
//
