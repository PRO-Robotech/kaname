...
var platformResidual = []platformResidualEntry{
	{
		...
		Ground: "контракты САМОЙ службы. Решением владельца (2026-09-13, дословно: «прото так же " +
			...
			"уезжают в этот репозиторий (kacho#2616). ИСХОДНИКИ И ЗАГЛУШКИ УЖЕ ЗДЕСЬ: " +
			"`proto/kaname/` несёт 41 контракт, `pkg/api/kaname/` — 87 порождённых файлов, и " +
			"`buf generate` в этом дереве исполняется без дерева платформы. Переключить 251 узел " +
			"импорта на них ОДНИМ изменением нельзя, и это ЗАМЕР, а не осторожность: два " +
			"пакета этой же ведомости — `pkg/ownerregister/ownerregister.go` и " +
			"`pkg/subjectchange/reader.go` — импортируют платформенные заглушки своим " +
			"НЕ-тестовым кодом, поэтому вторая копия дескриптора приезжает ТРАНЗИТИВНО. " +
			"Бинарь службы, собранный с переключёнными импортами, даёт код 0 на `go build` и " +
			"панику регистрации при запуске. Значит запись снимается ВМЕСТЕ с двумя " +
			"следующими (kacho#2614, порядок и предикат — kaname#36), а не прежде них; " +
			"класс держит гейт " +
			"`internal/contracthome` TestBothCopiesOfAContractNeverReachOneBinary — он " +
			"называет оба носителя по именам",
	},
	{
