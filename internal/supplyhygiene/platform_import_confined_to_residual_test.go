...
var platformResidual = []platformResidualEntry{
	{
		...
		Ground: "контракты САМОЙ службы: их дом — этот репозиторий, а не платформа. " +
			"Снимается переездом контрактов (kacho#2131, отдельная полоса): после него " +
			"путь ведёт в собственный модуль, и эта запись обязана исчезнуть",
	},
	{
		...
		Ground: "производитель родительской цепи владения — ОДИН, и живёт он у платформы. " +
			...
			"импорт законен. Снимается kacho#2612",
	},
	{
		...
		Ground: "контракт журнала смен субъекта: потерянная позиция, порог уборки, читатель. " +
			...
			"тянула бы платформенный путь заново. Снимается kacho#2612",
	},
}
