...
const producerRootCount = 2

// producerRoots — где ищутся ПРОИЗВОДИТЕЛИ имён, названных документацией: ряды
// наблюдаемости, контракты gRPC. Корней ДВА, и ни один не лишний:
//
//	дерево службы     — то, что она объявляет сама, ВКЛЮЧАЯ свои контракты;
//	модуль фундамента — `grpcsrv`, `observability`, `operations`: всё, что
//	                    производит ряды транспорта и хранения.
//
// ИСТОРИЯ ЭТОГО ЧИСЛА — ЗАМЕР, А НЕ ВКУС, И ОНО МЕНЯЛОСЬ ДВАЖДЫ. Первая
// редакция знала два корня — дерево и `<платформа>/pkg`, — и на день заведения
// это было верно: фундамент физически лежал там. После выделения фундамента
// отдельным модулем прежний адрес стал давать остаток платформы, где `grpcsrv`
// уже нет: страница наблюдаемости назвала три ряда `kacho_grpc_server_*`, а
// указатель их не нашёл — «названо то, чего нет» прозвучало о существующем.
// Тогда корней стало три, и третьим был остаток платформы: контракты службы
// публиковал её модуль.
//
// Ступень S0a (kacho#2617, исход C) этот остаток сняла: контракты службы
// публикует её собственный модуль, и их имена даёт корень номер один. Третий
// корень стал ИСКЛЮЧЕНИЕМ, КОТОРОМУ НЕЧЕГО ИСКЛЮЧАТЬ, — а заодно и последним
// местом, откуда служба разрешала каталог платформенного модуля. Со снятым
// ребром он неразрешим в принципе (`go list -m` отвечает «not a known
// dependency»), поэтому его снятие — не ослабление, а восстановление предмета:
// корней снова два, и оба разрешимы.
