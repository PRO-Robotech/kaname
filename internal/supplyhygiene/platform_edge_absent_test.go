...
// `kaname → corelib`, `kacho → kaname`. Ребро `kaname → kacho` запрещено,
// и вместе с `kacho → kaname` оно образует КОЛЬЦО МОДУЛЕЙ: опубликовать нельзя
// ни один, потому что каждый ждёт версию другого.
//
