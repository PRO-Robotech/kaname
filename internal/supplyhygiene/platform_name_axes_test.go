...
var platformNameAxes = []PlatformNameAxis{
	...
	{
		// Здесь стояло ЧИСЛО — 88 вхождений в 5 файлах каталога миграций, — и оно
		// к нулю не шло НИКОГДА: пять собственных функций заведены применённым
		// сводом, а его не правят (ban #5). Переименование сделано новой
		// миграцией (`20260916184500`), после которой текст свода прежний, а
		// объект в базе — нет; ось поэтому судится по ЖИВОЙ схеме, а не по файлам.
		// Таблиц с приставкой платформы — ноль (и было ноль).
		Axis:    "таблицы",
