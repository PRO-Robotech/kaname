...
//
// # Кодировка — ДЛИНОЙ, а не разделителем, и это про безопасность
//
// Часть доводов приходит из тела запроса, то есть от арендатора. Разделитель —
// какой угодно — можно поставить внутрь значения, и тогда два РАЗНЫХ набора
// доводов дадут ОДИН ключ: два пункта сольются в прогон, и один из них будет
// отвечен по чужим доводам. Длина перед каждой частью делает кодировку
// однозначной by construction, поэтому подобрать столкновение нечем.
//
// Тип значения входит в ключ вместе со значением: число 1 и строка "1" печатаются
// одинаково, а условие их различает.
func runKeyOf(p checkPlan) string {
	...
	for _, k := range keys {
		writePart(k)
		writePart(fmt.Sprintf("%T=%v", p.condCtx[k], p.condCtx[k]))
	}
	return b.String()
}
