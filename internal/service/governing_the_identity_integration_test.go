...
func TestGoverningTheIdentityIsNotReachableFromInsideTheAccount(t *testing.T) {
	...
	//
	// Путь 1 — ВЫДАЧА внутри аккаунта, чья роль даёт правку личности: ровно то,
	// что реконсайлер материализует на свежей строке приглашённого (invite.go
	// зовёт reconcileObject "iam.user" сразу после коммита).
	seedRoleGrantingUserEdit(t, w, "rol-govid1", "acb-govid1", inviter, acc)
	...
	// ── КОНТРОЛЬ ФИКСТУРЫ ────────────────────────────────────────────────────
	// Посев ДЕЙСТВИТЕЛЬНО живой: пригласивший держит на этой строке глагол правки,
	// который материализует выдача. Без этого утверждения каждое отрицание ниже
	// было бы истинно и на пустой базе.
	require.True(t, w.allowed(t, "user:"+inviter, "v_update", person),
		"КОНТРОЛЬ: пригласивший обязан держать глагол `v_update` на строке своего члена — "+
