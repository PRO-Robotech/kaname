...
const (
	...
	// выдаётся за находку (§«Чтение вердикта»).
	//
	// ─────────────────────────────────────────────────────────────────────────
...
//
// Находка БЕЗ модуля не печатает пустого имени. Прежде строка выходила как
// «модуль : канон не резолвится» — отказ резолва приписывался модулю, которого
// не назвали, и читатель искал виновника там, где его нет (kacho#2241). Находки
// без модуля законны: отказ резолва канона и отказ обхода дерева относятся ко
// ВСЕЙ сверке, а не к чьему-то модулю.
...
//
// Второй операнд сверки резолвится ОТ КОРНЯ ДЕРЕВА по постоянному относительному
// пути (authzplan) и за пределы этого корня НЕ ВЫХОДИТ. Подменить канон снимком,
// лежащим рядом со сверщиком, невозможно не по запрету, а потому что ПРЕДЪЯВИТЬ
// ЕГО НЕЧЕМ: у Sweep нет параметра, куда такой путь встал бы. Держит это
// компилятор, а не гейт.
//
// ЗДЕСЬ СТОЯЛО «резолвится обходом вверх», и это описывало подъём ДО КОРНЯ
// ФАЙЛОВОЙ СИСТЕМЫ, снятый задачей #2159. Подъём означал, что канон мог быть
// найден ВЫШЕ дерева root: перепись печаталась по root, а величины снимались с
// постороннего файла, и отличить такой зелёный от настоящего было нечем.
//
...
func Sweep(resources []catalog.ResourceRow, root string, waivers []Waiver) (Census, []Finding, int) {
	...
	// missingManifest — сколько находок именно об ОТСУТСТВИИ манифеста модуля.
	// Величина нужна, чтобы отличить «поставки соседей тут нет» от «сверка нашла
	// расхождение»: во втором случае послаблению не место.
	missingManifest := 0
