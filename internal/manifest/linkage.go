...
// требование покраснело бы на настоящем манифесте vpc: группу его вступления
// (`module-relation-writers`) заводит посев не модуля, а манифеста СЛУЖБЫ
// (`manifest.yaml`, раздел `seed`; приёмка MRW-1). Проверено прогоном валидатора
// черновика на день записи: «ВЕРДИКТ: целостно» при `вступлений 1` — тогда
// единственной группой вступления была `module-quota-readers`, снятая позже
// (kaname#106).
//
...
// (§B18) — поимённой выдаче человека при установке модуля
// предмета нет.
