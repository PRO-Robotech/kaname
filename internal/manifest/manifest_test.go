...
//
// Вторая половина сценария («тот же документ, проверенный опубликованной
// схемой, даёт ноль замечаний») здесь НЕ утверждается, и причина не в сроках:
// схема в дереве есть (services/iam/schema/), а валидатора JSON Schema для Go
// нет НАМЕРЕННО — §2.1 приёмки запрещает его заводить, потому что два судьи
// одной формы расходятся молча. Согласие схемы и структур держит вместо него
// MOD-MF-21: он сличает МНОЖЕСТВА ключей, не разбирая документ схемой.
//
// Прежняя редакция объясняла это тем, что «схемы в дереве ещё нет». Читатель,
// действующий по такому объяснению, заключил бы, что предмет появился и клаузу
// пора утверждать, — и потянулся бы ровно за той библиотекой, против которой
// написан MOD-MF-21.
