...
var (
	...
	// молчит навсегда (§«Гейт на класс», п. 9), поэтому он снят, а
	// не переименован.
)
...
//
// # Раздел ЗАВОДИТ строки, и заводит их ПРИМЕНИТЕЛЬ на старте (#2452)
//
// Здесь стояло «раздел НИЧЕГО НЕ ЗАВОДИТ: строки заводит применённая миграция»
// — верно на день записи (#1948) и неверно теперь. Применитель заведён:
// `apps/kaname/moduleseed`, позван композиционным корнем после применителя
// ролей. Он читает все четыре подраздела и приводит базу к объявленному.
//
// Раздел встал в один ряд с `resources` и `roles`: у всех трёх применитель
// ЕСТЬ, и читатель, достраивающий его по аналогии, теперь прав.
//
// # Почему применитель понадобился — это не симметрия ради симметрии
//
// Пока строки заводила миграция, их заводила ВСЯКАЯ установка: миграция
// применяется везде, включая ту, где платформы нет вовсе. Самостоятельная
// служба доступа получала пять служебных ЛИЧНОСТЕЙ чужого продукта с выданными
// правами. Условием заведения строк МОДУЛЕЙ стала ДОСТАВКА манифеста — её
// кладёт зонтичный чарт платформы и не кладёт чарт самостоятельной службы.
//
// Разряд у манифестов не один (приёмка MRW-1, Р3): СВОЙ манифест службы
// доставкой не связан — он встроен в образ (`internal/servicemanifest`) и
// применяется на любой установке первым, потому что группу, в которую модули
// вступают, объявляет он. Раздел `seed` у него есть, а `serviceAccounts` и
// `joins` — нет: своей служебной записи у службы не бывает.
//
// # Что применитель НЕ делает, и это сказано, чтобы не достроили
//
// Он не СНИМАЕТ строк. Модуль, ушедший из доставки, свою личность и свои права
// сохраняет: снятие есть отзыв, а отзыв без решения хуже отсутствия отзыва.
// Это остаток, названный вслух.
//
// # Цена прежнего устройства СНЯТА
//
// Новая служебная запись, группа, выдача или вступление модуля больше не
// требуют пересборки образа службы доступа: манифест доезжает доставкой, а
// применитель — обычным перезапуском.
//
// # Сходство объявленного с живой базой по-прежнему сверяется
//
// Гейт `internal/moduleseedparity` читает живую базу и сверяет её с этим
// объявлением; после #2452 он зовёт применитель сам, поэтому судит уже не
// согласие двух копий, а то, доезжает ли объявленное до строк.
type Seed struct {
...
func LoadWithReferent(data []byte, referent TypeReferent, opts ...LoadOption) (*Manifest, error) {
	...
	// (§«Снимая ПРЕДМЕТ, разбери проверки по ЗНАКУ утверждения»).
	//
