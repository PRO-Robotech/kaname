...
// TreeFileName — то же имя для читателей ВНУТРИ службы.
//
// Экспортируется ради одного свойства: имя приезжает в службу ОДНИМ импортом, а
// не заводится вторым у каждого читателя рядом.
//
// Здесь стояло другое основание — что пакет отнесён к классу платформы, а
// направление «служба → платформа» запрещено, поэтому импортировать его прямо
// нельзя. Основание отозвано вместе с классом: пакет объявляет договорённость
// о формате, которой платформа ПИШЕТ, а служба ЧИТАЕТ, и потому числится
// фундаментом (`corelib`), а не платформой. Прямой импорт стал законным, и
// ведомость границы фундамента больше не называет этого ребра ни одной
// записью.
//
// Функция при этом остаётся: одно место импорта дешевле трёх, а второй копии
// значения она не создаёт by construction.
//
// Второй копии значения это не создаёт: возвращается та же постоянная.
func TreeFileName() string { return manifestFileName }
