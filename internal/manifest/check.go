...
//
// Экспортируется ради одного свойства: читатель службы не вправе импортировать
// `pkg/modulemanifest` сам. Тот пакет отнесён к классу платформы, а направление
// «служба → платформа» запрещено целевой раскладкой — сегодня такое ребро
// собирается чисто лишь потому, что модуль в дереве один, и отказ пришёл бы
// только в день раскола. Здесь ребро УЖЕ объявлено ведомостью границы
// фундамента, поэтому имя раздаётся отсюда, а не заводится вторым импортом.
//
