...
//
// # ПЕРЕЕХАЛ ВМЕСТЕ С ПРЕДМЕТОМ (kaname#360)
//
// Живость и готовность жили на слушателе вебхуков поставщика личности, а тот
// под посадкой `own` не поднимается: поставщика нет. Поэтому они переехали на
// диагностическую поверхность, которая есть при любой посадке, и утверждения
// переехали вместе с ними ДОСЛОВНО — сменилась только проводка
// (`readiness_carrier_wiring_test.go`).
//
