...
//
// FindByExternalID возвращает строку личности (Kratos sub) КАК ЕСТЬ, в любом
// состоянии. Набором — потому что вопрос здесь «в каком состоянии субъект», и он
// задаётся строкам как есть, а не отфильтрованному подмножеству. Длина при
// непустом субъекте — НЕ БОЛЕЕ ОДНОЙ: ключ `users_identity_external_id_uniq`
// объявлен по `external_id` глобально, с условием на непустоту, а пустой субъект оба хука
// отвергают 400 до чтения.
//
// Здесь стояло «ВСЕ row через все Account-ы … для multi-Account membership
// берётся первый row … явный выбор аккаунта через `account_id` hint — будущее
// расширение». Всё три утверждения пережили свой предмет: строка человека одна,
// принадлежность выражают ЧЛЕНСТВА, а у «будущего расширения» не было
// производителя — обещание без владельца (ban #11).
//
...
// point of enforcement whatsoever. One port, one reader instance (wrapped once
// with its per-call limit by the composition root), one row — the two hooks
