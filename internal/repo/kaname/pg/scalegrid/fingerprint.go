...
var (
	...
	// triggerObjectDDL — объявление ТРИГГЕРА целиком, вместе с хвостом `ON <таблица>`.
	//
	// Хвост входит в снимаемое НАМЕРЕННО: без него имя измеряемой таблицы
	// осталось бы в операторе, а глагол ушёл — и оператор судился бы как
	// «упоминание без глагола», то есть верно, но по случайности. Снимая пару
	// целиком, мы получаем тот же исход ПО ПОСТРОЕНИЮ.
	triggerObjectDDL = regexp.MustCompile(
		`(?is)\b(?:CREATE|DROP)\s+(?:CONSTRAINT\s+)?TRIGGER\b.*?\bON\s+[A-Za-z_][\w.]*`)
)

// # Триггер структуры измеряемой таблицы не меняет (#69)
//
// Та же форма, что у временного объекта, и найдена тем же способом — новой
// популяцией: миграция, ВЕШАЮЩАЯ ТРИГГЕРЫ на измеряемые таблицы, несёт в одном
// операторе глагол `CREATE`/`DROP` и имя измеряемой таблицы, но DDL идёт над
// ТРИГГЕРОМ, а таблица названа лишь как его хозяин.
//
// Плана ЧТЕНИЯ это не меняет ни при каком содержимом триггера: триггер
// исполняется на записи, а предмет этих отчётов — стоимость запроса ВЕРДИКТА,
// то есть чтения (предикат отпечатка так и говорит: «таблицу, которую ЧИТАЕТ
// запрос вердикта»). Отчёт от такой миграции ложным не становится.
//
// # Почему это НЕ то суждение, которое граница выше отвергает
//
// Граница отвергает суждение о том, меняет ли КОНКРЕТНАЯ правка таблицы план
// («комментарий к колонке, снятие умолчания») — там решать пришлось бы по
// смыслу правки. Здесь решать нечего: объект DDL — другой, и это видно из
// формы оператора, а не из его смысла. Ровно тем же доводом снята конструкция
// над временным объектом.
//
// Обратная сторона сохранена и проверяется законным близнецом: `ALTER TABLE`
// над измеряемой таблицей в том же файле под отпечаток попадает по-прежнему —
// снимается ОБЪЯВЛЕНИЕ триггера, а не весь оператор и не весь файл.

// stripTempObjectDDL — оператор без конструкций DDL над временными объектами и
// над триггерами.
//
...
func stripTempObjectDDL(stmt string) string {
	out := tempOnCommit.ReplaceAllString(tempObjectCreate.ReplaceAllString(stmt, " "), " ")
	return triggerObjectDDL.ReplaceAllString(out, " ")
}
