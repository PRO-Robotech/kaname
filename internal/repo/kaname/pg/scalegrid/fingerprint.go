...
// subjectClause — ЧТО именно считается субъектом оператора. Одна константа на
// ОБА предиката: приборов два, а правило субъекта у них одно. Второй экземпляр
// этой фразы разошёлся бы с первым молча — и разошёлся бы там, где расхождение
// не видно: шапки двух отчётов утверждали бы разное об одном правиле.
const subjectClause = `оператор РАЗБИРАЕТСЯ (глагол, вид объекта, имена в грамматически ` +
	`определённых позициях), а не ищется подстрокой; границы операторов тоже разбираются — ` +
	`точка с запятой внутри литерала (обычного и с экранированием E'…\'…'), внутри имени в ` +
	`кавычках и внутри тела в долларовых кавычках оператора не кончает; комментарий не ` +
	`исполняется и субъектом не бывает; DDL над временным объектом, над функцией и над ` +
	`последовательностью субъектом ТАБЛИЦЫ не является by construction, а безвредность вида ` +
	`объекта НЕ ОБЪЯВЛЯЕТСЯ, а доказывается парой операторов с положительным контролем; ` +
	`СВЕРХ ОПРЕДЕЛЕНИЯ судятся формы, меняющие план не будучи им, — расширенная статистика, ` +
	`политика построчной безопасности, наследование, ANALYZE/REINDEX/CLUSTER/VACUUM; ` +
	`исполняемый SQL (EXECUTE) разбирается ТЕМ ЖЕ разбором, второго перечня глаголов нет — ` +
	`выписанный оператор судится ТОЧНО по своему субъекту, а подстановка format(%I) делает имя ` +
	`подстановочным и совпадает с любой измеряемой таблицей В ТОМ ПОЛЕ, где стоит; индекс и ` +
	`расширенная статистика, названные по имени, связываются с таблицей по словарю, ВЫВЕДЕННОМУ ` +
	`обходом того же каталога, а неизвестное имя и незнакомый вид объекта судятся ОСТОРОЖНО — ` +
	`«задевает», потому что молчание на них было бы слепотой; ОТБОР ЧИТАЕТ ФАЙЛ ЦЕЛИКОМ, ` +
	`включая раздел ОТКАТА, который в работе не исполняется: миграция попадает под отпечаток и ` +
	`за оператор, стоящий только там`
...
const FingerprintPredicate = `все не-тестовые .go каталогов ` + verdictDir + ` и ` + gridDir +
	`; те .sql каталога ` + migrateDir + `, у которых хотя бы один оператор определения имеет ` +
	`СУБЪЕКТОМ таблицу, которую читает запрос вердикта, — то есть таблица стоит на месте ` +
	`того, что оператор ИЗМЕНЯЕТ, а не того, на что он ссылается: ` + subjectClause +
	`; имена таблиц ВЫВЕДЕНЫ из кода вердикта по приставке "` +
...
func ComputeFingerprint(root string) (Fingerprint, error) {
	...
	migrations, err := migrationsNaming(root, fp.Tables, scopeReadPlan)
	if err != nil {
		return fp, err
	}
...
var fingerprintScaffolding = map[string]bool{
	...
	gridDir + "/ddlsubject.go":  true, // разбирает оператор определения для отпечатка
...
// migrationsNaming — миграции, задевающие измеряемые таблицы.
//
// # Почему каталог читается ДВАЖДЫ
//
// Первым проходом собираются сведения, которые из ОДНОГО файла не выводятся:
// словарь «индекс → его таблица» и множество функций, исполняющих динамический
// DDL. Без первого оператор
// `DROP INDEX kaname.memberships_user_cursor_idx` связать с таблицей нечем:
// имя индекса её не содержит by construction, и прежний предикат такой оператор
// не брал ВОВСЕ — слепая зона тем более опасная, что снятие индекса план чтения
// меняет наверняка. Словарь ВЫВОДИТСЯ обходом, а не выписывается: выписанный не
// двигался бы от нового индекса и продолжал бы знать снятые.
//
// Индекс, которого в словаре нет, судится осторожно — «задевает». Молчание на
// неизвестном имени было бы слепотой, а лишний файл под отпечатком стоит одного
// прогона, тогда как пропущенный — ложного числа.
func migrationsNaming(root string, tables []string, scope ddlScope) ([]string, error) {
	...
	corpus := buildCorpusIndex(bodies)

	var out []string
	for _, name := range names {
		if migrationTouches(bodies[name], tables, scope, corpus) {
...
// migrationTouchesStructure и разбор оператора определения живут в
// `ddlsubject.go`: предмет там один — СУБЪЕКТ оператора, и у него своя
// грамматика, свои лексемы и своя инъекция. Здесь остался только обход каталога.
//
// # Что снято вместе с прежним предикатом
//
// Снят отбор «глагол определения и имя таблицы ГДЕ УГОДНО в одном куске текста»
// и обе накладки к нему — снятие конструкции над временным объектом (#1833) и
// снятие объявления триггера (#69). Обе накладки существовали ровно затем,
// чтобы починить исход в тех формах, где имя таблицы стояло НЕ на месте
// субъекта. Распознаватель субъекта даёт тот же исход BY CONSTRUCTION: у
// `CREATE TEMP TABLE _x AS SELECT … FROM <измеряемая>` субъект — `_x`, у
// `CREATE TRIGGER … ON <измеряемая>` субъект — триггер. Пробы обеих находок
// сохранены и стали замком регрессии.
...
const WriteDeleteFingerprintPredicate = `все не-тестовые .go каталога ` + reconcileDir +
	`; те .sql каталога ` + migrateDir + `, у которых хотя бы один оператор определения имеет ` +
	`СУБЪЕКТОМ таблицу, которую пишет материализатор: ` + subjectClause +
	`; СВЕРХ ТОГО сюда идут триггер на такой таблице и ВХОДЯЩИЙ внешний ключ на неё — оба ` +
	`исполняются на ЗАПИСИ, то есть ровно на том пути, стоимость которого этот отчёт и меряет ` +
	`(у отпечатка ЧТЕНИЯ этих двух форм нет: план чтения они не меняют); имена таблиц ` +
	`ВЫВЕДЕНЫ из его кода по приставке "` + fingerprintTableMark + `", а не выписаны` +
	significantClause
...
func ComputeWriteDeleteFingerprint(root string) (Fingerprint, error) {
	...
	migrations, err := migrationsNaming(root, fp.Tables, scopeWriteCost)
	if err != nil {
		return fp, err
	}
