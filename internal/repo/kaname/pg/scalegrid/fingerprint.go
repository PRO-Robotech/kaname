...
const subjectClause = `оператор РАЗБИРАЕТСЯ (глагол, вид объекта, имена в грамматически ` +
	...
	`точка с запятой внутри литерала, имени в кавычках и тела в долларовых кавычках оператора ` +
	`не кончает; комментарий не исполняется и субъектом не бывает; DDL над временным объектом, ` +
	`над триггером, над функцией и над последовательностью субъектом ТАБЛИЦЫ не является ` +
	`by construction; индекс, названный по имени (DROP/ALTER INDEX), связывается с таблицей по ` +
	`словарю, ВЫВЕДЕННОМУ обходом того же каталога миграций, а неизвестное имя индекса, ` +
	`незнакомый вид объекта и DDL, чей субъект подставляется во время выполнения (EXECUTE ` +
	`format(...)), судятся ОСТОРОЖНО — «задевает», потому что молчание на них было бы слепотой`
