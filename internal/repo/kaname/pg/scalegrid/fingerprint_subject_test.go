...
func TestMigrationTouchesStructure_ProvenByInjection(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "постоянная таблица ИЗ ВЫБОРКИ измеряемой — не влияет",
			// РЕДАКЦИЯ ЭТОГО СЛУЧАЯ ИЗМЕНЕНА ВМЕСТЕ С ПРЕДИКАТОМ.
			//
			// Прежде здесь ожидалось «влияет», и довод был: «такая таблица живёт
			// в схеме и меняет её структуру». Довод верен для СХЕМЫ и неверен
			// для предмета отчёта: предмет — план чтения ИЗМЕРЯЕМОЙ таблицы, а
			// `access_bindings_snapshot` — другая таблица. Измеряемая стоит
			// здесь в разделе выборки, то есть только ЧИТАЕТСЯ; ни колонки, ни
			// индекса, ни статистики у неё не прибавилось.
			//
			// Прежний близнец менял против отрицательного случая ДВА факта
			// разом — и слово `TEMP`, и то, по какую сторону оператора стоит
			// измеряемое имя. Ниже он разведён на два одно-фактных.
			sql:  "CREATE TABLE kaname.access_bindings_snapshot AS SELECT * FROM kaname.access_bindings;",
			want: false,
		},
		{
			name: "ЗАКОННЫЙ БЛИЗНЕЦ: та же форма, субъект — ИЗМЕРЯЕМАЯ — влияет",
			// Отличие от случая выше РОВНО ОДНО: имя измеряемой таблицы стоит
			// на месте субъекта определения, а не в разделе выборки.
			sql:  "CREATE TABLE kaname.access_bindings AS SELECT * FROM kaname.legacy_bindings;",
			want: true,
		},
		{
