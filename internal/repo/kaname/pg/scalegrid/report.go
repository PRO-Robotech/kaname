...
// рядом: артефактов у прибора два, а копия разрешителя разошлась бы с
// оригиналом молча.
//
// ПЕРЕЕХАЛ СЮДА ИЗ `strength.go`, и это не вкус, а снятие заложника. Файл
// `strength.go` лежит ПОД ОТПЕЧАТКОМ предмета замера, а `report.go` объявлен
// оснасткой (`fingerprintScaffolding`), потому что не касается базы. Пока
// разрешитель артефакта жил в предмете, предмет нёс адрес пакета оснастки
// (`gitenv`) — и всякий переезд этого пакета между модулями объявлял четыре
// отчёта несвежими, требуя до двух часов пересъёмки на поднятой базе за правку,
// не тронувшую ни одного оператора. Ровно тот класс, против которого заведено
// само исключение оснастки.
//
