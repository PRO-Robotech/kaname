...
//
// ## Выгода распределена НЕРАВНОМЕРНО, и это покрывает ВЕСЬ файл, а не часть
//
// Замер по каталогу миграций (32 файла, 1201 оператор). «Экземпляров» — сколько
// операторов этой формы в каталоге ЕСТЬ; «своих файлов» — сколько файлов берёт
// ТОЛЬКО этот механизм и никакой другой. Ноль — законное число, когда назван.
//
//	механизм                                  экземпляров   своих файлов
//	──────────────────────────────────────────────────────────────────────
//	разрез операторов + разбор головы + ссылки   133            6
//	исполняемый SQL одним разбором                 2            1
//	словарь именованных объектов (218 записей)     1            0
//	ветка схемы (CREATE/DROP SCHEMA)               2            0
//	обслуживание (ANALYZE/REINDEX/CLUSTER/VACUUM)  1            0
//	расширенная статистика                         0            0
//	политика построчной безопасности               0            0
//	наследование (INHERITS / [NO] INHERIT)         0            0
//	внешняя таблица                                0            0
//	строка с экранированием E'…\'…'                 0            0
//
// Ответ «семь миграций вместо девяти» покупает ПЕРВАЯ строка. Вторая покупает
// ровно один файл, и он настоящий: прежний предикат его не видел. Всё остальное
// сегодня не купило НИ ОДНОГО файла.
//
// Больше того: словарь именованных объектов и ветка схемы срабатывают только в
// разделе ОТКАТА, который в работе не исполняется вовсе (перемерено: `DROP
// INDEX` и `DROP SCHEMA` в разделах применения этих миграций отсутствуют), а
// единственный `ANALYZE` каталога назван над НЕИЗМЕРЯЕМЫМИ таблицами.
//
// Файл вырос с ~120 строк прежнего предиката до 1436, и дорогая часть заняла
// около половины. Оправданы эти строки БУДУЩИМИ формами, а не сегодняшним
// предметом: называть их «закрытыми слепыми зонами» — приписывать себе чужую
// заслугу, слепая зона у них есть, предмета у неё сегодня нет. Оставлены они
// по одной причине: цена ошибки несимметрична — пропущенное изменение даёт
// ложное число и молчит, лишний файл даёт лишний прогон и виден. Две из форм
// заведены не по догадке, а по ВОСПРОИЗВЕДЁННОМУ молчанию: расширенная
// статистика и политика были найдены инъекцией настоящей миграции в каталог.
//
// ## Числа этой таблицы перемерены ПРЕДИКАТОМ ПРИБОРА, а не похожим
//
// Первый замер дал «39 операторов исполняемого DDL» и «287 строк с
// экранированием». Оба числа были получены предикатом ШИРЕ прибора: первое
// считало литералы, похожие на DDL, вместо тех, что прибор судит (определения
// функций он пропускает), второе искало подстроку `E'` вместо приставки как
// ОТДЕЛЬНОГО слова и ловило хвост всякого имени на `e`. Настоящие числа — 2 и
// 0. Это тот же класс, за которым эта полоса и ходит, пойманный в собственном
// замере: число, верное для другого предиката.
