...
func saKeySweepTargets(t *testing.T) []kanamepg.SecretSweepTarget {
	t.Helper()
	a, err := anypb.New(&iamv1.IssueSAKeyResponse{})
	require.NoError(t, err)
	return []kanamepg.SecretSweepTarget{{
...
func TestSweepStrandedSecrets_ClearsWhatTheDetachedCleanupNeverGotTo(t *testing.T) {
	...
	sw := kanamepg.NewOpsResponseRedactor(pool, "kaname")
	spec := kanamepg.SecretSweepSpec{
...
func TestSweepStrandedSecrets_LeavesTheGraceWindowAlone(t *testing.T) {
	...
	sw := kanamepg.NewOpsResponseRedactor(pool, "kaname")
	res, err := sw.SweepStrandedSecrets(ctx, kanamepg.SecretSweepSpec{
