...
//     фикстуры.
//  2. ИСПОЛНЯЕТСЯ ОБЕИМИ ВОЛНАМИ. Пробы пакета контейнера не поднимают и под
//     кратким режимом себя не пропускают, поэтому идут и в быстром прогоне.
//     Здесь стояло «уже в отборе: путь содержит /internal/repo/» — отбор
//     контейнерного задания больше не выписан образцом пути, он ВЫВОДИТСЯ
//     признаком «сборка проб импортирует pgtest либо testcontainers»
//     (kaname#19), и этот пакет под него не подпадает by construction.
