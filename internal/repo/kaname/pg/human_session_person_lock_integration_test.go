...
// (задача kaname#340).
//
// Дверь `EndOtherSessions` — единственный оператор, снимающий несколько
// записей сессии одного человека (`endSessionsOfSQL`). Её зовут четыре
// писателя, и двое из них после неё пишут в свою запись, то есть берут строки
// сессии «прочие → своя»; принудительный выход берёт все одним оператором в
// порядке просмотра. Общий полный порядок у них даёт только сериализация на
// строке личности, взятой ДО первой строки сессии, — и берёт её сама дверь,
// чтобы порядок не зависел от того, помнит ли о нём вызывающий.
//
...
//     выдачи (`FOR KEY SHARE … NOWAIT` проходит): вход человека эта
//     сериализация не останавливает;
