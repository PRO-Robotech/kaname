...
func TestReconcileForward_BurstOnOneOwnerBindingDoesNotSerialize(t *testing.T) {
	...
	// заняла бы НЕ МЕНЬШЕ последовательной; ровно это и утверждается.
	//
	// ДОЛИ ВЫИГРЫША В ПОРОГЕ БОЛЬШЕ НЕТ, И ЭТО ПОЧИНКА, А НЕ ПОСЛАБЛЕНИЕ. Здесь
	// стояло `(seq.wall*3)/4` — «хотя бы четверть выигрыша» — с доводом «на
	// порядок мягче наблюдаемого и потому не краснеет на занятой машине». Довод
	// был снят С ДРУГОЙ МАШИНЫ: на 32-ядерной выигрыш действительно кратный, на
	// ранере конвейера его нет и близко.
	//
	// Замер, опровергнувший довод (ствол 052ac97a, прогон 34284970002, точка N):
	// волна 11.986083ms против 15.760453ms последовательных — выигрыш 1.31x при
	// требуемых порогом 1.33x. Промах 1.4 %, то есть запаса не было ВОВСЕ, а не
	// «на порядок». На той же посадке точка 2N дала 1.62x и прошла.
	//
	// И это же измерение говорит, что предмет пробы ЦЕЛ: очередь на общем ключе
	// дала бы около 1.00x на ОБЕИХ точках (так утверждает печать выше), а
	// наблюдались 1.31x и 1.62x — параллель работает, сериализации нет. Красным
	// был порог, а не продукт.
	//
	// ПОЧЕМУ ИМЕННО НОЛЬ ДОЛИ, а не доля поменьше. Всякая доля выигрыша есть
	// утверждение «параллельность машины не ниже такой-то» — ровно та величина,
	// которую шапка этого файла запрещает возвращать в порог и из-за которой
	// отсюда уже удаляли предыдущий порог («вчетверо от одиночного прохода»).
	// Доля 3/4 требовала параллельности 1.33 и молча вернула запрет; любая
	// следующая доля вернёт его снова, просто с другим числом. Строгое «быстрее»
	// — единственная форма, которая не спрашивает у машины, сколько у неё ядер.
	//
	// ЧТО ЭТИМ ТЕРЯЕТСЯ, названо честно: ЧАСТИЧНАЯ сериализация ловится хуже —
	// но шапка файла и раньше обещала ловить сравнением только ПОЛНУЮ, а долю
	// называла неутверждаемой. Порог 3/4 давал не чувствительность, а красноту
	// на занятой машине. Пообъектный счётчик операторов блокировки выше остаётся
	// первым и ДЕТЕРМИНИРОВАННЫМ ответом; это сравнение — второй, для блокировки,
	// невидимой счётчику.
	require.Less(t, waveN.wall, seqN.wall,
	...
	// на «его нет» и «он есть», не спрашивая, сколько у машины ядер. Доля из
	// порога снята по той же причине, что и у точки N выше.
	require.Less(t, wave2N.wall, seq2N.wall,
