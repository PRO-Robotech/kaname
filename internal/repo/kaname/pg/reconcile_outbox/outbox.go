...
const (
	...
	//
	// РАДИУС ОТСЕЧКИ — ОДНО СОБЫТИЕ, А НЕ ОЧЕРЕДЬ. Событие здесь не несёт
	// дельты, поэтому порядок между объектами не значим и отсечённая строка не
	// запирает соседей; клейма по голове партиции (как у очереди кортежей) не
	// требуется by construction. Само обоснование коммутативности живёт ОДНИМ
	// экземпляром в росписи commutativeDrainExempt — здесь оно только
	// применяется к вопросу об отсечке.
	//
...
//
// КЛЮЧА ПАРТИЦИИ У ЭТОГО КЛЕЙМА НЕТ, и решение об этом принято НЕ ЗДЕСЬ.
// Выборка идёт по одному лишь `id`, поэтому при нескольких исполнителях
// `FOR UPDATE SKIP LOCKED` отдаёт потомка, пока предшественник заперт чужим
// клеймом, — и порядок применения строк одного объекта не удержан ничем. Для
// потока, несущего постановку и снятие одной единицы состояния, это означало бы
// воскресшее состояние; здесь не означает, и почему — сказано ОДНИМ экземпляром
// в росписи commutativeDrainExempt, которую держит гейт порядка очередей
// (`internal/check/drain_order_declared_test.go`). Он же покраснеет, если у
// записи не станет предмета.
//
// Пересказывать обоснование тут нельзя: второй его экземпляр разойдётся с
// первым молча — и разойдётся именно в том месте, где следующий решает,
// держится ли ещё освобождение от ключа.
