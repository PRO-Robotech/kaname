...
// подразумевается.
//
// Три места: СЛОВАРЬ ВИДОВ (доменная константа), адаптер (литерал, которым он
...
//
// Доменное объявление ПЕРЕЕХАЛО, а не исчезло: прежде оно жило в закрытом
// каталоге авторитета величин, каталог ушёл вместе с авторитетом
// (`PRO-Robotech/kacho#2117`, стадия S4), и носителя объявляет теперь словарь
// посадки. Предмет пробы от переезда не изменился ни на знак — три места
// по-прежнему обязаны называть одно.
//
...
func TestIdentityCarrierIsNamedTheSameEverywhere(t *testing.T) {
	...
	require.Equal(t, domain.CarrierIdentity, kanamepg.CarrierIdentity,
		"словарь видов и адаптер называют носителя по-разному: списание и чтение "+
...
// СЛОВАРЕ, а не о константе.
//
// Отрицание рядом с положительным: у соседнего вида того же домена носитель
// ДРУГОЙ. Без него проба зеленела бы и на словаре, где носитель у всех один.
func TestAccountKindIsCarriedByTheIdentity(t *testing.T) {
	t.Parallel()

	require.Truef(t, domain.IsIdentityCarriedKind("iam.account"),
		"вида `iam.account` словарь не носит личностью: потолка над аккаунтом не "+
			"существует, и завести их можно сколько угодно")
	require.Truef(t, domain.IsPostureStatedKind("iam.account"),
		"носитель объявлен, а источника величины нет: арендатор читал бы потолок "+
			"из места, которого не существует")
	...
	const neighbour domain.LimitKind = "iam.user.credential"
	require.True(t, domain.IsPostureStatedKind(neighbour))
	require.Falsef(t, domain.IsIdentityCarriedKind(neighbour),
	...
	require.Equalf(t, domain.LimitKind("iam.user"), neighbour.ParentKind(),
		"носитель соседа не выводится из его имени: тогда «носитель другой» было бы "+
