...
// ВЫХОДА ЖДЁТ ЗАМКОВ НЕ ДОЛЬШЕ СВОЕГО ПРЕДЕЛА (задача kaname#340).
//
// Предел — `lock_timeout` самой транзакции. Утверждается наблюдаемое: снятие
// сессии, строку которой держит чужая транзакция, отказывает недоступностью
// ВСКОРЕ после предела, а не по сроку вызова и не по потолку оператора пула.
