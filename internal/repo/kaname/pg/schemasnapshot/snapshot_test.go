...
// ровно один — этот:
//
//  1. `internal/migrations` (где он лежал) шёл целью, где действует
//     «ПРОПУСК = ОТКАЗ»: она считает строки `--- SKIP` и краснеет на любой.
//     Пропуск по незапрошенному снимку неотличим там от пакета, не
//     исполнившегося вовсе, — ради этой неразличимости цель и заводилась.
//     Инструмент ронял её КАЖДЫМ прогоном. (Перечня `PG_OUTSIDE_SELECTION_PKGS`
//     в этом дереве нет: он остался в монорепо вместе с целью.)
//
...
//
//  3. Пакет обязан попадать ХОТЬ В КАКОЙ-НИБУДЬ прогон: пакет, который не
//     исполняет ни одно задание, красным быть не может — его молчание
//     неотличимо от исправной работы.
//
// Отбор контейнерного задания ВЫВОДИТСЯ признаком «сборка проб импортирует
// pgtest либо testcontainers», а не выписан образцом пути (kaname#19). Этот
// пакет под признак подпадает, поэтому исполняется; пропуски задание терпит
// (`.github/scripts/classify-integration-outcome.sh` их не считает), и долгом
// пакет не числится — ведомость прощений тут была бы послаблением без предмета.
//
// Здесь стояли гейт `shortgatedselection_test.go` и образец
// `/internal/(repo|clients|reconciler|subscriptionjournal)` внутри `services/`.
// Гейт остался в монорепо, каталога `services/` в этом дереве нет — координаты
// пережили вынос службы.
