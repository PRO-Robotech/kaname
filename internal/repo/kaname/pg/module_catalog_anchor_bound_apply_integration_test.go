...
func TestApplyBeyondTheParityAnchorIsRefused(t *testing.T) {
	...
	after, perr := seed.AssertCatalogParity(ctx, kanamepg.NewCatalogRepo(pool), seed.ImageAnchor())
...
func TestApplyFixingDriftPassesAndLeavesParityWhole(t *testing.T) {
	...
	drifted, derr := seed.AssertCatalogParity(ctx, kanamepg.NewCatalogRepo(pool), seed.ImageAnchor())
	...
	fixed, ferr := seed.AssertCatalogParity(ctx, kanamepg.NewCatalogRepo(pool), seed.ImageAnchor())
...
func verbApplierOver(t *testing.T, pool *pgxpool.Pool) *modulecatalog.VerbApplier {
	t.Helper()
	return modulecatalog.NewVerbApplier(kanamepg.NewCatalogWriteRepo(pool))
}
