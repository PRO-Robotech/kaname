...
func applierOver(t *testing.T, pool *pgxpool.Pool) *modulecatalog.Applier {
	t.Helper()
	return modulecatalog.NewApplier(kanamepg.NewCatalogWriteRepo(pool))
}
...
func TestModuleCatalogApplierWaitsForTheCatalogLock(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = holder.Exec(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, kanamepg.CatalogLockKey)
	require.NoError(t, err)
