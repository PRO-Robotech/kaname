...
//
//	строка 1 — СНИМОК:          обновление (чтение ОБЕИХ половин одним снимком)
//	                            плюс обращение к уже прочитанному факту
//	строка 2 — ПО ТРЕБОВАНИЮ:   чтение ЖИВОЙ половины НА КАЖДОМ обращении
//	                            горячего пути — цена ОТВЕРГНУТОГО устройства
//
// # СТРОКИ МЕРЯЮТ РАЗНОЕ, и с kacho#1814 это перестало быть тождеством
//
// Прежде обновление читало ровно ту же живую половину, что и чтение по
// требованию, и «обновление не дороже чтения» было сравнением ОДНОГО предмета с
// собой. Теперь обновление берёт ОБЕ половины: снятый ресурс обязан приходить
// арендатору тем же ответом, что живой, иначе преемник существует данными и не
// доезжает ни одним путём чтения.
//
// Отсюда третья величина — цена чтения ОБЕИХ половин, — и утверждение о цене
// круга обновления сверяется с НЕЙ. Сверять его с ценой живой половины значило
// бы требовать, чтобы шесть операторов стоили как три: предикат, отвергающий
// принятое решение, а не защищающий свойство.
//
// Горячий путь снятых строк НЕ спрашивает (доменный гейт, проекция, ключ), и
// строка 2 остаётся ценой живой половины: она измеряет отвергнутое устройство —
// «спрашивать базу на каждом обращении», — а ему снятое не нужно.
//
...
func TestIAMCT2_SnapshotCostAgainstOnDemand(t *testing.T) {
	...
	// Цена чтения ОБЕИХ половин одним снимком — то, чем круг обновления и
	// является. Прогрев ей достаётся тот же: план и соединение общие.
	halvesRead := measureSnapshotCost(t, iterations, func() {
		if _, err := repo.ReadCatalogHalves(ctx); err != nil {
	...
	perPeriod := refresh.median // цена одного круга обновления за период
	...
	t.Logf("строка 2 · ПО ТРЕБОВАНИЮ: чтение ЖИВОЙ половины медиана %v (p99 %v) НА КАЖДОМ обращении",
		dbRead.median, dbRead.p99)
	t.Logf("чем ЯВЛЯЕТСЯ круг обновления: чтение ОБЕИХ половин одним снимком медиана %v (p99 %v); "+
		"живая половина одна — %v, то есть снятая стоит примерно столько же",
		halvesRead.median, halvesRead.p99, dbRead.median)
	...
	// Обновление НЕ дороже ТОГО ЧТЕНИЯ, КОТОРЫМ ОНО ЯВЛЯЕТСЯ. Операнд — цена
	// обеих половин, а не живой: с kacho#1814 круг читает обе, и сравнение с
	// живой требовало бы, чтобы шесть операторов стоили как три.
	//
	// Запас вдвое оставлен прежний и сторожит прежнее свойство: сборка фактов
	// поверх чтения стоит своего, но не кратно. Заведи кто-нибудь снимку ВТОРОЕ
	// чтение — отношение уйдёт за два, и это будет находкой, а не шумом.
	if refresh.median > 2*halvesRead.median {
		t.Errorf("круг обновления %v дороже чтения обеих половин %v более чем вдвое — "+
			"снимок перестал наполняться ТЕМ ЖЕ чтением (живая половина для справки: %v)",
			refresh.median, halvesRead.median, dbRead.median)
	}
}
