...
//
// Настоящий предмет не в величине, а в том, что ОГРАНИЧИТЕЛЯ У ОЖИДАНИЯ НЕТ:
// `lock_timeout` и `statement_timeout` в непроверочном коде дерева не
// выставляет никто (измерено — вне комментариев и заголовка дампа попаданий
// ноль), то есть ждущий ждёт СТОЛЬКО, СКОЛЬКО ЖИВЁТ ЧУЖАЯ ТРАНЗАКЦИЯ, а не
// сколько отведено.
//
// И ждущих на строке сессии теперь ЧЕТЫРЕ класса писателей — уборка
// (`SweepUnservableSessions`), свой выход (`EndSession`), принудительный выход
// (`endSessionsOfSQL`) и перепредъявление (`RotateBearer`, `Present`), — к
// которым эта выдача добавляется пятым участником. Бюджет ожидания здесь не
