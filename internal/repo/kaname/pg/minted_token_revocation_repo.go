...
//
// Строка субъекта и клиента отвергает токен, отчеканенный РАНЬШЕ
// `revoke_before`. Токен живёт не дольше `MaxTokenTTL` и принимается с допуском
// `ClockSkew`. Значит после `revoke_before + MaxTokenTTL + ClockSkew` строка не
// может изменить ни одного исхода: всякий токен, который она отвергла бы, к
// этому моменту уже отвергнут собственным сроком.
//
// Строка ключа семейства отвергает токен семейства при ЛЮБОЙ отметке выпуска
// (`tokenrevocation.FamilyKeyClaim`), и тот же довод держит её только для
// выпусков не позже `revoke_before`. Выпуск семейства позже отметки ложится в
// срок операции церемонии, прочитавшей семейство живым до неё, и его покрывает
// запас (`RemovalSlack`) — довод и его граница названы у писателя строки
// (`writeFamilyCutoffsTx`).
//
...
// `apps/kaname/retention` из `pkg/tokenpolicy`.
//
