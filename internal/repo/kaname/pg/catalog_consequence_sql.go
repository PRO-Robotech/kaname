...
const resettleRuleRefSQL = `
		...
		SELECT (SELECT count(*) FROM dropped),
		       (SELECT coalesce(array_agg(DISTINCT role_id), ARRAY[]::text[]) FROM dropped)`
...
// ─────────────────────────────────────────────────────────────────────────────
// ОБЪЯВЛЕНИЕ ОТЗЫВА ПОДПИСЧИКУ (задача службы #76)
//
// # Предмет
//
// Снятие строки каталога ОТБИРАЕТ у арендаторской роли глагол либо сегмент:
// право, которым он пользовался, перестаёт действовать. Три оператора выше
// делают это, строки роли не трогая НИ ОДНИМ оператором, — а журнал подписки
// эмитит правку роли только со строки роли (решение #73). Значит событие не
// рождается, и для подписчика отзыв неотличим от «ничего не произошло»: он
// узнает о нём, только если перечитает роль по другому поводу.
//
// Класс тот же, что «контроль, действующий на выдаче, но не на предъявлении»:
// механизм отработал верно, а наблюдаемости у его результата нет.
//
// # Почему ЭМИССИЯ ИЗ ПУТИ, а не два других исхода
//
// Задача называла три варианта, и два отвергнуты ЗАМЕРОМ, а не вкусом.
//
// ТРИГГЕР на трёх подтаблицах роли отвергнут числом #73: холостой подъём кладёт
// в них 1558 строковых событий (полная замена проекции плюс досев селекторов), и
// сужение `OLD.* IS DISTINCT FROM NEW.*` отсекает из них НОЛЬ — над двумя
// таблицами оператора `UPDATE` не существует вовсе, а досев селекторов двигает
// `updated_at = now()`, поэтому предикат существа на нём истинен всегда.
// Отличить «снятие последствием каталога» от «полной замены при подъёме» на
// уровне строки нечем: обе суть `DELETE` по той же таблице. То есть триггер
// вернул бы ровно тот объём холостых событий, ради отсутствия которого #73 и
// решалось, — и вернул бы его в обмен на редкое нехолостое.
//
// ЗАПИСЬ СТРОКИ РОЛИ тем же оператором (двинуть `updated_at`, чтобы сработал
// живой триггер `roles_resource_journal_update_trg`) отвергнута по двум
// основаниям сразу. Первое: применитель каталога стал бы ПИСАТЕЛЕМ строки роли —
// ровно того, чего не допускает дисциплина единственного автора проекции.
// Второе и решающее: `updated_at` — поле КОНТРАКТА, которое читает арендатор, и
// второй его производитель говорил бы «роль правили» в момент, когда её никто не
// правил; заодно сдвинутый `xmin` отверг бы конкурирующую правку арендатора
// сторожем версии — за изменение, которого арендатор не делал.
//
// Остаётся эмиссия из самого пути, и у неё есть то, чего нет у двух других: она
// исполняется РОВНО ТОГДА, когда отобрано, потому что путь последствий иначе не
// зовётся.
//
// # Почему это НЕ противоречит доводу «триггер, а не вызов в каждом месте записи»
//
// Довод миграции журнала считал ПИСАТЕЛЕЙ: их полсотни, и явная эмиссия означала
// бы полсотни точек, каждая обязана быть в транзакции мутации, а пропущенная
// теряет событие молча. Здесь писатель ОДИН — путь последствий каталога, — и он
// единственный в продукте, кто пишет подтаблицу роли в обход её строки. Довод
// про полсотни к одному месту не относится, а его предмет (событие обязано
// лежать в транзакции мутации) выполняется здесь by construction: оператор идёт
// под `w.tx`, то есть под той же транзакцией, что снятие и переселение.
//
// # «РОВНО ОДНО событие на роль и транзакцию» — свойство ОПЕРАТОРА, а не входа
//
// Отбор идёт `FROM kaname.roles r WHERE r.id = ANY($1)`, а `id` — первичный
// ключ. Значит строка роли попадает в выборку ОДИН раз, сколько бы раз её
// идентификатор ни повторился во входном массиве. Три популяции поэтому
// складывают свои роли встык, без сведения множеств на языке Go: сведение,
// написанное рядом, было бы вторым местом, отвечающим за единственность, и
// разошлось бы с оператором молча.
//
// Роль, СНЯТАЯ к этому мигу, события не получает — её нет в `roles`, и это
// верно: собственное снятие роли эмитит её же триггер, а второе событие о том же
// было бы дублем.
//
// # Якорь и области берутся У ЖУРНАЛА, а не собираются здесь
//
// `kaname.resource_journal_project` и `kaname.resource_journal_scope` — те же
// функции, которыми их считает триггер эмиссии. Выписанные здесь вторично, они
// стали бы вторым правилом вывода якоря и областей, и разошлись бы на первом же
// ярусе объявления роли — молча, потому что обе величины суть значения одного
// типа. Нагрузка — только идентификатор, как и у триггера: состояния этот журнал
// не производит (задача службы #72).
const announceRoleGrantWithdrawalSQL = `
		INSERT INTO kaname.resource_journal
		       (resource_kind, resource_id, project_id, scope, event_type, payload)
		SELECT 'iam_role', r.id,
		       kaname.resource_journal_project('iam_role', to_jsonb(r)),
		       kaname.resource_journal_scope('iam_role', to_jsonb(r)),
		       'UPDATED',
