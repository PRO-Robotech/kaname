...
// (`ceremonyport.AccessTokens`). Проба судит ЧТЕНИЕ решения тремя
// поверхностями; то, что адаптер пишет запись до ответа и роняет выпуск на её
// отказе, держат его пробы (`internal/ceremonyport`: `issuance_record_test.go`
// и сквозная `revocation_reaches_presentation_test.go`) и ось гейта
// `internal/check` («выпуск пишет запись»).
//
// # Таблица согласия
//
...
// семейство: подпись и запись в том порядке, в каком их исполняет адаптер
// порта выпуска (`ceremonyport.AccessTokens`).
