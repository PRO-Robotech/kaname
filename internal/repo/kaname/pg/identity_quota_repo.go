...
//
// У них снимок величины приезжает от соседа и может отставать; здесь величина
// объявлена ПОСАДКОЙ этой же службы и лежит в этой же базе проекцией
// (`kaname.own_ceilings`), а списание обновляет снимок в том же операторе.
// Поэтому нет ни резолва по сети, ни курсора дельты, ни материализации на
// стороне Go — есть чтение и добор недостающего из посадки.
//
// # ИСТОЧНИК ВЕЛИЧИНЫ СМЕНИЛСЯ, ПРЕДМЕТ ЧТЕНИЯ — НЕТ
//
// Прежде недостающее добиралось из АВТОРИТЕТА величин (`kaname.limits`, область
// `DEFAULT`). Авторитет выпилен из службы целиком решением владельца 2026-09-06
// (`PRO-Robotech/kacho#2117`, приёмка `KAN-QUOTA-1`, стадия S4), и ветвь,
// читавшая его таблицу, снята вместе с ним: она осталась бы запросом к таблице,
// которой нет, — то есть отказом на пути чтения, приходящим вместо ответа.
//
// Арендаторское чтение при этом НЕ УХОДИТ и не сужается: `IdentityQuotaService`
// объявлен собственным контрактом службы именно затем, чтобы пережить уход
// модуля (решение `Д9`, `kacho#2362`).
//
// # Почему ответ добирается, а не отдаётся как есть
//
// Строка учёта заводится триггером на первом же аккаунте, поэтому у всякой
// вошедшей личности она есть. Но до первого аккаунта её нет, а значит нет и
// потолка в ответе. Пустой набор был бы прочитан как «предела нет», ровно
// наоборот действительности.
...
func (r *IdentityQuotaRepo) States(ctx context.Context, identity string) ([]quotaread.State, error) {
	...
	// Добор недостающего ИЗ ПОСАДКИ. Вид, которого посадка не объявила, в ответ
	// не попадает вовсе — и это не пропуск: «потолок не назван» есть отказ, а не
	// бесконечность, и показывать его числом было бы неправдой в обе стороны.
	//
	// Перечень видов берётся у СЛОВАРЯ, а не выписывается здесь: выписанный
	// разошёлся бы с ним молча при первом же новом виде, и арендатор перестал бы
	// видеть свой потолок, не узнав об этом ниоткуда.
	for _, kind := range domain.IdentityCarriedKinds() {
		if have[string(kind)] {
			continue
		}
		st, ok, derr := r.statedDefault(ctx, string(kind), identity)
		if derr != nil {
			return nil, derr
		}
		if ok {
...
// statedDefault читает величину, объявленную ПОСАДКОЙ.
//
// Область у ответа одна — `DEFAULT`, и это ограничение НАЗВАНО, а не умолчание,
// за которое можно спрятаться: словарь областей контракта знает `DEFAULT`,
// `ACCOUNT` и `PROJECT`, а величина посадки одна на установку и принадлежит
// тому, кто её ставит, — то есть ни арендатору, ни его аккаунту, ни его проекту.
// Личный потолок отдельному человеку невыразим; это предмет, а не пропуск.
//
// ИСТОЧНИК ОДИН, И ВТОРОГО БОЛЬШЕ НЕТ. Прежде здесь стояла развилка: вид посадки
// читался из проекции, всякий другой — из таблицы авторитета. Авторитет снят
// стадией S4, и вместе с ним снята развилка: всякий вид, носителем которого
// является личность, объявляется посадкой by construction — свойство держит
// проба словаря, а не эта строка.
//
// Вид, пришедший сюда мимо посадки, есть НАРУШЕНИЕ ИНВАРИАНТА, а не состояние
// мира: отвечать на него пустотой значило бы показать арендатору «потолка нет»
// там, где о потолке не известно ничего.
func (r *IdentityQuotaRepo) statedDefault(
	...
	if !domain.IsPostureStatedKind(domain.LimitKind(kind)) {
		return quotaread.State{}, false, fmt.Errorf(
			"вид %s носится личностью, но посадкой не объявлен: источника величины "+
				"у него нет ни одного", kind)
	}

	value, ok, serr := statedOwnCeiling(ctx, r.pool, kind)
	if serr != nil {
		return quotaread.State{}, false, serr
	}
	if !ok {
		// Посадка вид не объявила. Ответить нулём нельзя: ноль есть решение «не
		// заводить», а здесь величина неизвестна — и «потолок не назван» есть
		// отказ, а не бесконечность и не запрет.
