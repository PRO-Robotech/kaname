...
//
// ЗДЕСЬ СТОЯЛО ШЕСТЬ, и довод был такой: гарантию порядка несёт
// детерминированная проба ниже, а вероятностное обнаружение гарантией не
// является ни при каком числе прогонов, — значит платить за него временем
// незачем. Довод снят, потому что не выдержал собственного замера: сокращение
// делалось заодно ради чужого флейка стенного времени
// (`cmd/kaname/loginlane_envelope_test.go:82`), и флейка оно НЕ починило —
// дерево покраснело на нём и с шестью столкновениями. Сеть сузили, не купив
// ничего.
//
// Двенадцать, а не минимально допустимые шесть: две пробы ловят РАЗНОЕ.
// Детерминированная утверждает сам инвариант на ОДНОМ чередовании — том,
// которое построено руками. Эта перебирает чередования, которых она не
// строит, и ценна ровно этим; нижняя граница приемлемого целью не является.
