...
// lockOrderRuns — число прогонов на каждую сцену старта. Три сцены по два
// прогона: ШЕСТЬ столкновений, и каждое печатает свой исход.
//
// Шесть, а не больше, и причина названа: гарантию порядка несёт
// ДЕТЕРМИНИРОВАННАЯ проба ниже (`TestOAuthExchangeTakesTheFamilyBeforeTheChild`),
// которая краснеет на сломанной форме ВСЕГДА. Эта — судит исход гонки, и её
// вероятностное обнаружение гарантией не является ни при каком числе прогонов.
// Платить за него временем и параллельной нагрузкой всего дерева смысла нет.
const lockOrderRuns = 2
