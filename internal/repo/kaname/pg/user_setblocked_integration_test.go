...
//
// Run: `go test ./internal/repo/kaname/pg/... -run UserSetInviteStatus`.
...
func readInviteStatus(t *testing.T, ctx context.Context, repo *kanamepg.Repository, id domain.UserID) string {
	t.Helper()
...
func newSetBlockedEnv(t *testing.T) (context.Context, *kanamepg.Repository, *pgxpool.Pool) {
	t.Helper()
	if testing.Short() {
	...
	return ctx, kanamepg.New(pool, nil), pool
}
