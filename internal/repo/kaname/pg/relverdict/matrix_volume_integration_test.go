...
const matrixRunCommand = "KACHO_MATRIX_VOLUME=1 go test -C services/iam " +
	"./internal/repo/kaname/pg/relverdict/ -run TestMatrixVolume_Report " +
	"-count=1 -v -timeout 120m"

// matrixReportPath — куда ложится отчёт, рядом с отчётами соседних приборов.
const matrixReportPath = "services/iam/internal/repo/kaname/pg/scalegrid/REPORT-R7-3-matrix-volume.txt"
...
func measureMatrixPoint(t *testing.T, ctx context.Context, pool *pgxpool.Pool,
	repo *kanamepg.Repository, capture *verdictCapture, p matrixPoint, seq *int) matrixResult {
	t.Helper()
