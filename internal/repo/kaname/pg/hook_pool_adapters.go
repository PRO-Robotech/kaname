...
func (s *SessionRevocationsAdapter) RevokeAllUserTokensTx(
	...
	// ОТСЕЧКА КЛАДЁТСЯ В ОБЕ ЗАПИСИ ОДНОЙ ТРАНЗАКЦИЕЙ (задача kaname#313).
	//
	// # Почему записей две и почему обе обязательны
	//
	// Они не взаимозаменяемы: у каждой свой ключ, свой круг читателей и своя
	// величина сравнения. Запись выше судят хуки выдачи — они сравнивают её с
	// моментом аутентификации СЕССИИ. Запись ниже судит авторитет отзыва на
	// ПУТИ ЗАПРОСА — он сравнивает её с отметкой выпуска предъявленного
	// носителя (`internal/tokenrevocation`), и по ней проходит каждое
	// предъявление токена, отчеканенного нами.
	//
	// Снятие доступа, дошедшее до одной и не дошедшее до другой, есть контроль,
	// исполненный наполовину и выглядящий исполненным целиком: глагол отвечает
	// успехом, а предъявление продолжает приниматься.
	//
	// # Почему ОДНОЙ транзакцией, а не двумя обращениями
	//
	// Две записи об одном решении, положенные порознь, расходятся на отказе
	// между ними — и расходятся в сторону, которой никто не увидит, потому что
	// вызывающему уже отвечено. Здесь они коммитятся вместе либо не коммитятся
	// вовсе (запрет #10), и частичного снятия доступа не бывает by construction.
	//
	// # Почему не сведены в ОДНУ запись и не спрошены ОБЕ читателем
	//
	// Свести — значит перекеить одну из них и переписать обоих читателей,
	// включая схемных писателей-триггеров; это меняет полосу, которая сегодня
	// исполняет. Спросить обе на предъявлении — значит сравнивать отметку
	// выпуска носителя с величиной, заведённой под сравнение с моментом
	// аутентификации сессии: один и тот же столбец получил бы два смысла.
	// Обе дороги меняют наблюдаемое поведение прежней посадки; эта — не меняет.
	//
	// ЧТО СТАНЕТ НЕПРАВДОЙ, ЕСЛИ РЕШЕНИЕ НЕГОДНО: если у отсечки предъявления
	// появится второй смысл (не «выпущенное раньше недействительно»), запись её
	// здесь станет расширением отзыва за пределы того, что решил распорядитель.
	// Признак этого — появление у `minted_token_revocations` читателя, который
	// сравнивает её НЕ с отметкой выпуска.
	//
	// # РЕШИВШИЙ НАЗЫВАЕТСЯ ВСЕГДА — ПРИНЦИПАЛОМ ЛИБО МЕХАНИЗМОМ
	//
	// Актор здесь законно бывает пуст: этот же оператор исполняет и выход
	// САМОГО человека, у которого распорядителя нет вовсе (`RevokeInput.RevokedBy`
	// объявлен пустым для этого случая). Запись предъявления пустого решившего
	// не принимает — ограничение схемы требует непустого, — и это верно: отсечка
	// без принявшего неоспорима.
	//
	// Поэтому пустой актор замещается ИМЕНЕМ МЕХАНИЗМА, а не пустой строкой и не
	// подставным человеком. Имя берётся у события, которое этот же вызов кладёт
	// в журнал, — оно из закрытой таксономии и уже различает два акта. Тот же
	// приём, которым называют себя схемные писатели этой строки
	// (`kaname:client-key-revoked`, `kaname:owner-deactivated`).
	decidedBy := string(revokedBy)
	if decidedBy == "" {
		decidedBy = "kaname:" + eventType
	}
	if err := upsertMintedCutoff(ctx, tx, string(userID), revokeBefore, reason, decidedBy); err != nil {
		return err
	}
	payload := map[string]any{
		"actor":        string(revokedBy),
