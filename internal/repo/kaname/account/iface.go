...
//
// Подставного репозитория с общим домом У ЭТОГО ПОРТА НЕТ, и это решение, а не
// пропуск: подставные объявляет САМА проба use-case'а, рядом с собой (замер на
// момент записи — 29 файлов проб объявляют свои). Общий дом объявлял бы один
// набор ожиданий на всех вызывающих, и проба, которой нужно другое поведение
// порта, правила бы чужое. Прежняя редакция этой шапки отправляла читателя в
// каталог `internal/repo/repomock`, где не было ни одного объявления — только
// его описание; каталог снят вместе с описанием (#2503).
package account
...
type ListFilter struct {
	...
	// каждому выносит модель прав (§«Авторизация живёт в МОДЕЛИ»).
