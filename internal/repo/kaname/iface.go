...
type Writer interface {
	...
	// строкой приглашения (ban #10) — И ПРЕЖДЕ списывает окно частоты адресата
	// (приёмка ID-MAIL-1, Р14/Р22, MAIL-25).
	//
	...
	//
	// ОГРАНИЧИТЕЛЬ СТОИТ ЗДЕСЬ, НА ЕДИНСТВЕННОМ ПУТИ ПИСЬМА, и поэтому всякий
	// глагол, отправляющий письмо, проходит через него по построению — второго
	// способа поставить намерение у use-case нет. Сверхнормативное намерение в
	// очередь не попадает; вызов при этом отвечает `queued=false`, а НЕ ошибкой:
	// ответ глагола обязан быть неотличим от ответа в норме (Р9). Непозитивное
	// ограничение — отказ: значения «без ограничения» не существует.
	//
	// Intent.UserID служит ключом партиции порядка: письма одному человеку
	// уходят в том порядке, в котором их поставили. Ссылки-предъявителя
