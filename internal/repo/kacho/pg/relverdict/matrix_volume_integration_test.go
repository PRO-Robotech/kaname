...
//
//   - ДВИЖОК ОТНОШЕНИЙ (OpenFGA) не участвует: он требует поднятого стенда.
//     Меряется РЕЛЯЦИОННАЯ форма вердикта — тот код, который отвечает на вопрос
//     о доступе по таблицам iam;
...
//     `b.revoked_at IS NULL`. Материализация нужна для кортежей движка, то есть
//     для предмета соседнего прибора, а не этого;
