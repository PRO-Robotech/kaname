...
//
// Ничего о сегодняшнем пути принятия решения. Реляционный вердикт провязан
// теневым сравнителем, а решение о доступе принимает движок отношений. Числа
// этой пробы описывают БУДУЩИЙ путь, и складывать их со стоимостью движка
...
func renderStrengthReport(grid [][]scalegrid.StrengthPoint, results [][]strPointResult,
	...
	w("  1. Числа описывают путь, который СЕГОДНЯ РЕШЕНИЯ НЕ ПРИНИМАЕТ. Реляционный\n")
	w("     вердикт провязан теневым сравнителем; решение о доступе принимает движок\n")
	w("     отношений. Читать эти числа как характеристику продакшена нельзя, и\n")
	w("     складывать их со стоимостью движка тоже: у них разные единицы.\n")
