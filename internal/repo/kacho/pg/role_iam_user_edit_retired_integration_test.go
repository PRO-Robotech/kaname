...
// запросе. Поэтому рядом стоят соседние роли того же семейства — они обязаны
// ОСТАТЬСЯ: `iam.user.view` даёт чтение (у `get`/`list` читатели есть), а
// `iam.user.admin` подстановкой `*` разворачивается в набор типа и продолжает
// давать всё, что тип объявляет.
package pg_test
...
func TestRoleIamUserEdit_RetiredWhileItsNeighboursRemain(t *testing.T) {
	...
	for _, n := range []struct{ name, why string }{
		...
		{"iam.user.admin", "подстановка `*` разворачивается в набор ТИПА, поэтому роль продолжает " +
			"давать всё, что тип объявляет"},
	} {
