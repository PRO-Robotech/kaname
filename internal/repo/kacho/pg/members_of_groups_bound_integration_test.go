...
func TestIntegration_R914_MembersOfGroupsIsBoundedAndNamesTheTruncation(t *testing.T) {
	...
	repoForSeed := kachopg.New(pool, nil)
	...
	read := rowsReadBy(t, ctx, pool, kachopg.MembersOfGroupsSQL,
		[]string{smallGroup, hugeGroup}, kachopg.MaxMembersInGrantSurface+1)
	assert.LessOrEqualf(t, read, float64(kachopg.MaxMembersInGrantSurface+1),
		"оператор прочитал %.0f строк при пределе %d: без предела выборки он читает ВЕСЬ "+
			...
			"начинает принадлежать чужому членству", read, kachopg.MaxMembersInGrantSurface)
