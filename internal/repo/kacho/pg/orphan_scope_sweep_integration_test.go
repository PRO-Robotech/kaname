...
func newOrphanScopeSweeper(pool *pgxpool.Pool, repo *kachopg.Repository) *seed.OrphanScopeSweeper {
	return seed.NewOrphanScopeSweeper(repo, kachopg.NewOrphanScopeAdapter(pool), seed.OrphanScopeConfig{})
}
...
func TestOrphanScope_03_CensusNamesTheDeadScopeAndNotTheLiveOne(t *testing.T) {
	...
	scopes, err := kachopg.NewOrphanScopeAdapter(pool).ListOrphanBindingScopes(ctx, 100)
	require.NoError(t, err)
