	...
	for _, kind := range domain.AssertionClientKinds() {
		for _, cause := range causes {
			t.Run(string(kind)+"/"+cause.name, func(t *testing.T) {
				...
				revocations := kachopg.NewMintedTokenRevocationRepo(f.pool)
...
func TestF2_32_UnavailableRevocationAuthorityRefuses(t *testing.T) {
	...
	requireIntrospect(t, newIntrospectAuthority(rig, kachopg.NewMintedTokenRevocationRepo(f.pool)),
	...
	rec := askAuthority(t, newIntrospectAuthority(rig, kachopg.NewMintedTokenRevocationRepo(dead)), out.AccessToken)
