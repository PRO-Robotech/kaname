...
//
// # Что здесь меряется, и почему это не «рамка A» и не «рамка B целиком»
//
// Рамка A (pg/relverdict + pg/scalegrid) меряет ЧТЕНИЕ реляционной формы — путь,
// который сегодня решения о доступе не принимает. Рамка B — сегодняшний путь:
// материализатор прав плюс движок отношений. Движок здесь НЕ УЧАСТВУЕТ: он
// требует поднятого стенда, и это записано прямо, а не подразумевается.
//
// Что участвует — ПРОДУКТОВЫЙ МАТЕРИАЛИЗАТОР, целиком и своим кодом:
// `reconcile.Reconciler` поверх настоящего `pg.ReconcileAdapter` и настоящего
// Postgres (testcontainers). Именно его до сих пор не мерил НИКТО: соседний
// прибор `tools/authzformbench` меряет ДВИЖОК, и приписывать его числа продукту
// нельзя — у них разные единицы.
//
// Что из-за отсутствия движка НЕ измеряется и потому не утверждается:
//
//	· время применения кортежа к движку (слагаемое 6 окна небезопасности);
//	· ожидание в очереди `fga_outbox` до дренажа (слагаемое 5);
...
func TestStrengthWriteDelete_Report(t *testing.T) {
	...
	if os.Getenv(strengthWDEnv) == "" {
		t.Skipf("замер записи и удаления идёт РУЧНЫМ прогоном: %s=1 go test "+
			"./services/iam/internal/repo/kacho/pg/ -run TestStrengthWriteDelete_Report "+
...
func renderWDReport(results []wdResult, wall time.Duration, final bool) string {
	...
	w("  НЕ ИЗМЕРЯЕТ движок отношений: его здесь НЕТ. Значит НЕ измерены и не\n")
	w("            утверждаются: время применения кортежа к движку; ожидание в очереди\n")
	w("            движка до дренажа; кэши вердиктов края и сервиса-владельца;\n")
	w("            «время до первого ОТКАЗА» — несущая величина удаления, чей предмет\n")
	w("            есть наблюдаемый отказ на публичном RPC. Это ТРЕТИЙ исход по ним,\n")
	w("            а не «уложились».\n")
	w("  Числа этого отчёта НЕЛЬЗЯ складывать с числами прибора tools/authzformbench:\n")
	w("  тот меряет ДВИЖОК, единицы разные.\n")
