...
var knownUnresolvableSeedPairs = map[string]string{
	...
	//
	// Записано как цена, а не как норма, и цена крупнее, чем читалась: имена
	// придётся привести к словарю (тогда роли заработают) либо правила снять
	// (тогда роль уйдёт в легаси-ветку и выдаст ярус — это ДРУГАЯ выдача, не
	// уборка, и она требует своей приёмки).
