...
var knownUnresolvableSeedPairs = map[string]string{
	...
	// ── Учётка оператора сети: веер СНЯТ (миграция 0076) ─────────────────────
	// Здесь стояли ЧЕТЫРЕ записи — `vpc.subnetses`, `iam.projectses`,
	// `vpc.networks` и `vpc.network_interfaces`. Все четыре принадлежали ОДНОЙ
	// роли `module.vpc_operator_sa`, и это стоит отметить: две последние лежали
	// в блоке служебных учёток выше и читались как общий класс модульных SA,
	// хотя автором каждой была только эта роль — перепись по дереву даёт ровно
	// одно вхождение на имя.
	//
	// Роль снята целиком (правила, права, привязка, роль) миграцией 0076: ни
	// одно из четырёх имён закрытая таблица типов не несёт, поэтому
	// материализация не эмитила ни одного кортежа, а каскад
	// `viewer … or system_viewer from cluster`, ради которого веер задумывался,
	// из модели удалён. Записи сняты вместе с предметом — перечень обязан
	// самоистекать, иначе он описывает мир, которого нет.
	//
	// Кортеж `system_viewer@cluster` оператора при этом ОСТАЛСЯ: у него с тех
	// пор появился второй, действующий читатель (см.
