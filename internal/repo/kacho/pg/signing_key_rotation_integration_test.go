...
func signingKeyPool(t *testing.T) (*pgxpool.Pool, *kachopg.SigningKeyRepo) {
	t.Helper()
	pool, err := coredb.NewPool(context.Background(), setupTestDB(t))
	require.NoError(t, err)
	pgtest.ClosePoolAtEnd(t, pool)
	return pool, kachopg.NewSigningKeyRepo(pool)
}

func seedKey(t *testing.T, repo *kachopg.SigningKeyRepo, kid string, state domain.SigningKeyState) domain.SigningKeyRecord {
	t.Helper()
