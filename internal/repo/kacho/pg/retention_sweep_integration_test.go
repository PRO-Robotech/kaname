...
func countAssertions(t *testing.T, ctx context.Context, repo *kachopg.ClientAssertionReplayRepo) int64 {
	t.Helper()
	n, err := repo.Len(ctx)
	require.NoError(t, err)
	return n
...
func TestRetentionSweep_UserDeleteOutcomeChangesOnlyForExpiredRows(t *testing.T) {
	ctx, pool := retentionPool(t)
	repo := kachopg.NewSessionRevocationRepo(pool)
	repository := kachopg.New(pool, nil)
...
func deleteUser(t *testing.T, ctx context.Context, repo *kachopg.Repository, uid domain.UserID) error {
	t.Helper()
