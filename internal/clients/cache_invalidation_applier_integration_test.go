...
// ─────────────────────────────────────────────────────────────────────────────
// setupTestPGDrainer — hands the caller its own migrated database on the
// package's shared Postgres, and returns a pool aimed at it.
//
...
// a migrated template is the isolation a separate container gave. The
// LISTEN/NOTIFY proofs below are unaffected: NOTIFY is per-database, so a
...
func setupTestPGDrainer(t *testing.T) *pgxpool.Pool {
	...
	dsn := pgtest.NewDB(t) + "&options=-c%20search_path%3Dkacho_iam%2Cpublic"
