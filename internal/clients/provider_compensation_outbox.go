...
//
// 404 ОСТАЁТСЯ УСПЕХОМ — и это решение, а не умолчание. Тот же ответ зеркало
// набора ключей читает как «по адресу не тот эндпоинт», то есть ДВЕ поверхности
// одной службы читали один сигнал противоположно, и разницу никто не решал: при
// неверно настроенном административном адресе снятие «удавалось» на каждой
// строке, и очередь сходилась, не сняв ничего.
//
// Признать 404 отказом было бы хуже: строка перестала бы помечаться
// доставленной, дошла бы до порога отравления и ЗАКЛИНИЛА свою партицию, а
// занятое у поставщика осталось бы занятым. Поэтому неразличимость признана
// ЯВНО и получила свою клетку счётчика — [ProviderRoadOutcomeAbsent]; разбор
// размена живёт в одном месте, `provider_road.go`, и здесь не пересказывается.
