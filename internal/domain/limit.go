...
var countableKinds = []CountableKind{
	...
	//
	// Поэтому снятие ЗАКРЫВАЕТ класс «принято-и-проигнорировано», а не
	// заводит потерю контроля: применять было
...
func (k LimitKind) Validate() error {
	...
	// ВЕЛИЧИНУ ЭТИХ ТРЁХ ОБЪЯВЛЯЕТ ПОСАДКА, и назначить её снаружи нельзя
	// (`limit_posture_stated.go`, приёмка `KAN-QUOTA-1`, `П25`).
	//
	// Отказ стоит ПОСЛЕ проверки членства намеренно: вид, которого в каталоге
	// нет вовсе, обязан получить прежний отказ о членстве, иначе оператор
	// узнал бы про посадку вместо опечатки в имени вида.
	//
	// Проверка судит ВХОД авторитета — создание и сужение перечня. Списание и
	// арендаторское чтение её не зовут и звать не должны: эти три вида
	// по-прежнему считаются, у них сменился только источник величины.
	if IsPostureStatedKind(k) {
		return refusePostureStatedKind(k)
	}
	return nil
}
