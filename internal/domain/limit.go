...
func ResolveEffective(service string, stated []Limit) []EffectiveLimit {
	...
	for _, k := range CountableKindsOfService(service) {
		// Виды, считаемые В РОДИТЕЛЬСКОМ РЕСУРСЕ, этим чтением не отвечаются.
		//
		// Резолв спрашивают, называя корень аренды — проект либо аккаунт. У вида,
		// чей носитель есть родительский ресурс, на этом уровне НЕТ единственного
		// значения: подсетей столько-то в каждой сети, а не в проекте. Отдать его
		// здесь можно, только назвав носителем то, чем он не считается.
		//
		// Именно это и происходило: потребитель проставлял каждой строке носителя
		// «проект» константой, и арендатор получал четыре строки с неверным
		// носителем и потреблением, которое не наполнится никогда (списание идёт
		// по носителю). Хуже, состав ответа зависел от истории проекта — свежий
		// получал двенадцать строк отсюда, тот же после первой мутации восемь из
		// своих строк учёта.
		//
		// Ось «сколько детей у родителя» закрывается своим чтением, по родителю.
		if c, known := CarrierOfKind(k); known && c != CarrierProject && c != CarrierAccount {
			continue
		}
		w, ok := winner[k]
		if !ok {
