...
// стоит на ФИЛЬТРУЕМОЙ полосе (право `iam.roles.get`), где край не спрашивает
// пообъектно ничего, а одиночное чтение кастомной роли энфорсится В СЕРВИСЕ той же
// самой функцией, что фильтрует страницу (`role.resolveVisibleRoleIDs`, общая для
// ListRoles и GetRole). Здесь стояла ПОЛОСА ОСВОБОЖДЕНИЯ — неверно: она допускает
// вызов вообще без принципала, а сужать по вызывающему тогда не по кому (kacho#1922). Обе поверхности задают ОДИН вопрос, поэтому разойтись не
