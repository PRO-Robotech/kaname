...
// ПАРАЛЛЕЛЬНО — это его заявленное свойство, — и продовый клиент движка прав
// безопасен для конкурентного вызова. Дублёр, который таковым не является,
// расходится с настоящим ровно в том, ради чего его подставляют, и роняет пробу
// гонкой в самом инструменте измерения.
...
// бы в Lost.
func TestTake_EngineFailureIsNotAbsenceOfAccess(t *testing.T) {
	_, err := Take(context.Background(),
		&stubChecker{err: errors.New("engine unavailable")},
		pagesOf("p1", "p2"), "user:u", "v_get", "project")
	require.Error(t, err,
		"отказ движка обязан доезжать до вызывающего, а не превращаться в пустой снимок")
}
...
func TestTake_WalksEveryObjectAcrossPages(t *testing.T) {
	...
	require.Equal(t, len(ids), s.Examined,
		"осмотрены обязаны быть ВСЕ объекты: остановка на первой странице дала бы усечённое "+
			"множество — ровно то, из-за чего перечисление у движка не годится")
