...
// пропуск стал безусловным. Замер на стволе `83ddd08a` (`go test -short -v ./...`,
// строки пропуска по этой предпосылке): **58** строк · **53** верхнеуровневые
// пробы · 2 подпробы · 1 строка без `SKIP` рядом. Среди них гейт безопасности
// «у каждого публичного RPC есть пообъектный вопрос». Молчание такой пробы
