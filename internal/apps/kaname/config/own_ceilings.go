...
//
// Служба доступа перестаёт быть авторитетом величин, но её собственные
// потолки остаются действующими: сколько аккаунтов заводит одна личность,
// сколько путей входа держит человек и машина, сколько ключей доступа держит
// человек. Спросить величину не у кого —
...
var OwnCeilingKnobs = []ownCeilingKnob{
	...
	{
		Key:  ownCeilingKeyPrefix + "access-keys-per-user",
		Env:  "KANAME_OWN_CEILINGS__ACCESS_KEYS_PER_USER",
		Kind: "iam.user.accessKey",
		Why: "сколько ключей доступа (WebAuthn) держит ОДИН человек одновременно. Ключ — путь входа " +
			"целиком, срока у него нет by construction, и снять его за спиной владельца нельзя; без " +
			"потолка число ключей ничем не ограничено (Ф7 Р8). Перенос ключей из прежнего компонента " +
			"расходует те же слоты",
		Value: func(c OwnCeilingsConfig) *int64 { return c.AccessKeysPerUser },
	},
}
...
type OwnCeilingsConfig struct {
	...
	// AccessKeysPerUser — потолок вида `iam.user.accessKey` (Ф7 Р8).
