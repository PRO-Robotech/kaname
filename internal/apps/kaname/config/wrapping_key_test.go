...
// ─────────────────────────────────────────────────────────────────────────────
// ЗДЕСЬ БЫЛ СОСЕД, СУДИВШИЙ РАЗДЕЛ ДОКУМЕНТАЦИИ О ЧЕКАНКЕ ЭТОГО КЛЮЧА
//
// `wrapping_key_readme_test.go` исполнял команду чеканки, напечатанную в
// разделе «Sealed-secret integration» README подчарта, и требовал, чтобы её
// вывод принимался ЭТИМ резолвом: документ, обещающий команду, которая даёт
// негодное значение, дороже отсутствующего документа.
//
// Раздел жил в зонтичном чарте стенда ПЛАТФОРМЫ. В этот репозиторий он не
// входит, и своего раздела о чеканке ключа обёртки у службы нет: `INSTALL.md`
// называет ручку и формат значения, но команды не приводит. После выноса службы
// проба не исполнялась ни разу — она спрашивала координату дерева платформы и
// пропускала себя целиком (kaname#108).
//
// Проба снята вместе с предметом. Предмет не исчез: он ждёт своего раздела в
// доме службы — и вернётся вместе с ним.
