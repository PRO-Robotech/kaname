...
func RegisterDefaults(v *viper.Viper) {
	...
	// api-server.rate-limit — ВЕЛИЧИНЫ ДОПУСКА обоих слушателей, на вызывающего.
	//
	// # Зачем ключи объявлены здесь, если ни одного числа тут нет
	//
	// Имена переменных окружения этой службы не встречаются в коде литералами —
	// viper выводит их из ПУТИ КЛЮЧА и разрешает переменную только для ключа,
	// который уже знает. Знает он его по умолчанию либо по привязке; ключ,
	// объявленный одним лишь тегом структуры, читается ФАЙЛОМ (`Unmarshal` идёт по
	// тегам) и переменной — НЕТ. До этих восьми строк ручка принималась профилем и
	// не читалась процессом.
	//
	...
	//
	// # Почему НОЛЬ — не «разумное значение, подставленное за оператора»
	//
	// Ноль здесь не величина, а ОБЪЯВЛЕННОЕ ТРЕТЬЕ СОСТОЯНИЕ: «посадка молчит».
	// Разрешает его не эта строка, а `AdmissionKnobs.Resolve`, и разрешает полом
	// платформы — `PlatformPublicAdmission` (чтений 100/с, мутаций 20/с, всплеск
	// ×5, одновременных 16) и `PlatformInternalAdmission` (1000/500/×5/256).
	// Молчащий оператор получает не отсутствие предела, а названный предел, и
	// композиционный корень печатает его в журнал с пометкой `from_posture=false`.
	//
	// Поэтому объявление ключей НЕ МЕНЯЕТ ИСХОДА при пустом окружении, и это
	// утверждается пробой, а не обещанием: законный близнец
	// `TestAdmissionKnobsSilentEnvTakesThePlatformFloor` требует пола на пустом
	// вводе и был зелёным ДО этих строк ровно так же, как после.
	//
	// # Что теперь достижимо и чего не было
	//
	// Отказ старта на ЧАСТИЧНОМ наборе. Он обещан комментарием структуры настроек
	// и исполнялся только файлом: неизвестный виперу ключ давал пустой набор, то
	// есть законное молчание, — и самый опасный вход (назвал темп, забыл
	// одновременность) проходил молча, оставляя процесс на полу платформы, пока
	// оператор считал предел выставленным.
	//
	// ENV: KANAME_API_SERVER__RATE_LIMIT__{PUBLIC,INTERNAL}__{READ_PER_SEC,
