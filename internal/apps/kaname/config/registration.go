...
// первом заведении был бы отказом завестись. Величина считает заведения за
// окно, СЧИТАЯ первое, и отвергает N+1-е в окне (при 3 — четвёртое): повторную
// регистрацию адресом, чья строка снята, и последующие `Account.Create` того же
// человека. Ноль законен: после первого — ни одного.
//
...
type RegistrationConfig struct {
	...
	// окно, СЧИТАЯ первое (при 3 четвёртое в окне отвергается). Указатель: ноль
	// законен («после первого — ни одного») и отличим от «не задано».
...
func (r RegistrationConfig) ValidateAdmissionRate() error {
	...
	case r.AdmissionsPerWindow == nil:
		errs = multierr.Append(errs, fmt.Errorf("%s не задан (%s): потолок темпа заведения аккаунтов одной личностью объявляет "+
			"ПОСАДКА; первое заведение (регистрация) проходит при любой величине и входит в счёт окна. "+
			"Задайте неотрицательное целое — сколько заведений за окно, считая первое; 0 означает «после первого — ни одного»",
			maxKey, registrationEnv(maxKey)))
