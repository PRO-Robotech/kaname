...
// JWKSProxyClientAuthModeValue — эффективный режим проверки клиента на
// слушателе зеркала проверочных ключей, тот самый, что уходит в транспорт.
//
// Нужен ТЕКСТУ ОТКАЗА: страж обязан назвать оператору не только ручку, но и
// величину, которую он на ней прочёл, — иначе отказ говорит «не то» и не
// говорит «что именно». Незаданная ручка отвечает своим разрешённым значением,
// а не пустой строкой: пустоту оператор прочёл бы как «ручки нет».
func (m MTLSConfig) JWKSProxyClientAuthModeValue() string {
	return resolveClientAuthMode(m.JWKSProxyClientAuthMode)
}

// JWKSProxyRequestingModeName — имя ЗАПРАШИВАЮЩЕГО режима, которое страж
// называет оператору как требуемое значение.
//
// Отдаётся отсюда, а не пишется у стража литералом: величина, которую
// транспорт принимает, объявлена в этом файле, и вторая её запись разошлась бы
// с первой молча — отказ продолжал бы советовать значение, которого транспорт
// уже не знает.
//
// Именно запрашивающий, а не взаимный: на том же слушателе живёт набор
...
// InternalRESTRequiresClientCert отвечает, ТРЕБУЕТ ли внутренний REST-фронт
// проверенного клиентского сертификата на рукопожатии.
//
// # Почему отдельный вопрос, а не чтение поля режима
//
// Спрашивают об этом ДВОЕ: страж посадки и ось самоотчёта о том, чем
// поверхность аутентифицирует вызывающего. Прочитанные порознь, они разошлись
// бы молча — и разошлись бы именно там, где расхождение не видно снаружи:
// показание продолжало бы называть механизм, которого на проводе нет. Ответ
// один, и оба берут его здесь.
//
// # Почему «требует» — это ТОЛЬКО взаимный режим
//
// Запрашивающий режим (`optional-mutual`) сертификата НЕ ТРЕБУЕТ: соединение
// без него проходит, и сужает уже обработчик, который проверенного пира
// требует сам. У этой поверхности такого обработчика НЕТ by construction —
// мультиплексор фронта своего рубежа не заводит и личность вызывающего внутрь
...
func (m MTLSConfig) InternalRESTRequiresClientCert() bool {
	...
	return resolveClientAuthMode(m.InternalRESTClientAuthMode) == clientAuthMutual
}

// InternalRESTClientAuthModeValue — эффективный режим проверки клиента на
// внутреннем REST-фронте, тот самый, что уходит в транспорт.
//
// Нужен ТЕКСТУ ОТКАЗА: страж обязан назвать оператору не только ручку, но и
// величину, которую он на ней прочёл, — иначе отказ говорит «не то» и не
// говорит «что именно». Незаданная ручка отвечает своим разрешённым значением,
// а не пустой строкой: пустоту оператор прочёл бы как «ручки нет».
func (m MTLSConfig) InternalRESTClientAuthModeValue() string {
	return resolveClientAuthMode(m.InternalRESTClientAuthMode)
}

// InternalRESTMutualModeName — имя взаимного режима, которое страж называет
// оператору как ТРЕБУЕМОЕ значение.
//
// Отдаётся отсюда, а не пишется у стража литералом: величина, которую
// транспорт принимает, объявлена в этом файле, и вторая её запись разошлась бы
// с первой молча — отказ продолжал бы советовать значение, которого транспорт
// уже не знает.
func InternalRESTMutualModeName() string { return clientAuthMutual }
