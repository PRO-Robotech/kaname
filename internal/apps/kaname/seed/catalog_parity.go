...
// CatalogSource — порт стража: ЖИВОЕ множество каталога и СНЯТОЕ, РАЗДЕЛЬНО.
//
// Объявлен ЗДЕСЬ, у потребителя, и половины спрашиваются по отдельности, потому
// что страж МЕРЯЕТ ИХ ПОРОЗНЬ: живую он сверяет с опорой, снятую — печатает
// объёмом и по ней отличает строку, снятую решением, от непроехавшей вовсе. Порт
// снимка (`catalog.RowSource`) отдаёт обе одним значением и одной транзакцией —
// у него вопрос другой: показать арендатору обе половины ОДНИМ ответом.
//
// Здесь прежде стояло, что снятое спрашивает ровно один вызывающий, а снимок
// «о снятом знать не должен». Второе утверждение снято вместе со своим
// предметом: снимок обязан называть снятый ресурс и его преемника, иначе
// преемник существует данными и не доезжает до арендатора ни одним путём чтения
// (kacho#1814). Первое осталось верным в своей части — СВОЕГО чтения снятого
// снимок не заводит: он получает строки, прочитанные ЭТИМ стражем
// (`CatalogParityCensus.Halves`).
type CatalogSource interface {
	// ReadLiveCatalog — строки, по которым выдача принимается.
...
type CatalogParityCensus struct {
	...
	Live catalog.Rows
	// Retired — СНЯТОЕ множество, прочитанное ЭТОЙ ЖЕ сверкой.
	//
	// Прежде здесь оставались одни числа (`RetiredModules`/`RetiredResources`/
	// `RetiredVerbs`), а СТРОКИ выбрасывались — при том что читаются они
	// безусловно и всегда. Снимок каталога из-за этого до первого обновления
	// отдавал арендатору пустой перечень снятого, и клиент, пришедший за
	// преемником снятого ресурса, узнавал, что платформа ничего не снимала
	// (kacho#1814, IAM-SUC-11).
	//
	// Донос уже прочитанного стоит НУЛЯ новых операторов — свойство арифметики
	// пути старта, а не обещание: строки уже внутри тех шести, что шлёт страж.
	Retired catalog.Rows
}

// Halves — обе половины, прочитанные этой сверкой, ОДНИМ значением.
//
// Метод существует затем, чтобы половины нельзя было разлучить по дороге к
// снимку: вызывающий, собирающий пару сам, вправе забыть снятую — и забывал бы
// молча, потому что пустая снятая половина от «ничего не снято» ничем не
// отличается.
func (c CatalogParityCensus) Halves() catalog.Halves {
	return catalog.Halves{Live: c.Live, Retired: c.Retired}
}
