...
const (
	...
	// (kaname#319): идентификатор выпуска → семейство. Темп задаёт арендатор:
	// строку пишет каждый обмен кода и каждая ротация. Читают её поверхности
	// предъявления, и каждая отвергает истёкший токен по его сроку, поэтому
	// строка за сроком токена ни одного исхода не меняет.
