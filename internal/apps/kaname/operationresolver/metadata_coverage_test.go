...
var unresolvedMetadata = []string{
	...
	// Здесь стояла тройка `Limit{Create,Update,Delete}Metadata`. Записи сняты
	// вместе со своим предметом: модуль величин выпилен из службы, и типов
	// метаданных предела контракт больше не объявляет ни одного. Перепись
	// назвала записи истёкшими сама — ровно как обещает шапка выше.
	//
	// Увидеть это удалось не сразу: проба спрашивала координату дерева платформы
