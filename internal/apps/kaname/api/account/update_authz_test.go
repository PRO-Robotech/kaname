...
func (f *authzAcctRepo) Reader(context.Context) (kanamerepo.Reader, error) {
	return &authzAcctReader{parent: f}, nil
}
func (f *authzAcctRepo) Writer(context.Context) (kanamerepo.Writer, error) {
...
// ненаречённое.
func (r *authzAcctReader) Visibility() visibility.ReaderIface { return nil }
...
// «сузить нечем», и списочный use-case обязан на нём ОТКАЗАТЬ, а не листать
// ненаречённое.
func (r *authzAcctWriter) Visibility() visibility.ReaderIface { return nil }
...
// подставляют.
func (w *authzAcctWriter) EmitInviteMail(_ context.Context, intent outboxtypes.InviteMailIntent) (bool, error) {
	if intent.To == "" {
