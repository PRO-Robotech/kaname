...
//   - почта не попадает в лог ни на успешном, ни на отказном пути АКТИВАЦИИ.
//
// # Граница названа, потому что прежде она была шире сделанного
//
// Здесь стояло «ни на одном из путей», а телом исполнялся ОДИН — активация
// (`doUpsert`). Путь ПРИГЛАШЕНИЯ (`InviteUserUseCase.Execute`) эта проба не
// осматривает вовсе, и почта писалась именно там; заявление звучало
// завершённо и было неверным (задача #2482). Путь приглашения судит
// `invite_email_out_of_log_test.go` — своим use-case и своим дублёром.
...
// IAM-ID-1-62: почта не попадает в лог ни на успешном, ни на отказном пути
// АКТИВАЦИИ. Про путь приглашения эта проба не утверждает ничего — он
// исполняется другим use-case и судится `invite_email_out_of_log_test.go`.
//
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NotContains(t, logBuf.String(), activationProbeEmail,
				"почта end-user'а не пишется в лог ни на успешном, ни на отказном пути "+
					"АКТИВАЦИИ (security.md §Hardening п.2); коррелировать следует по не-PII "+
					"идентификатору. Про путь приглашения этот отказ не говорит ничего — "+
					"его судит invite_email_out_of_log_test.go")
		})
	}
}
