...
//     «не завершена»;
//   - снятие ждёт блокировки строки сессии, которую держит ОДНА чужая
//     транзакция, дольше срока запроса → то же, и ОТВЕТ ПРИХОДИТ В СРОК
//     ЗАПРОСА: одно ожидание замка ограничено своим пределом, а не сроком
//     вызывающего (сумму ожиданий при нескольких держателях по очереди предел
//     не ограничивает, и сцена этого не утверждает);
...
// что при ОДНОМ держателе ответ приходит ВНУТРИ него: значит собственный
// предел одного ожидания замка короче, и на запись частичного исхода остаётся
// время.
