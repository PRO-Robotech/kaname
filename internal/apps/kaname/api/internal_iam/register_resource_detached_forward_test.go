...
func TestRegisterResource_ForwardSurvivesCallerCancellation(t *testing.T) {
	rec := &dfRecon{}
	txb := &smTxBeginner{}
	uc := NewRegisterResourceUseCase(smEmitter{}, mirrorAdapter{}, txb, seededCatalogTypes{}, &recordingPublisher{}).WithObjectReconciler(rec, nil)
...
func TestRegisterResource_DetachedForwardKeepsItsOwnBudget(t *testing.T) {
	rec := &dfRecon{}
	txb2 := &smTxBeginner{}
	uc := NewRegisterResourceUseCase(smEmitter{}, mirrorAdapter{}, txb2, seededCatalogTypes{}, &recordingPublisher{}).WithObjectReconciler(rec, nil)
