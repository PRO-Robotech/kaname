...
// ПРЕДМЕТ
//
// Писателей, снимающих несколько записей сессии одного человека, четыре:
// принудительный выход, смена пароля, снятие второго фактора, завершение
// восстановления. Двое из них снимают прочие записи и затем пишут в свою —
// строки сессии берутся «прочие → своя», а принудительный выход берёт все одним
// оператором в порядке просмотра. Порядки встречные, и соседняя сцена смены
// пароля (`force_logout_concurrent_teardown_integration_test.go`) ловит это
// взаимной блокировкой. Здесь — две сцены того же класса, которых там нет:
//
