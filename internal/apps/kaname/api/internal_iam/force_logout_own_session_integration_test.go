...
// личность неактивна.
//
// ЧТО ЭТА ПРОБА УТВЕРЖДАЕТ, А ЧТО НЕТ. Приёмка Ф3-25 ставит отказ предъявленной
// сессии НА КРАЮ — сравнением момента аутентификации с отсечкой, — и край живёт
// в другом доме. Про него здесь не утверждается ничего. Утверждается то, что
// измеримо отсюда: без снятия строка остаётся живой, и `Resolve` отвечает
// «сессия есть» на носитель, выданный ДО выхода. Отсюда же следует, что уборка
// такую строку не сносит (она сносит истёкшие и снятые), что след одного и того
// же выхода у человека и у распорядителя разный, и что «сессии нет» получает
// только тот, кто сверх резолва спросил и отсечку.
//
...
func TestIntegration_ForceLogoutEndsOurOwnLoginSession(t *testing.T) {
	...
	require.NotEqual(t, humansession.SessionFound, reason,
		"принудительный выход НЕ снял нашу сессию входа: носитель, выданный до "+
			...
			"резолв её не применяет by construction, и уборка не сносит не снятую строку")
