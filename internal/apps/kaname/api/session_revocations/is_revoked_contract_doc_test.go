...
// остался прежним: «True if a non-expired row exists in `session_revocations`»,
// источники отзыва — только выход и принудительный выход. Интегратор, читающий
// контракт, строил бы на нём и ждал бы `revoked_at`/`reason` у каждого
// отозванного — а у отзыва семейства их нет.
//
