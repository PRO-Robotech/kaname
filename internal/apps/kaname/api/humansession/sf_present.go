...
// состояние строки (не заведён) — у глаголов ПОД СЕССИЕЙ, недоступность
// материала, исчерпание ёмкости. На ВХОДЕ «не заведён» после совпавшего
// пароля — попытка и тот же отказ входа (Ф12-13 «е», kaname#257): это решает
// вход (`login.go`, `refuseSecondFactor`), а не сверка — здесь вердикт
// рождается, наружу его выпускает вызывающий.
