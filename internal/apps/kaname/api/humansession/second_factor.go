...
const (
	...
	// сессии — способ, уровень до и после и исход в поле `outcome`:
	// `accepted` у принятого, `refused` у отклонённого суждённого. Принятое
	// пишут четыре глагола с предъявлением внутри сессии — церемония,
	// подтверждение заведения, снятие фактора, перечеканка набора; отклонённое
	// — только церемония (`step_up.go`), тем же видом события. Производитель
	// журнала — Ф11; здесь заведён первым вместе с церемонией (§9 п.2 приёмки
	// Ф12), и имена способов в нём — словарь Ф11 Р8.
