...
type Store interface {
	...
	// PersonWriter открывает транзакцию записи, ПЕРВЫМ оператором которой
	// взята строка личности userID ключевым замком (`FOR KEY SHARE`) — той
	// силой, что взяла бы сама проверка внешнего ключа, но РАНЬШЕ любой
	// строки-ребёнка личности (kaname#382). Так открывает свою транзакцию
	// каждый вариант использования, пишущий строки сессии одной личности и не
	// снимающий прочих её записей: вход, выход, повышение, подтверждение
	// второго фактора, перечеканка запасных кодов.
	//
	// Удаление личности идёт «личность → дети»: строка `users`, затем каскадом
	// записи сессии, способы входа, отсечки. Писатель, взявший ребёнка раньше
	// личности (строку сессии выходом, строку способа сверкой кода) и
	// берущий личность проверкой внешнего ключа следующей вставки, шёл
	// навстречу: 3 взаимные блокировки из 3 прогонов у выхода, входа с
	// запасным кодом, подтверждения фактора и перечеканки, жертвой каждый раз
	// удаление (`session_row_writers_identity_deletion_integration_test.go`).
	// Ключевой замок совместим с остальными писателями личности и конфликтует
	// ровно с её удалением. Пустая личность — строки нет, держать нечего.
	PersonWriter(ctx context.Context, userID domain.UserID) (Writer, error)
}
