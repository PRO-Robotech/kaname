...
// `Resolve` отвечает «сессия» либо «сессии нет» и отсечки не видит. На ВЫДАЧЕ
// входа отсечку читает транзакция выдачи (`login.go`, kaname#385): вход,
// накрытый ею, сессии не выдаёт; предъявления это не меняет.
...
type Writer interface {
	...
	// (kaname#385, Р4): строка берётся замком `FOR SHARE`, и ПОСЛЕ этого
	// отдельным оператором читается стоящая отсечка личности. Замок конфликтует
	// с писателем нескольких сессий (принудительный выход, смена пароля) и с
	// удалением личности, совместим со вторым захватом и с проверкой внешнего
	// ключа. Отсечка, прочитанная после ожидания замка, видит выход,
	// зафиксированный за это ожидание; оператор, берущий замок, его не видит.
	//
	// Исходов четыре, и различаются они формой ответа: (T, true, nil) — строка
	// взята, отсечка T в разрешении хранилища; (_, false, nil) — строка взята,
	// отсечки нет; `NOT_FOUND` — строки нет; прочая ошибка — операция не
	// ответила. Пустая личность — отказ аргументом без обхода базы.
	LockPersonForLogin(ctx context.Context, userID domain.UserID) (revokeBefore time.Time, hasCutoff bool, err error)
