...
func (f *authzProjRepo) Reader(context.Context) (kanamerepo.Reader, error) {
	return &authzProjReader{parent: f}, nil
}
func (f *authzProjRepo) Writer(context.Context) (kanamerepo.Writer, error) {
...
// ненаречённое.
func (r *authzProjReader) Visibility() visibility.ReaderIface { return nil }
...
// «сузить нечем», и списочный use-case обязан на нём ОТКАЗАТЬ, а не листать
// ненаречённое.
func (r *authzProjWriter) Visibility() visibility.ReaderIface { return nil }
...
// подставляют.
func (w *authzProjWriter) EmitInviteMail(_ context.Context, intent outboxtypes.InviteMailIntent) (bool, error) {
	if intent.To == "" {
