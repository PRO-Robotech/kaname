...
// delete.go — DeleteProjectUseCase.
//
// НЕПУСТОЙ ПРОЕКТ НЕ УДАЛЯЕТСЯ (приёмка `non-empty-project-is-not-deleted.md`,
// задача продукта PRO-Robotech/kacho#1231). Состав проекта служба знает СВОЕЙ
// базой: владельцы ресурсов пяти доменов регистрируют каждый объект в зеркале
// (`kaname.resource_mirror`) внутренним глаголом, а своего ребёнка — проектную
// роль — служба ведёт собственной таблицей. Спрашивать владельцев на удалении
// не требуется и нельзя: служба — лист графа вызовов.
//
// Отказ по непустоте производится ОДНИМ оператором писателя
// (`projectWriter.Delete`) внутри воркера операции и приезжает в
// `Operation.result.error` как `FAILED_PRECONDITION` с перечнем видов и чисел
// и признаком `REFERENCE_IN_USE`. Синхронной полосы у него нет и не заводится:
// отказ по проектной роли уже живёт внутри `DELETE`, и второй канал доставки
// одного отказа одного глагола разошёлся бы с первым (приёмка §2.1).
//
// Что отказ НЕ гарантирует — сказано контрактом глагола: строка зеркала
// приходит к службе после коммита владельца, окно доставки существует у любого
// механизма, и удаление в этом окне проходит (`IAM-PNE-1-13`). Осиротевшие
// строки называет проход по зеркалу на старте (стадия S2 той же приёмки).
...
func (u *DeleteProjectUseCase) doDelete(ctx context.Context, id domain.ProjectID, actor, accountID string) (*anypb.Any, error) {
	if err := shared.DoWithWriteTxVoid(ctx, u.repo,
		func(ctx context.Context, w Writer) error {
			...
			if rerr := w.EmitReconcileEvent(ctx, shared.ReconcileEventDelete, "iam.project", string(id)); rerr != nil {
				return rerr
			}
