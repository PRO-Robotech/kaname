...
// update_label_rematerialize_test.go — a service-account label change must
...
//
//   - IAM-DIRECT (this path): ServiceAccount.Update only co-committed a
...
// TestUpdateServiceAccount_LabelChange_RematerializesObjectInProcess — clearing a
// label runs the object-forward pass for THIS service account, so the revoke does not
// queue behind the global reconcile backlog.
func TestUpdateServiceAccount_LabelChange_RematerializesObjectInProcess(t *testing.T) {
	repo := newLcsRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateServiceAccountUseCase(repo, newLcsOps()).WithObjectReconciler(rec, nil)
	ctx := operations.WithPrincipal(context.Background(),
		operations.Principal{Type: "user", ID: lcsOwnerID})

	_, err := uc.Execute(ctx, UpdateServiceAccountInput{
		ID:         lcsSaID,
	...
	want := "object_forward:iam.serviceAccount:" + lcsSaID
	calls := rec.awaitTrace(want, 5*time.Second)
	assert.Contains(t, calls, want,
		"a label change must re-materialize the service account in-process (the forward's "+
...
// TestUpdateServiceAccount_NonLabelChange_NoRematerialization — a name/description
// update cannot change selector membership, so it must not pay the O(scope) pass.
func TestUpdateServiceAccount_NonLabelChange_NoRematerialization(t *testing.T) {
	repo := newLcsRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateServiceAccountUseCase(repo, newLcsOps()).WithObjectReconciler(rec, nil)
	ctx := operations.WithPrincipal(context.Background(),
