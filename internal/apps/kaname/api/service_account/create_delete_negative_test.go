...
// path; the nil-embedded kanamerepo.Reader promotes the rest and fail-loud
// (nil-panic) if an unexpected method is ever called on this path.

type negDelRepo struct{ getErr error }

func (r *negDelRepo) Reader(context.Context) (kanamerepo.Reader, error) {
	return &negDelReader{getErr: r.getErr}, nil
}
func (r *negDelRepo) Writer(context.Context) (kanamerepo.Writer, error) { return nil, nil }
func (r *negDelRepo) Close()                                            {}

type negDelReader struct {
	kanamerepo.Reader // nil-embed: only ServiceAccounts + Rollback are used here
	getErr            error
}
