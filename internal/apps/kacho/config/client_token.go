...
type ClientTokenConfig struct {
	...
	//
	// Сверка запрошенного адресата идёт с ЭТИМ перечнем, а не с перечнем
	// клиента: колонки адресатов у клиентов, способных к утверждению, в схеме
	// нет, и требовать состояния, которого схема не допускает, нельзя.
