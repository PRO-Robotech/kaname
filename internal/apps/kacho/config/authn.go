...
// JWKSEncryptionKeyEnvName — имя переменной окружения, из которой берётся
// значение ручки ключа ОБЁРТКИ, когда она не задана напрямую.
//
// Объявлено ОДНИМ местом: имя переменной называют и сам резолв, и текст его
// отказа. Две копии разошлись бы молча — на той, которую забыли поправить, —
// и оператор искал бы не ту переменную.
