...
// свёртка одного журнала `kacho_iam.fga_outbox` — на этом стоит проекция
...
func inviteEmitted(t *testing.T) []service.RelationTuple {
	...
	// Вставка идёт на асинхронном продолжении — ждём ДЕТЕРМИНИРОВАННО, по
	// условию, а не паузой.
...
func TestInvite_UserAccountPointerIsCoCommittedToTheJournal(t *testing.T) {
	...
	require.NotEmpty(t, emitted,
		"транзакция приглашения не со-коммитила НИ ОДНОЙ строки журнала: указатель на "+
			"предка уходит мимо `kacho_iam.fga_outbox`, и проекция relation_fact его не увидит никогда")
