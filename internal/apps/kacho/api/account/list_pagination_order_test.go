...
func TestListPaginationFormatCheckedBeforeIdentityShortCircuit(t *testing.T) {
	...
	t.Run("анонимный вызывающий — отказ по формату курсора", func(t *testing.T) {
		uc := NewListAccountsUseCase(newAcctListFakeRepo()).WithRelationStore(newAcctFGAStub())

		_, _, err := uc.Execute(context.Background(), repoaccount.ListFilter{PageSize: 100, PageToken: garbageToken})
	...
	t.Run("анонимный вызывающий — отказ по page_size вне диапазона", func(t *testing.T) {
		...
		_, _, err := uc.Execute(context.Background(), repoaccount.ListFilter{PageSize: 1001})
	...
	t.Run("анонимный, законная страница — по-прежнему пустая страница без ошибки", func(t *testing.T) {
		...
		out, next, err := uc.Execute(context.Background(), repoaccount.ListFilter{PageSize: 100})
