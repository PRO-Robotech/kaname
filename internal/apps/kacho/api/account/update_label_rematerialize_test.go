...
// update_label_rematerialize_test.go — an account label change must re-materialize the
// object IMMEDIATELY, not only through the reconcile queue.
//
// iam.account is label-selectable (domain.labelSelectableTypes carries "iam.account";
// kacho_iam.accounts.labels is probed by `labels @> match_labels` in MatchIAMDirect),
// so removing a label that an ARM_LABELS grant matches is a REVOCATION: the per-object
// member row and its FGA tuples must go. Two feeds reach the same reconciler for that:
//
...
//
//   - IAM-DIRECT (this path): Account.Update only co-committed a
...
// TestUpdateAccount_LabelChange_RematerializesObjectInProcess — clearing a label runs
// the object-forward pass for THIS account, so the revoke does not queue behind the
// global reconcile backlog.
func TestUpdateAccount_LabelChange_RematerializesObjectInProcess(t *testing.T) {
	repo := newLcaRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateAccountUseCase(repo, newLcaOps()).WithObjectReconciler(rec)
	ctx := operations.WithPrincipal(context.Background(),
		operations.Principal{Type: "user", ID: lcaOwnerID})

	_, err := uc.Execute(ctx, UpdateAccountInput{
		ID:         lcaAcctID,
	...
	want := "object_forward:iam.account:" + lcaAcctID
	calls := rec.awaitTrace(want, 5*time.Second)
	assert.Contains(t, calls, want,
		"a label change must re-materialize the account in-process (the forward's delete-stale "+
...
// TestUpdateAccount_NonLabelChange_NoRematerialization — a name/description update
// cannot change selector membership, so it must not pay the O(scope) pass.
func TestUpdateAccount_NonLabelChange_NoRematerialization(t *testing.T) {
	repo := newLcaRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateAccountUseCase(repo, newLcaOps()).WithObjectReconciler(rec)
	ctx := operations.WithPrincipal(context.Background(),
