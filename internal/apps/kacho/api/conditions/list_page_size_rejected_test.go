...
func TestCondListRejectsOutOfRangePageSizeOnTheRawRequest(t *testing.T) {
	t.Run("отрицательный page_size — InvalidArgument", func(t *testing.T) {
		err := pageProbecallNoPanic(t, func() error {
			_, e := (&Handler{}).List(context.Background(), &iamv1.ListConditionsRequest{PageSize: -1})
			return e
		})
	...
	t.Run("page_size выше предела — InvalidArgument", func(t *testing.T) {
		err := pageProbecallNoPanic(t, func() error {
			_, e := (&Handler{}).List(context.Background(), &iamv1.ListConditionsRequest{PageSize: 1001})
			return e
		})
		require.Error(t, err)
		assert.Equal(t, codes.InvalidArgument, status.Code(err))
	...
	t.Run("граница диапазона проходит формат-гейт", func(t *testing.T) {
		err := pageProbecallTolerating(func() error {
			_, e := (&Handler{}).List(context.Background(), &iamv1.ListConditionsRequest{PageSize: 1000})
			return e
		})
	...
	t.Run("законная страница проходит формат-гейт", func(t *testing.T) {
		err := pageProbecallTolerating(func() error {
			_, e := (&Handler{}).List(context.Background(), &iamv1.ListConditionsRequest{PageSize: 100})
			return e
		})
		pageProbeassertNotRejectedByPageFormat(t, err)
	})
