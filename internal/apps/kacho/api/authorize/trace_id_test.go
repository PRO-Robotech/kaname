...
// TestAuthorizeCheck_TraceIdIsBounded — предел 64 ОБЪЯВЛЕН в контракте, но никем не
// энфорсится: protovalidate в этом дереве не подключён, `(length)` — декорация.
// Значит идентификатор приходит от вызывающего произвольной длины, а лог обязан
// оставаться логом. Обрезка — в коде, а не в надежде на объявление.
