...
func TestCreateAuthority_IsTheParentWriteTier_NotAPerObjectVerb(t *testing.T) {
	...
	// И ровно то отношение, которое сняли: его в хранилище нет ни у кого и нигде.
	// (`check` считает неопределённое отношение отказом, поэтому это утверждение
	// сильно ровно настолько, насколько сильна предпосылка выше — она и несёт вес.)
	require.False(t, c.check(t, subj, "v_create", homeProject),
		"на проекте резолвится `v_create` — тип его не объявляет, кортежа никто не писал")
...
func TestCreateAuthority_RegistryNamespaceKeepsItsReader(t *testing.T) {
	...
	// существует. Иначе «оставили одному реестру» было бы неотличимо от «оставили
	// всем» — проба выше зеленела бы и в том, и в другом случае.
	require.False(t, c.check(t, owner, "v_create", "vpc_network:net_ca000000000000"),
		"`v_create` резолвится на обычном ресурсном типе — отношение вернулось туда, где его "+
			"никто не спрашивает")
}
