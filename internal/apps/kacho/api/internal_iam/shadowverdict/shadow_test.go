...
// TestCompare_DisabledWritesNothingToTheJournal — kacho#763, вторая половина
// пункта «выключенное сравнение НАБЛЮДАЕМО выключено».
//
// # Что здесь утверждается сверх соседней пробы
//
// `TestCompare_DisabledIsANoOp` утверждает первую половину — счётчики нулевые.
// Вторая половина того же пункта предиката — «записей нет», и она держалась
// ПОСТРОЕНИЕМ: замыкание стоит в каждой точке входа отдельно. Построение —
// не проверка: оно верно ровно до первой правки и молчит, когда перестаёт быть
// верным.
//
// Различие не косметическое. Сравнитель, который перестал считать, но
// продолжает писать, платит за фазу, от которой отказались, и заполняет журнал
// службы решениями, которых не принимал: оператор, выключивший сверку ручкой,
// увидел бы её следы и заключил, что ручка не сработала.
//
// # Почему в паре с положительным контролем
//
// «Записей нет» зеленеет и на сравнителе, который не пишет НИКОГДА, — то есть
// ровно на том дефекте, ради которого сверка и заведена: «расхождений ноль»
// становится неотличимо от «сравнений ноль». Поэтому те же вызовы идут вторым
// заходом по ВКЛЮЧЁННОМУ сравнителю, и он обязан написать.
//
// # Почему перечислены все точки входа, а не одна
//
// Замыкание стоит в КАЖДОЙ по отдельности, общего места у них нет. Проба,
// спросившая одну, оставила бы остальные без утверждения — и первая же новая
// точка входа приехала бы незамкнутой, не покраснив ничего. Инъекцией проверены
// все четыре: снятие стража в `Summarise` и в `Unaskable`, запись в журнал до
// стража в `Ask` и в `askSet` — каждая по отдельности красит эту пробу.
func TestCompare_DisabledWritesNothingToTheJournal(t *testing.T) {
	// exercise — все точки входа сравнителя, доходящие до журнала. Порог
	// наблюдения опущен до Debug: «выключено» означает тишину на любом уровне,
	// а не «ниже предупреждения не пишем».
	exercise := func(c *Comparator) {
		ctx := context.Background()
		c.Ask(ctx, "user:usr-1", "vpc_network", "net-1", "v_get", nil)(true, true)
		c.AskObjects(ctx, "user:usr-1", "vpc_network", []string{"v_get"})(
			[]string{"net-1"}, true, true)
		c.AskSubjects(ctx, "vpc_network", "net-1", "v_get")(
			[]string{"user:usr-1"}, true, true)
		c.AskSources(ctx, "vpc_network", "net-1", "v_get")(
			[]string{"user:usr-1"}, true, true)
		c.Unaskable("объект не разобран", "vpc_network", "v_get")
		settled(c).Summarise()
	}

	capture := func() (*bytes.Buffer, *slog.Logger) {
		var buf bytes.Buffer
		return &buf, slog.New(slog.NewTextHandler(&buf,
			&slog.HandlerOptions{Level: slog.LevelDebug}))
	}

	off, offLogger := capture()
	disabled := New(nil, offLogger)
	exercise(disabled)
	if got := logOf(disabled, off); got != "" {
		t.Fatalf("выключенное сравнение написало в журнал:\n%s\n"+
			"«выключено» обязано быть отличимо от «работает тихо»: оператор, снявший "+
			"сверку ручкой, увидел бы её следы и заключил, что ручка не сработала", got)
	}

	// ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ. Форма отвечает иначе, чем движок, — значит
	// включённому сравнителю ЕСТЬ что написать. Без этого захода утверждение
	// выше верно и для сравнителя, который молчит всегда.
	on, onLogger := capture()
	enabled := New(&stubAsker{allow: false, set: []string{"net-9"}, complete: true}, onLogger)
	exercise(enabled)
	if logOf(enabled, on) == "" {
		t.Fatal("включённое сравнение не написало НИЧЕГО — контроль провален, и утверждение " +
			"о тишине выключенного ничего не значит: оно зеленеет и на сравнителе, который " +
			"не пишет никогда")
	}
}
