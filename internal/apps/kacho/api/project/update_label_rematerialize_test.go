...
// update_label_rematerialize_test.go — a project label change must re-materialize the
// object IMMEDIATELY, not only through the reconcile queue.
//
// iam.project is label-selectable, so removing a label that an ARM_LABELS grant
// matches is a REVOCATION: the per-object member row and its FGA tuples must go. Two
// feeds reach the same reconciler for that:
//
...
//     grant is revoked there and then. That is why label-revoke-vpc is green.
//
//   - IAM-DIRECT (this path): Project.Update only co-committed a
...
//     QUEUE. The queue is strictly FIFO (ORDER BY id ASC) and drained by one worker at
//     roughly five events/second, each event costing a FULL O(scope) recompute; the
//     e2e suite emits five to eight events/second, so the queue runs a multi-minute
//     backlog. Measured on the stand: the label-clear event was enqueued at 19:59:18.97
//     and marked drained at 20:06:49.82 — 7m30s — and the tuple only died at 20:00:23
//     when the 30s periodic sweep happened to reach that binding, 65s after the clear.
//     The black-box probe had long given up and still read `allowed: true`.
//
...
// TestUpdateProject_LabelChange_RematerializesObjectInProcess — clearing a label runs
// the object-forward pass for THIS project, so the revoke does not queue behind the
// global reconcile backlog.
func TestUpdateProject_LabelChange_RematerializesObjectInProcess(t *testing.T) {
	repo := newLcpRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateProjectUseCase(repo, newLcpOps()).WithObjectReconciler(rec)
	ctx := operations.WithPrincipal(context.Background(),
		operations.Principal{Type: "user", ID: lcpOwnerID})

	_, err := uc.Execute(ctx, UpdateProjectInput{
		ID:         lcpProjID,
	...
	want := "object_forward:iam.project:" + lcpProjID
	calls := rec.awaitTrace(want, 5*time.Second)
	assert.Contains(t, calls, want,
		"a label change must re-materialize the project in-process (the forward's delete-stale "+
...
// TestUpdateProject_NonLabelChange_NoRematerialization — a name/description update
// cannot change selector membership, so it must not pay the O(scope) pass.
func TestUpdateProject_NonLabelChange_NoRematerialization(t *testing.T) {
	repo := newLcpRepo(domain.Labels{"labelrevoke": "treska"})
	rec := &recordingObjectReconciler{}
	uc := NewUpdateProjectUseCase(repo, newLcpOps()).WithObjectReconciler(rec)
	ctx := operations.WithPrincipal(context.Background(),
