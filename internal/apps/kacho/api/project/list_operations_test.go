...
// list_operations_test.go — unit test for ProjectService.ListOperations.
//
// Verifies the fix for the no-op handler: ListOperations must return the
// operations recorded for the given project id, and reject a malformed id.
...
func TestProject_ListOperations_ReturnsRecordedOps(t *testing.T) {
	repo := &fakeOpsList{ops: []operations.Operation{
		{ID: "iop00000000000000001", Description: "Create project x", CreatedAt: time.Unix(1, 0), Principal: opsCaller},
	}, next: "tok"}
	h := project.NewHandler(nil, nil, nil, nil, nil).
		WithListOperations(shared.NewListOperationsUseCase(repo))

	resp, err := h.ListOperations(opsCallerCtx(),
		&iamv1.ListProjectOperationsRequest{ProjectId: "prj00000000000000001"})
	if err != nil {
...
func TestProject_ListOperations_MalformedID_InvalidArgument(t *testing.T) {
	h := project.NewHandler(nil, nil, nil, nil, nil).
		WithListOperations(shared.NewListOperationsUseCase(&fakeOpsList{}))

	_, err := h.ListOperations(context.Background(),
		&iamv1.ListProjectOperationsRequest{ProjectId: "not-a-project-id"})
	if got := grpcstatus.Code(err); got != codes.InvalidArgument {
		t.Fatalf("malformed project id must be InvalidArgument, got %s", got)
	}
}
