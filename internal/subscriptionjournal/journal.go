...
//
// Три подтаблицы состава (`group_members`, `memberships`,
// `access_binding_subjects`) эмитят правку владельца; три подтаблицы роли
// (`role_verb`, `role_rule_ref`, `role_rule_selectors`) не эмитят ничего. Это
// РЕШЕНИЕ, а не пропуск, и держится оно на двух вещах сразу.
//
// ПЕРВОЕ: правка состава роли наблюдаема ЧЕРЕЗ СТРОКУ РОЛИ. Вызовов трёх
// `Replace*` в прод-коде девять, и каждый адъюдицирован. Создание роли — три
// замены в той же транзакции, что вставка строки. Правка — три замены под
// условием «маска назвала правила», а оператор под сторожем версии эти самые
// правила в строку и пишет. Приведение системной роли — замена под условием
// «поля изменились либо роль ожила», и строку роли пишет каждое из двух.
// Оставшиеся два вызова — досев старта, и он не меняет НИЧЕГО.
//
// ВТОРОЕ: цена обратного исхода измерена, а не предположена. Подъём, в котором
// не изменилось ничего, кладёт в три подтаблицы 1558 строковых событий — полная
// замена проекции (661+661 глаголов, 92+92 сегментов) плюс 52 правки
// селекторов, — и все они про 48 ролей, то есть 32 дубля на роль. Сужение
// `OLD.* IS DISTINCT FROM NEW.*`, которым сужены семь основных таблиц, отсекает
// из них НОЛЬ: над двумя таблицами оператора `UPDATE` не существует вовсе
// (полная замена даёт DELETE+INSERT), а досев селекторов двигает
// `updated_at = now()`, поэтому предикат существа на нём истинен всегда. То
// есть сужение не «мало отсекает» — оно неприменимо by construction.
//
// Числа — ориентир ревизии, и по памяти они здесь не переписываются: у них есть
// ПРОИЗВОДИТЕЛЬ — `internal/migrations/role_subtable_boot_volume_integration_test.go`.
// Он же держит пару, на которой решение стоит (правка строки роли событие даёт,
// холостой подъём — не даёт), а цену обратного исхода меряет инъекция рядом:
// те же триггеры, заведённые на три подтаблицы, дают 1558 событий журнала за
// один холостой подъём.
//
// ОСТАТОК БЫЛ НАЗВАН И ЗАКРЫТ (`PRO-Robotech/kaname#76`), а решение выше
// осталось прежним — и это не противоречие.
//
// Остаток: путь последствий каталога отбирает у роли глагол либо сегмент,
// строки роли не трогая, — такой отзыв подписчику виден не был. Закрыт он НЕ
// триггером на подтаблицах: триггер вернул бы ровно те 1558 холостых событий,
// которыми решение и обосновано, потому что «снятие последствием» и «полная
// замена при подъёме» на уровне строки суть один и тот же `DELETE`. Закрыт
// ЯВНЫМ объявлением из единственного пути, который пишет подтаблицу роли в обход
// её строки: `internal/repo/kaname/pg/catalog_consequence_sql.go`,
// `announceRoleGrantWithdrawalSQL`; применитель зовёт его шагом 8а, в той же
// транзакции, что снятие, — откат уносит объявление вместе с отобранным.
//
// Довод миграции «триггер, а не вызов в каждом месте записи» этим не нарушен:
// он считал ПИСАТЕЛЕЙ семи основных таблиц, их полсотни. Здесь писатель ОДИН, и
// он единственный в продукте.
//
// Что держит обе половины порознь: свойство СХЕМЫ (голая запись подтаблицы
// события не даёт) — §3 той же пробы объёма; НАБЛЮДАЕМОСТЬ отзыва —
// `internal/repo/kaname/pg/role_grant_withdrawal_reaches_the_subscriber_integration_test.go`.
