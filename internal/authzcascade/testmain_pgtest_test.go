...
// per test.
//
// kachopg.NewTestPostgres and fgatest.New — which the page-cost measurement here
// calls together — each used to boot a container and, for Postgres, replay the
// whole iam migration chain per call. Both now hand back an isolated slice of the
// one container this test BINARY owns: a database cloned from the migrated
// template, and a store on the shared relation server. The containers belong to
// the binary, so the wiring has to live in the package that owns the binary.
//
