...
//     границе 3.
//  6. ЭКРАНИРОВАНИЕ НА КРАЮ ДИАПАЗОНА судится как у базы (сверено с
...
// sqlCodePointMax — верх диапазона кодовой точки в экранировании `\u`, `\U` и
// U&: значение выше и ноль база отвергает («invalid Unicode escape value») —
// здесь такое экранирование остаётся в прочтении как написано (граница 6).
// Проверка стоит ПЕРЕД rune(v): она и есть довод, что знаковая руна не
// переполнится, — gosec G115 читает его сам, пометки не нужно.
const sqlCodePointMax = 0x10FFFF
...
func sqlUnbackslash(s string) string {
	...
	for i := 0; i < len(s); i++ {
		...
		case c >= '0' && c <= '7':
			...
			// База берёт МЛАДШИЙ байт (strtoul → unsigned char): \563 → 's',
			// \777 → 0xFF, \400 → 0x00. Усечение — её лексика, и повторяется
			// здесь явно: отказ раскрытия дал бы пропуск имени, собранного
			// через \ooo выше \377. Годность байта в кодировке — не предмет
			// распознавателя, как и у \xhh.
