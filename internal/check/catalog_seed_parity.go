...
// catalog_seed_parity.go — ЯДРО гейтов посева каталога модуля, отделённое от корня
...
//
// Предмет — ПАРИТЕТ: строки, которые сеет миграция каталога, обязаны быть
...
//
// # Гейта ФОРМЫ КЛЮЧА здесь больше нет — он судит действующую схему (kaname#278)
//
// Здесь жил второй гейт того же предмета — `TestIAMCT113_CatalogKeysCarryTheDeclaredForm`
// (`RESTRICT` рядом с `DEFERRABLE` и немедленность ключей проекции правила). Его
// ядро читало текст ОДНОЙ базовой миграции и потому не видело ни ключей поздних
// миграций, ни неявного снятия: `DROP COLUMN` уносит ключ без единой строки о нём,
// а запись ведомости держалась за имя в тексте, который не правят (запрет #5), и
// не истекла бы никогда. Гейт переведён на `pg_catalog` после наката всей цепочки
// и живёт там, где такая база у модуля уже есть: `internal/repo/kaname/pg`,
// `catalog_key_form_test.go`. Посев остался здесь: его предмет — строки, а не
// объекты схемы, и сверка с литералом по-прежнему идёт разбором текста цепочки.
...
// сказанное в объяснении, оператором не является, и без этой строки гейт
