...
// личных данных (`security-hardening.md` п. 2, задача `kacho#2483`).
//
// # Предмет
//
...
//
// `external_id` попадает в перечень СОЗНАТЕЛЬНО, и это расходится с
// `security-hardening.md` п. 2, где он назван не-личным идентификатором
// корреляции. Расхождение разрешается ЕДИНИЦЕЙ: там речь о СТРОКЕ ЖУРНАЛА
...
// он МОЛЧИТ, и записанное в этой форме оказывается вне наблюдения
// (`testing.md` §«Гейт на класс», п. 7). Формы измерены по дереву, а не
// придуманы:
//
