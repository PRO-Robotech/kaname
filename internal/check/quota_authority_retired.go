...
// ВЕДОМОСТЬ ПУСТА, И ЭТО ЕЁ ЦЕЛЬ, А НЕ ЗАБЫТОЕ ОБЪЯВЛЕНИЕ.
//
// Единственная её запись прощала `quota_reader` — право читать пределы, чьи
// глаголы сняла стадия S4. Запись несла свой предикат снятия («объявления нет
// ни в одной копии модели»), он выполнен (`kaname#59`), и запись снята ТЕМ ЖЕ
// изменением, каким ушло объявление: послабление, пережившее предмет, прикрыло
// бы следующую находку.
//
// Пустая ведомость ось НЕ гасит: сверка идёт в обе стороны, и всякое отношение
// авторитета, заведённое в модели заново, станет находкой сразу — прощать его
// теперь нечем. Способность оси падать и молчать на пустой ведомости доказана
// инъекцией (`quota_authority_retired_injection_test.go`).
