...
// (`testing.md` §«Гейт на класс», п. 9). Отличить это от исправной работы можно
// только переписью ОБЪЁМА: «находок ноль» и «прочитано ноль» обязаны быть
// разными вердиктами, и оба напечатаны.
//
...
// стоит в этом же файле, а надгробие снятого метода — в `authzguard`. Тот же
// класс, что «гейт читает исполняемую часть, а не текст» (`testing.md` §«Гейт на
// класс», п. 4), только применённый к отсутствию.
//
...
// неё не подпадают by construction — и это ровно то, что требует
// `security-disclosure.md`: снятое называется в отрицательной форме, а не
// стирается молча.
//
...
// близнецом в инъекции: без второй половины «находок нет» было бы неотличимо от
// «разбор не смотрит».
package check
...
func (c AuthorityResidueCensus) String() string {
	return fmt.Sprintf(
		"перепись: прод-файлов Go разобрано %d (узлов-имён %d), контрактов прочитано %d "+
			...
			"не разобрано %d",
		c.GoFiles, c.GoIdents, c.Contracts, c.ContractDecls, c.Models, c.Kept, len(c.Unparsed))
}
...
func JudgeAuthorityResidue(corpus map[string]string) (AuthorityResidueCensus, []AuthorityResidueFinding) {
	var (
