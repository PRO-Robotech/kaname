...
//
// Форма ВЗЯТА, а не придумана здесь: `owner/name:` у координаты ввёл
// `PRO-Robotech/kacho-workspace` `scripts/docs-gate/check-03-holding-claim-resolves.py`
// по тому же поводу — вынос службы оставил его координаты нерезолвящимися в
// стволе продукта. Оттуда же довод в пользу двоеточия: спутать `owner/name:путь`
// с git-формой `<ревизия>:<путь>` нельзя, потому что слева требуется слэш,
// которого у ревизии не бывает. Отличие одно и вынужденное: там дом стоит у ПУТИ,
// здесь — у ИМЕНИ, потому что координата ЭТОГО гейта и есть имя.
//
...
//
// РЕЗОЛВ В НАЗВАННОМ ДОМЕ ТЕПЕРЬ ДЕЛАЕТСЯ — `probe_home.go` плюс `JudgeHomes`
// у вызывающего. Ядро по-прежнему судит ЗНАЧЕНИЯ и чужих деревьев не читает:
// оно собирает координаты чужих домов в `ForeignCoordinates`, а резолвит их тот,
// у кого есть корень. Ниже — прежний разбор, объясняющий, почему ветвь нельзя
// было завести БЕЗ производителя входа; он остаётся верным и описывает, чем
// производитель стал.
//
// ПРЕЖНИЙ ОСТАТОК (закрыт `kaname#44`): резолв в названном доме здесь НЕ делался, и это остаток с предметом, а не
// граница по незнанию: воркспейс его умеет (опознаёт дерево идентичностью
// `origin`, отвечает третьей категорией, когда дерева рядом нет), но в конвейере
// службы чужого дерева нет НИКОГДА — ветвь была бы вечно-пустой, то есть мёртвым
// стражем ровно того вида, который запрещает ban #16. Заводится вместе с
// резолвером дома и инъекцией на двух синтетических деревьях — `kaname#44`.
//
...
//     уходит молча. Связанные РЕВИЗИЕЙ считаются отдельно: они не проверяемы даже
//     там, где дерево дома есть;
...
//  3. ДВА СТОРОЖА, и живут они в гейте (`acceptance_probe_coordinate_test.go`), а
//     не здесь, потому что им нужен дом ЭТОГО дерева — ядро судит значения, и для
//     него всякий непустой дом чужой. Первый отвергает корпус, где ВСЕ координаты
//     объявлены чужими: судить нечего, и «находок ноль» означало бы «ноль
//     прочитанного». Второй ловит СВОЙ дом, названный чужим
//     (`PRO-Robotech/kaname:TestFoo`), — иначе всякая мёртвая координата
//     закрывается припиской своего же имени репозитория. Дом дерева выводится из
//     `go.mod` (`OwnHomeOfTree`), а не складывается литералом.
//
	...
	for _, p := range paths {
		for _, co := range ProbeCoordinatesIn(p, docs[p]) {
			...
			if co.Home != "" {
				...
				homes[co.Home] = true
				c.ForeignCoordinates = append(c.ForeignCoordinates, co)
				continue
			}
