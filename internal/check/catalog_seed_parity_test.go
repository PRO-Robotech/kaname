...
// catalogRepoRoot — корень ОБХОДА: дерево платформы в монорепо, корень модуля в
// самостоятельном клоне.
//
// Своей копии подъёма здесь БОЛЬШЕ НЕТ, и это не косметика. Копия брала самый
// внешний `go.mod` на пути вверх — верно для «не первый встречный» (у службы
// свой модуль, и остановка в её каталоге удваивала бы сегмент `services/iam`),
// и неверно в другую сторону: подъём шёл до корня файловой системы и пересекал
// границу судимого дерева МОЛЧА. Рабочую копию заводят внутри другого checkout'а
// продукта — тогда корнем объявлялся ОБЪЕМЛЮЩИЙ каталог, координаты обхода
// получали лишнюю приставку и не совпадали ни с одним ключом ведомости. Гейт
// краснел дважды на исправном дереве: «отброшено вне ведомости» на всём и
// «нечего исключать» по каждой записи (kacho#2429).
//
// Резолв, держащий границу, в модуле уже есть — `treeposture.CorpusRoot`: он
// поднимается к БЛИЖАЙШЕМУ `go.mod` (корню модуля), а дерево платформы опознаёт
// СТРУКТУРНО — по тому, что модуль лежит в каталоге модулей платформы, — и
// поэтому за границу не выходит ни при какой вложенности. Его собственные
// инъекции это и доказывают (`platformtree_gate_test.go`, в т.ч. «чужой каталог
// модулей выше по дереву»).
func catalogRepoRoot(t *testing.T) string {
	t.Helper()
	root, err := catalogRepoRootFrom(wdOf(t))
	if err != nil {
...
// catalogRepoRootFrom — ЯДРО резолва, вынесенное отдельно, чтобы инъекция гоняла
// то же самое, что гейты по дереву. Проба, повторяющая резолв своей копией,
// доказывала бы свойство копии.
func catalogRepoRootFrom(start string) (string, error) {
	root, _, err := treeposture.CorpusRoot(start)
	return root, err
}
