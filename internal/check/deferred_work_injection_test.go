...
// библиотеке.
//
// ЧЕМ СВОЙСТВО ДЕРЖИТСЯ, СМЕНИЛОСЬ, А САМА ПРОБА — НЕТ. Прежде рядом стоял
// отдельный вычет этого имени; он не перенесён вместе с разбором в фундамент,
// потому что ветвь недостижима — вызов через точку от обращения к читателю кода
// отличает класс границы в самом образце. Проба остаётся, и предмет у неё тот
// же: она судит ИСХОД на кроне этого дерева, а не ту ветвь, которой больше нет.
// Ослабят класс границы — покраснеет она и её близнец в фундаменте,
