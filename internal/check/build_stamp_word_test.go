...
//
// Служба доступа — отдельный Go-модуль, и от модуля платформы он НЕ ЗАВИСИТ
// вовсе: ступень S0a (kacho#2617, исход C) сняла ребро `kaname → kacho` целиком.
// Прежняя редакция этого абзаца обосновывала два объявления пином
// (`go.mod`: `github.com/PRO-Robotech/kacho v0.1.0`) — довод от бампа пина
// перестал быть верным вместе с пином, а вывод стал СИЛЬНЕЕ: общей константы у
// них не может быть даже теоретически, потому что общего модуля больше нет ни
// одного. Значит объявления два by construction, а держит их согласие этот гейт.
//
