...
// в файле законно; `Subject` — предмет послабления и предикат его снятия.
//
// Перечень закрыт и самоистекает в обе стороны.
var failureResetLedger = map[string]check.FailureResetLedgerEntry{
	...
	// расхождение приёмка, а не реализация:
	//
	...
	//
	// Предикат снятия: обе приёмки говорят о восстановлении одно.
	"internal/apps/kaname/api/humansession/recovery_complete.go": {
		...
		Subject: "восстановление доступа: `recovery-of-access.md` Р5 и `login-lane-…md` Р10 ред. 11 расходятся " +
			"об уровне восстановившего; снимается, когда обе приёмки скажут одно",
	},
	...
	// нет, а «не обнулять» — безопасная сторона. Запись стоит здесь потому, что
	...
	"internal/apps/kaname/api/registration/register.go": {
		...
		Subject: "регистрация: сессия выдаётся впервые заведённому адресу, счёта по нему нет; " +
			"снимается, если регистрация начнёт выдавать сессию уже существующему адресу",
	},
}
