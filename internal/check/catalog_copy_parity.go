...
//
// ─────────────────────────────────────────────────────────────────────────────
// ДВЕ ЗАПИСИ ОКНА — ФОРМА ОПЕРАЦИИ (PRO-Robotech/kacho#2601)
//
// Класс предсказан абзацем в шапке файла и наступил третьим переименованием в
// фундаменте: `corelib@v1.8.0` назвал форму операции корнем фундамента
// (`kacho.cloud.operation` → `corelib.operation`). Служба линкует новую версию и
// СЛУЖИТ глаголы под новым полным именем; копия края на стволе платформы до её
// подъёма пинов называет прежнее. Перепись окна (копия края — ствол платформы
// `41c75cb678`, наша — перерождённая):
//
//	записей у каждой стороны   338
//	расходится                   2  — `OperationService/Get` и `/Cancel`, только
//	                                  сегмент пакета в `fqn`; остальные поля
//	                                  записи совпадают дословно
//
// Направление — отстала копия края, синхронизация ЗАПРЕЩЕНА (довод выше).
// Предикат снятия у обеих ВНЕШНИЙ и один: платформа подняла пины фундамента и
// службы и перегенерировала каталог — копия края называет оба глагола именем
// фундамента. Тогда сверка назовёт обе записи потерявшими предмет, и снимает их
// изменение, следующее за вливанием платформы.
var catalogFoundationRenames = []CatalogFoundationRename{
	{
		EdgeFQN: "kacho.cloud.operation.OperationService/Get",
		OwnFQN:  "corelib.operation.OperationService/Get",
		Why: "форма операции названа корнем фундамента в corelib@v1.8.0; служба линкует его и " +
			"служит глагол под новым полным именем, край на стволе платформы — ещё под прежним",
		Removal: "копия края называет глагол corelib.operation.OperationService/Get: платформа " +
			"подняла пины фундамента и службы и перегенерировала каталог",
		Refs: "PRO-Robotech/kacho#2601 · PRO-Robotech/corelib#8",
	},
	{
		EdgeFQN: "kacho.cloud.operation.OperationService/Cancel",
		OwnFQN:  "corelib.operation.OperationService/Cancel",
		Why: "форма операции названа корнем фундамента в corelib@v1.8.0; служба линкует его и " +
			"служит глагол под новым полным именем, край на стволе платформы — ещё под прежним",
		Removal: "копия края называет глагол corelib.operation.OperationService/Cancel: платформа " +
			"подняла пины фундамента и службы и перегенерировала каталог",
		Refs: "PRO-Robotech/kacho#2601 · PRO-Robotech/corelib#8",
	},
}
