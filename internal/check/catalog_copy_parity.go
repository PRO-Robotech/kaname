...
//	платформа на своём стволе пинит `corelib@v1.5.0`):
...
// приходит как `/corelib.subscription.…/Subscribe`. Записав сюда имя края, мы
// положили бы в посев имя метода, которого этот двоичный файл НЕ СЛУЖИТ, —
// мёртвая запись, а у служимого глагола записи бы не стало.
//
...
// другое: величина названа замером (одна запись из 338), предмет заведён
// (`Refs`), снятие наступает от ВНЕШНЕГО факта, а не от чьей-то памяти.
// Самоистечение держит `CompareCatalogCopies`: запись, чьей стороне в дереве
// больше нечего исключать, — находка.
var catalogFoundationRenames = []CatalogFoundationRename{
	{
		EdgeFQN: "kacho.cloud.subscription.InternalSubscriptionService/Subscribe",
		OwnFQN:  "corelib.subscription.InternalSubscriptionService/Subscribe",
		Why: "форма подписки переименована выпуском фундамента corelib v1.7.0 " +
			"(имя платформы у контракта, живущего в библиотеке, утверждало владение, " +
			"которого нет). Служба линкует v1.7.0 и служит глагол под новым именем; " +
			"платформа на своём стволе пинит v1.5.0 и служит под старым. Обе копии " +
			"верны каждая для своего пина; переезд контракта НЕДЕЛИМ по пакету, " +
			"поэтому промежуточного состояния, где совпали бы обе, не существует.",
		Removal: "копия края называет глагол `corelib.subscription…` — то есть платформа " +
			"подняла пин фундамента до v1.7.0 и перегенерировала каталог. Тогда эта " +
			"запись перестаёт что-либо исключать, и сверка краснеет на ней самой.",
		Refs: "PRO-Robotech/kacho#2601 · PRO-Robotech/corelib#7 · kaname#79",
	},
}
