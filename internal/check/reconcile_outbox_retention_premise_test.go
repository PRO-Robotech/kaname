...
//
// У очереди сверки ключа партиции НЕТ и не требуется: её поток коммутативен, и
// почему — сказано ОДНИМ экземпляром в росписи commutativeDrainExempt
// (`internal/check/drain_order_declared_test.go`), а не здесь. Поэтому её
// уборщик проще — он снимает доставленную строку по возрасту и ничего не щадит.
//
