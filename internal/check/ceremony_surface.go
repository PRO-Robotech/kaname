...
//     switch, выбор из карты, функция сопоставления над путём и над
//     значениями, в которые он течёт; куда выведение доходит и где кончается,
//     названо в ceremony_surface_manual.go), и мультиплексор, отданный чужому
//     коду, гейт не моделирует — и потому краснеет на них, а не молчит.
//   - Значения, прошедшие через пустой интерфейс, не прослеживаются. Вызов
//     метода регистрации через интерфейс, который реализует мультиплексор,
//     когда хоть у одного значения получателя реализация не найдена, —
//     находка «не прослежен», даже если другие получатели разрешились;
//     мультиплексор, переданный аргументом методу интерфейса с таким
//     получателем, — находка «передан методу интерфейса». Ни то ни другое не
//     молчание.
//   - Регистрация, которую разбор видит, стоит в переписи: на мультиплексоре
//     своего вида, в непрослеженных или в недостижимом коде. Вызов через
//     интерфейс, разрешившийся лишь у части получателей, стоит в двух графах:
//     на мультиплексорах разрешившихся и в непрослеженных.
...
// недособранному разбору. Сколько раундов ушло на самом деле, печатает
// перепись (SolveRounds, «раундов разбора N из предела»): число здесь не
// выписано, чтобы не пережить свой предмет.
...
// исчерпал бюджет шагов): это не зелёное и не находка. ctx ограничивает сбор
// радиуса (`go list`) вместе с его сроком.
