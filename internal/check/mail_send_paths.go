...
// РОВНО ОДИН, и отправляет он ТОЛЬКО наши виды письма.
//
// # Предмет
//
// Видов письма в продукте три (подтверждение адреса, восстановление доступа,
// приглашение), а производители у них РАЗНЫЕ — решение Р23 приёмки ID-MAIL-1.
// Приглашение отправляем МЫ: его предмет — строка в НАШЕЙ базе, и поставщику о
// ней не известно ничего. Восстановление доступа — НАШЕ с фазы Ф5 (одобренная
// `docs/engineering/acceptance/recovery-of-access.md`, Р3 и Д5): код чеканим мы,
// поток поставщика истёк вместе с поставщиком, и без переезда у письма не
// осталось бы ни одного отправителя — ровно тот признак, который Р23 называет.
// Подтверждение адреса остаётся за почтовым процессом поставщика до своей фазы
// (Ф6): его предъявитель принадлежит ему, и составляет письмо он.
//
...
// Виды письма, чей производитель — наш код.
const (
	// MailKindInvite — приглашение (ID-MAIL-1 Р23).
	MailKindInvite = "invite"
	// MailKindRecovery — восстановление доступа (Ф5 Р3, Д5 — переезд из полосы
	// поставщика; `kacho#1271`).
	MailKindRecovery = "recovery"
)
...
var OurMailKinds = []string{MailKindInvite, MailKindRecovery}

// mailKindIsOurs — предикат перечня.
func mailKindIsOurs(kind string) bool {
	for _, k := range OurMailKinds {
		if k == kind {
			return true
		}
	}
...
func AdjudicateMailSendPaths(paths []MailSendPath, kinds []MailKindSite) []MailSendFinding {
		...
		for _, k := range p.Kinds {
			...
			out = append(out, MailSendFinding{
				...
				Why: "Решение Р23 приёмки ID-MAIL-1 отдаёт этот вид почтовому процессу " +
					...
					"Переезд вида к нам объявляет одобренная приёмка (как Ф5 для восстановления), " +
					"а не правка перечня `OurMailKinds`.",
			})
		}
	}
	...
	// §12 п. 3а приёмки, и это находка, а не молчание. Судится КАЖДЫЙ наш вид:
	// путь у них один, а обещаний два.
	for _, ours := range OurMailKinds {
		if _, ok := byKind[ours]; ok {
			continue
		}
		out = append(out, MailSendFinding{
			Axis: "transport",
			What: "путей отправки вида `" + ours + "` — ноль",
			Why: "Вид отправляет НАШ код (ID-MAIL-1 Р23 — приглашение; Ф5 Р3 — восстановление): " +
				"его предмет — строка в нашей базе. Ноль путей означает обещание без " +
				"производителя: ресурс создаётся успешно, письма нет.",
		})
	}

	// Ось 2. Словарь видов. Вид вне нашего перечня — находка где угодно.
	for _, k := range kinds {
		if mailKindIsOurs(k.Kind) {
			continue
		}
		out = append(out, MailSendFinding{
