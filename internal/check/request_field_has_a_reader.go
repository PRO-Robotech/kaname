...
// # Предмет
//
// Поле запроса, которое принимают и не читают, — «принято-и-проигнорировано»
// (`api-conventions.md`), и оно тише отказа: вызывающий получает УСПЕХ и уверен,
// что его параметр применён. Продукт обещает возможность, которой нет.
// Обнаруживается это не отладкой, а последствиями — в чужой.
//
// Конвенция объявляет гейт этого класса прямо: «поле публичного запроса обязано
// иметь читателя в прод-коде своего сервиса. Отсутствие геттера в non-test
// дереве — находка, а не стиль». Здесь он и стоит.
//
