...
// SeedCensusFrozenBucket — ведро, которому ноль НЕДОСТИЖИМ by construction:
// применённая миграция посеяла прежнее написание и правке не подлежит (ban #5),
// а переводящая обязана его назвать, чтобы найти строку.
//
// Имя объявлено здесь ОДИН раз и сверяется с тем, что печатает предикат: ведро,
// переименованное у предиката и не переименованное здесь, дало бы «его нет», то
// есть отключило бы проверку ниже молча.
...
//
// Предикат готовности перехода (§7-П2 приёмки) звучит «живое ведро даёт ноль».
// Ноль там наступает ДВУМЯ путями, и они неотличимы по самому числу: переход
// состоялся — либо обход ослеп (литерал переписан, популяция сузилась, ревизия
// не та). Поэтому здесь проверяется ВТОРАЯ величина: замёрзшее ведро обязано
// быть НЕПУСТЫМ. Оно доказывает, что обход всё ещё находит написание там, где
// ban #5 держит его вечно, — то есть что ноль живого есть вердикт, а не
// молчание прибора.
func AdjudicateSeedCensus(decl SeedCensusDeclaration, rep SeedCensusReport) []string {
	...
	case !printed:
		findings = append(findings, fmt.Sprintf(
			"предикат не печатает ведра %q — ноль живого стал бы неотличим от ослепшего "+
				"обхода", SeedCensusFrozenBucket))
	case frozen.Hits == 0:
		findings = append(findings, fmt.Sprintf(
			"ведро %q пусто (%s): прежнее написание не найдено даже в применённых "+
				"миграциях, где ban #5 держит его вечно. Значит обход ослеп, а не дерево "+
				"очистилось", SeedCensusFrozenBucket, frozen))
	}
