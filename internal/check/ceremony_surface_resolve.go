...
// функции, значением, выведенным из самого себя через преобразование
// (`p = p + "/x"`: множество значений не ограничено). Лист — не пропуск:
// регистрация с листом, не объявленным в ведомости, — находка «путь не
// разрешён».
//
// Цикл записей копированием (P = Q, Q = P) сводится к объединению всех его
// записей, и значение не зависит от того, какую регистрацию свели первой
// (strResolver).
