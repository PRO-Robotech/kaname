...
//     вызовов писателя записи в дереве — находка. Сколько реализаций в дереве,
//     печатает перепись («реализаций порта выпуска N»), а не этот заголовок:
//     число меняется с деревом. Ноль реализаций законен — выпускать нечему, и
//     ось о нём молчит. Предпосылка оси — объявление писателя в дереве: без
//     него ось ослепла бы молча.
//
// ─────────────────────────────────────────────────────────────────────────────
...
//  6. что писатель позван ИМЕННО на пути выдачи и ДО ответа, и что этот путь
//     собран: ось судит, что вызов в дереве есть. Порядок на уровне адаптера
//     держит `TestIssue_RecordsTheIssuanceInItsFamilyBeforeTheTokenLeaves`
//     (`internal/ceremonyport`), сборку — гейт перечня источников отзыва
//     службы (`TestContractListsFamilyAsProducedTodayOnlyWhenIssuanceIsRecorded`),
//     путь запроса — сборка церемонии (kaname#407);
