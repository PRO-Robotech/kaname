...
// (ban #17, задача #41).
//
// Норма, цена класса и довод «разбор, а не образец» живут в фундаменте
// (`github.com/PRO-Robotech/corelib/treehygiene`), шов к нему — в шапке
// `ascii_identifiers.go`. Здесь они не пересказываются, чтобы два места об одном
// предмете не разошлись.
//
// СВОЯ часть этого дерева — ОБХОД и его корень: фундамент разбирает один
// исходник и о деревьях не высказывается. Обход, перепись и порог
// беспредметности — ниже.
//
