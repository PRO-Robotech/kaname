...
func vertexLedger() map[string]vertexWaiver {
	return map[string]vertexWaiver{
		...
		// ── ЗАПИСИ РЕВЬЮ: предмет вопроса — ДРЕЙФ ЛИНИИ С ТОЧКИ ОТВЕТВЛЕНИЯ ВЕТКИ ──
		"docs/specs/reviews/login-issues-no-session-the-cutoff-covers/c9a0c3c7215972023e923cdcdc638a3903a86afd6a44a74a6ffeb41b053cb6c3.yaml#merge-base": {Calls: 1,
			Why: "рецензент круга 2 перемеряет предикат дрейфа из шапки документа: что ветка " +
				"волны `358` изменила в каталогах, на которые ссылается документ, С ТОЧКИ, где " +
				"от неё ответвилась ветка приёмки (upstream `acc-385` — `origin/358`, " +
				"`merge-base` → f4c6537a). Рабочая вершина здесь — не предмет суждения, а " +
				"операнд, задающий точку ответвления, и стволом быть не может: " +
				"`merge-base origin/main origin/358` даёт cbbac984, на 307 коммитов ниже, и " +
				"спросил бы о другом — обо всём, что линия несёт поверх ствола. Ответ записан " +
				"вместе с ревизиями, на которых снят (base_commit f4c6537a, origin/358 " +
				"196bf966). Вопрос о жизни ветки ДО посадки: схлопывание снимает его предмет " +
				"вместе с ветвью, а не меняет ответ. Запись ревью не правится — история ревью " +
				"только дописывается, — поэтому исход ведомостью, а не переводом вершины"},
		// ── СТРАЖИ ПРАВИЛА GIT: предмет вопроса — то, что УЕЗЖАЕТ или ЗАПРАШИВАЕТСЯ ──
		"scripts/hooks/branch-rule.sh#log": {Calls: 3,
			Why: "предикат судит коммиты, которые ему ПОДАЁТ потребитель: отправляемые (вершина " +
				"отправки минус опубликованное) и коммиты запроса (голова минус база), а T0 — " +
		...
		"scripts/hooks/prepush-rule.sh#rev-list": {Calls: 1,
			Why: "предмет — коммиты, которые уезжают ЭТОЙ отправкой: вершина, поданная git на " +
				...
				"опубликован by construction и операндом-вершиной быть не может"},
		"scripts/hooks/prepush-rule.sh#rev-parse": {Calls: 1,
			Why: "резолв вершины отправки (`local_sha^{commit}`), которую подал git: " +
				"предпосылка суждения, а не вопрос о вхождении — вершины у него нет"},
		"scripts/hooks/prepush-rule.sh#cat-file": {Calls: 1,
			Why: "резолв прежней вершины удалённой ветки (`remote_sha`), чтобы вычесть её из " +
				"диапазона: предпосылка отрицания, а не вопрос о вхождении"},
		".github/scripts/pr-rule-check.sh#rev-list": {Calls: 1,
			Why: "предмет — САМ ЗАПРОС: свои первые родители головы (`HEAD_SHA --not BASE_SHA`). " +
				"База запроса в ветку-номер — ветка линии, а не ствол, и диапазон строится от " +
				"неё; вершина-ствол дала бы пустой диапазон всегда"},
		".github/scripts/pr-rule-check.sh#cat-file": {Calls: 1,
			Why: "резолв базы и головы запроса в клоне конвейера — предпосылка построения " +
				"диапазона: коммита нет, и судить не по чему (исход 2), а не вопрос о вхождении"},
		"scripts/hooks/branch-rule-inject.sh#rev-parse": {Calls: 1,
			Why: "вершина — СИНТЕТИЧЕСКИЙ репозиторий фикстуры: вход хука строится из вершин её " +
				"веток и удалённых ссылок голого назначения. Ствола `origin/main` в таком " +
				"дереве нет вовсе — спрашивать о нём было бы вопросом к тому, чего фикстура " +
				"не заводит"},
