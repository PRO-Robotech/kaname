...
func vertexLedger() map[string]vertexWaiver {
	return map[string]vertexWaiver{
		...
		"internal/check/migration_version_monotonic_test.go#log": {Calls: 1,
			Why: "вершина ЕСТЬ ствол и передана аргументом — постоянной `trunkRefName`, " +
				"единственным объявлением имени ствола в этом пакете. Разрешить её разбор " +
				"не может: постоянные он берёт ПОФАЙЛОВО, а объявление живёт в соседнем " +
				"файле пакета, и завести здесь вторую копию строки значило бы получить " +
				"два расходящихся молча объявления одного имени ради того, чтобы " +
				"распознаватель увидел литерал. Предмет вопроса — исторический остаток " +
				"каталога миграций, то есть свойство ПРИЛЕТЕВШЕГО: рабочая вершина дала " +
				"бы в него собственную добавленную миграцию"},
