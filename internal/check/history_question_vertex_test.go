	...
	return map[string]vertexWaiver{
		...
		"docs/specs/reviews/login-lane-issues-our-session-and-logout-ends-it-server-side/4ee03c398152d87b4cdb74646b2ca34a513476d8d4e31799fec4f453d541cd22.yaml#diff": {Calls: 1,
			Why: "рецензент сверяет ДВЕ РЕДАКЦИИ одного документа внутри ветки PR (прежняя " +
				...
				"меняет ответ"},
		// ── ЗАПИСИ РЕВЬЮ: предмет вопроса — ПОРЯДОК ДВУХ ПРАВОК одного документа ──
		"docs/specs/reviews/address-confirmation-on-our-own/941940356ff69ea3ec0c3f0254756cd105b30e2501ba60eaafca9fd65e820c96.yaml#log": {Calls: 1,
			Why: "рецензент круга 1 сверяет ДВЕ ОТМЕТКИ ВРЕМЕНИ внутри ветки PR — последней " +
				"правки документа и правки его строки состояния (`git log -1 --format=%ct " +
				"-L8,8:<документ>`), — тот же вопрос, что у гейта правок после вердикта " +
				"(`acceptance_edit_after_verdict.go#log` выше): ПОРЯДОК двух правок в одной " +
				"истории, а не принадлежность ревизии стволу. Схлопывание двигает оба операнда " +
				"вместе, и ответ не меняется ни в одной посадке. Запись ревью не правится " +
				"(история ревью добавляется, не переписывается), поэтому вопрос прощён здесь, " +
				"а не переведён на ствол"},
		"docs/specs/reviews/address-confirmation-on-our-own/fa2209ca42bac9c728fa2ee7214c1624113977a7bdc99bb5ff6259e334ee08fd.yaml#log": {Calls: 1,
			Why: "рецензент круга 5 спрашивает о РОДИТЕЛЯХ коммита слияния ветки PR " +
				"(`git log -1 --format=%p 783da6f8`): ветка ли догнала линию — это вопрос о " +
				"ТОПОЛОГИИ САМОЙ ВЕТКИ, у ствола такого предмета нет by construction " +
				"(схлопывание вершину слияния предком ствола не сделает никогда). Ответ " +
				"фиксирует условие круга, а не вердикт о дереве после посадки. Запись ревью " +
				"не правится, поэтому вопрос прощён здесь, а не переведён на ствол"},
