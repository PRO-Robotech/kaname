...
func vertexLedger() map[string]vertexWaiver {
	return map[string]vertexWaiver{
		...
		"internal/check/seed_identity_census_test.go#rev-list": {Calls: 1,
			Why: "признак ВЫГРУЗКИ (`--count HEAD` == 1): распакованный состав коммита со свежим " +
				"`git init`. Это свойство рабочего каталога, у ствола его не спросить — ссылки на " +
				"origin в выгрузке нет вовсе"},
		"internal/check/seed_identity_census_test.go#log": {Calls: 1,
			Why: "вторая половина того же признака выгрузки: у единственного коммита нет " +
				"родителей. Спрашивается о том же дереве, что и `rev-list` выше"},

		// ── ОТСРОЧКА: вершина выбрана неверно, предмет заведён ─────────────────
		"internal/check/seed_identity_census_test.go#merge-base": {Calls: 1, Issue: 65,
			Why: "НАХОДКА, а не довод: ревизия замера §0 судится предком рабочей вершины — тот " +
				"же класс, что закрыт у гейта датировки переводом на `serviceTrunkRef`. Правка " +
				"идёт своим изменением: файл ведёт соседняя полоса"},
