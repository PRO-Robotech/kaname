...
// injProducerAppeared — ДЕФЕКТ самоистечения: у прощённого семейства появился
// вызывающий, значит набор стал перечислим у него.
const injProducerAppeared = `package somewhere

func use(r *Registry) {
	r.ObserveAuthzStoreAttempt("check", "ok", false)
}
`

// injProducerOnlyMentioned — ЗАКОННЫЙ БЛИЗНЕЦ: имя метода стоит в ПРОЗЕ и в
// объявлении, а вызова нет. Предикат по подстроке краснел бы на собственном
// объяснении проверяемого.
const injProducerOnlyMentioned = `package somewhere

// ObserveAuthzStoreAttempt — сюда пока никто не ходит; это объяснение, а не вызов.
func (r *Registry) ObserveAuthzStoreAttempt(op, outcome string, reused bool) {}
`

// TestIAM2500_InjectionRedsTheForgivenFamilyThatGrewAProducer — прощение
// «засевать нечем» обязано истекать от появления вызывающего.
func TestIAM2500_InjectionRedsTheForgivenFamilyThatGrewAProducer(t *testing.T) {
	defect := writeInjTree(t, map[string]string{"caller.go": injProducerAppeared})
	n, err := countMethodCallers(defect, "ObserveAuthzStoreAttempt")
	if err != nil {
		t.Fatalf("%v", err)
	}
	if n != 1 {
		t.Errorf("вызовов ожидался 1, насчитано %d — самоистечение не сработало бы", n)
	}

	twin := writeInjTree(t, map[string]string{"decl.go": injProducerOnlyMentioned})
	n, err = countMethodCallers(twin, "ObserveAuthzStoreAttempt")
	if err != nil {
		t.Fatalf("%v", err)
	}
	if n != 0 {
		t.Errorf("объявление и проза засчитаны как %d вызов(ов) — прощение истекло бы "+
			"от собственного объяснения", n)
	}
}
