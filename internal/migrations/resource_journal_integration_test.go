...
func TestIntegration_JournalMigrationRollsBack(t *testing.T) {
	...
	// Обратный ход идёт ДО СВОЕЙ версии, а не «на один шаг».
	//
	// Один шаг утверждал бы, что предмет этой пробы — ПОСЛЕДНЯЯ миграция
	// дерева. Утверждение верно в день записи и перестаёт быть верным от
	// появления любой следующей: проба отменяла бы ЧУЖУЮ миграцию и падала на
	// «висящие триггеры журнала остались», то есть обвиняла свой предмет в
	// чужом изменении. Тот же класс уже ловился здесь однажды — см.
	// `own_ceilings_down_roundtrip_integration_test.go`, где он назван и закрыт
	// этой же формой; наблюдалось повторно на `kaname#59`, добавившей миграцию
	// отзыва выдачи следом за журналом.
	//
	// Версия названа ЧИСЛОМ, а не выведена из положения в каталоге: положение и
	// есть то допущение, которое ломается.
	const journalVersion int64 = 20260914090000
	steps := 0
	for {
	...
	// прошёл бы дальше, и утверждения ниже зеленели бы на схеме, где предмет
	// отмены просто не применялся.
	require.Positive(t, steps, "обратный ход не сделал ни шага — утверждения ниже беспредметны")
	t.Logf("обратный ход: миграций отменено %d (до версии ниже %d)", steps, journalVersion)
