...
func TestLimitExport_TheDocumentedCommandExportsEveryAssignedValue(t *testing.T) {
	...
	// МИР — ВЕРСИЯ ПЕРЕД СНЯТИЕМ, а не голова цепи (kaname#58).
	//
	// Имя файла говорит «before retirement», и до стадии S4 это читалось как
	// «пока снятия нет в дереве». Снятие в дереве появилось, и та же фраза стала
	// означать другое: до головы таблицы больше нет, поэтому запрос выгрузки
	// исполняется на версии НЕПОСРЕДСТВЕННО ПЕРЕД сносом.
	//
	// Предмет от этого не сузился, а стал точнее: выгрузку делает оператор,
	// стоящий на предыдущем выпуске и получивший отказ наката, — то есть ровно
	// на этой версии. Прежний мир (голова) такого оператора не изображал никогда.
