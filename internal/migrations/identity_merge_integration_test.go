...
// identityReadGateRPC — RPC, чей объект гейта и есть предмет окна: единичное
// чтение записи личности. Перечислен ОДИН, потому что предпосылке довольно
// одного наблюдения: объект у чтения и перечисления операций переезжает одним
// решением, и разойтись они не могут — их гейтит один и тот же тип.
const identityReadGateRPC = "kaname.cloud.iam.v1.UserService/Get"

// catalogRel — координата встроенной копии каталога прав ОТ КОРНЯ дерева.
const catalogRel = "services/iam/internal/apps/kaname/seed/embedded/permission_catalog.json"

// identityReadGateObjectType — тип объекта, которым каталог гейтит чтение
// личности. Именно он решает, через какой объект резолвится `admin from
// account`, то есть именно он закрывает окно этой пробы.
//
// Читается РАЗБОРОМ, а не поиском подстроки: имя `iam_user` встречается в этом
// файле десятки раз в прозе соседних записей, и предикат по подстроке отвечал
// бы про чужую строку.
func identityReadGateObjectType(t *testing.T) string {
	t.Helper()
	b, err := os.ReadFile(platformtree.RequirePath(t, catalogRel))
	require.NoError(t, err,
		"ПРЕДПОСЫЛКА: каталог прав обязан читаться — без него предпосылка окна не "+
			"проверяется, и «окно открыто» стало бы неотличимо от «мы не посмотрели»")
	var entries []struct {
		FQN            string `json:"fqn"`
	...
	require.NoError(t, json.Unmarshal(b, &entries), "каталог прав не разобран")
	require.NotEmpty(t, entries, "каталог прав разобран в НОЛЬ записей — предпосылка сломана")
	for _, e := range entries {
		if e.FQN == identityReadGateRPC {
			return e.ScopeExtractor.ObjectType
		}
	}
	t.Fatalf("каталог не знает %s — перечень чтения личности пережил свой предмет", identityReadGateRPC)
	return ""
}
...
//
// # Почему проба ИСТЕКАЕТ САМА — и почему предпосылка читает ГЕЙТ, а не ТИП
//
// Её предпосылка — что окно ещё открыто: чтение личности гейтится ГЛОБАЛЬНЫМ
// объектом `iam_user`, и потому `admin from account` дотягивается до личности
// через ЛЮБОЕ её членство. Закроется гейт на членстве — проба покраснеет и
// потребует переписать себя под новый объект.
//
// ЗДЕСЬ ЧИТАЛОСЬ ОБЪЯВЛЕНИЕ ТИПА `iam_membership` В МОДЕЛИ, и это был НЕ ТОТ
// признак. Он приравнивал «тип заведён» к «объект гейта переехал», а это два
// разных события, и между ними есть окно: подфаза S3.1 заводит тип шагом EXPAND
// (модель, словарь, ветвь цепи областей), а объект гейта переезжает ПОЗЖЕ —
// сменой `scope_extractor` у двух RPC, то есть правкой аннотации контракта и
// поля `membership_id` в запросе. Пока переехал только тип, окно ОТКРЫТО ровно
// так же, как было: гейт по-прежнему спрашивает `iam_user`, предок личности
// по-прежнему берётся из всех её членств, и админ соседнего аккаунта
// по-прежнему дотягивается. Проба, истёкшая на заведении типа, объявила бы
// закрытой дыру, которая зияет, — то есть совершила бы ровно ту ложь, от
// которой её самоистечение и заведено.
//
// Признак поэтому читает ТО, ЧТО ЗАКРЫВАЕТ ОКНО: объект гейта чтения личности в
// каталоге прав. Послабление, которое не истекает само, переживает свой предмет
// и начинает лгать; послабление, истекающее РАНЬШЕ предмета, лжёт в другую
// сторону — и обе лжи стоят одинаково.
func TestIntegration_TheIdentityScopeChainNamesExactlyTheAccountsOfItsMemberships(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test (requires Docker)")
	}
	// Предикат читает РЕШЕНИЕ КАТАЛОГА — тип объекта, которым гейтится чтение
	// личности, — а не прозу и не объявление типа в модели (см. шапку пробы: тип
	// заводится шагом EXPAND раньше, чем переезжает гейт).
	require.Equal(t, "iam_user", identityReadGateObjectType(t),
		"ПРЕДПОСЫЛКА ИСТЕКЛА: чтение личности больше не гейтится глобальным `iam_user` — "+
			"значит объект аккаунт-скоупа переехал на членство, и окно, которое эта проба "+
