...
--
-- 20260823002000_relation_write_moves_onto_the_cluster — право модуля писать кортежи
-- переезжает с якоря ВНЕ ИЕРАРХИИ на КЛАСТЕРНОЕ отношение и становится обычной
-- системной выдачей.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- ПОЧЕМУ НОМЕР ИМЕННО ЭТОТ, А НЕ ТОТ, ПОД КОТОРЫМ МИГРАЦИЯ ПИСАЛАСЬ
--
-- Прежний номер был `20260822212316`, и он попадал в ОКНО между двумя миграциями
-- ствола: `20260822160000` (kacho#917) снимает колонку `fga_outbox.tuple_key`, а
-- `20260823001000` (kacho#1033) снимает её писателя — триггер, который эту колонку
-- заполняет. В окне колонки уже нет, а триггер ещё есть, поэтому ЛЮБАЯ вставка в
-- `fga_outbox` отвергается телом триггера:
--
--     ERROR: record "new" has no field "tuple_key" (SQLSTATE 42703)
--
-- Эта миграция в `fga_outbox` вставляет — значит она обязана идти ПОСЛЕ
-- `20260823001000`. Ни одна из двух миграций ствола не виновата: их пара
-- согласована, и между ними в стволе никто не вставляет. Расхождение появилось
-- ровно при слиянии, потому что порядок наката задаёт НОМЕР, а не время вливания.
--
-- Отсюда правило для следующей правки: номер этой миграции нельзя опускать ниже
-- `20260823001000`, пока в её теле есть вставка в `fga_outbox`. Проверяется одним
-- прогоном любой контейнерной пробы iam — накат шаблона падает первым же.
--
-- ─────────────────────────────────────────────────────────────────────────────
