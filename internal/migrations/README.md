...
- **способ входа человека** — строка на пару «человек, вид» с проверочным
  материалом (`user_login_methods`; материал — секрет: копий и механизмов,
  исполняющихся при записи без ведома писателя, нет)
  и подтверждённость адреса рядом с самим адресом (`users.email_verified_at`,
  снимается базой при смене адреса) — фаза Ф2, `kacho#1268`;
...
переименована миграцией `20260917130000`),
