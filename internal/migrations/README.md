...
возрастанию), следующие выведены из НОМЕРА ЗАДАЧИ (`<номер><порядковый:3>_…`).
**Действующая форма НОВОЙ миграции — метка времени заведения**
`YYYYMMDDHHMMSS_<имя>.sql`: она монотонна by construction (четырнадцать цифр
против шести), поэтому новый файл не встаёт ниже применённого. Форму держит гейт
`internal/repohygiene` `TestNewMigrationOutranksEveryAppliedOne`, и он судит
ТОЛЬКО добавленные относительно ствола файлы — унаследованные номера остаются как
есть, переномеровать применённое нельзя. Расхождение этого документа с гейтом
заведено задачей #1026; здесь названа та его часть, на которой спотыкаются при
заведении миграции. Создают они:
