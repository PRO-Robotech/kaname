...
func TestSixWithdrawnKinds_LeaveTheAccountingAndTheThreeLiveOnesStay(t *testing.T) {
	...
	for _, kind := range withdrawn {
		var accounting int
		// ЗДЕСЬ СТОЯЛО ВТОРОЕ УТВЕРЖДЕНИЕ — «величины этого вида нет у авторитета».
		// Оно снято ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ: хранилище авторитета покинуло схему
		// (kaname#58, стадия S4), и вопрос «есть ли там строка» больше не задаётся
		// ничему. Оставить его значило бы держать утверждение, выполняющееся
		// тождественно: у несуществующей таблицы нет строк ни у одного вида.
	...
	// ЗДЕСЬ СТОЯЛ ЗАКОННЫЙ БЛИЗНЕЦ — «у ЧУЖОГО вида величина у авторитета
	// остаётся», — и его собственный текст называл предикат своего снятия:
	// «снятие авторитета целиком — предмет стадии S4, НЕ ЭТОЙ». Стадия наступила
	// (kaname#58): хранилище покинуло схему, чужих величин в ней больше нет, и
	// близнец снят вместе с предметом, а не ослаблен.
	//
	// Отрицание, которое он стерёг, ушло тем же изменением, поэтому пара осталась
	// парой: выше утверждается ПРИСУТСТВИЕ величины у трёх действующих видов —
	// именно оно отличает снятие своего предмета от снятия шире предмета.
	//
	// Что таблицы больше нет, здесь НЕ утверждается: у этого предмета есть свой
	// держатель (`dropguard_integration_test.go`, храповик снятий), и второе место
	// об одном предмете разошлось бы с ним молча.
	t.Logf("перепись: снятых видов сверено %d, действующих %d — у каждого снятого "+
		"проверена строка учёта, у каждого действующего величина посадки",
