...
--
-- Посев служебных учёток по цепочке миграций разбирает дерево
-- (`internal/check`, `FoldSeededServiceAccounts`), и разбор судит СТАТИЧЕСКИЙ
-- оператор: динамическое имя через переменную он не узнаёт. Тот же довод уже
-- записан шапкой `20260909202745_module_identities_leave_the_baseline.sql`.
-- Переменные здесь читают состояние и решают исход; пишут — литералы.
--
-- ─────────────────────────────────────────────────────────────────────────────
