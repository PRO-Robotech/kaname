...
// доменом: функция `HumanSessionEndReasons() []string`, отдающая составной
// литерал, собранный из КОНСТАНТ ПО ИМЕНИ. Перечень читается разбором
// `internal/domain`, а не вызовом, потому что разбор держит ФОРМУ, которой
// вызов не видит: литерал вместо константы и переменная вместо функции со
// свежим срезом — находки. Разбор же дал честный красный до реализации: вызов
// несуществующей функции не собрался бы, а несобранная проба — это
// «не выполнилось», а не красный.
