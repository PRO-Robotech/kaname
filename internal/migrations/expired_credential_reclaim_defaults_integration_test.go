...
func TestCredRcl29_CeilingDefaultsInTheTreeAreTheRevisedOnes(t *testing.T) {
	...
	// ЗДЕСЬ СТОЯЛО «и у АВТОРИТЕТА этих величин больше нет». Утверждение снято
	// ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ: хранилище авторитета покинуло схему (kaname#58,
	// стадия S4), и вопрос о строке в нём не задаётся больше ничему. Оставленное,
	// оно выполнялось бы тождественно — у несуществующей таблицы строк нет, — то
	// есть стало бы утверждением, которое не может упасть.
	//
	// Предмет, который оно стерегло, не исчез, а СМЕНИЛ ДЕРЖАТЕЛЯ: что источник
	// величины ровно один, держит теперь сама схема — второго места, откуда её
	// можно прочитать, в ней не существует.
	...
	t.Logf("перепись: объявленных посадкой величин %d, величины %v — источник в схеме "+
		"ровно один, второго места для чтения в ней не существует", live, defaults)
}
