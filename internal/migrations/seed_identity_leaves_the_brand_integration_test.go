...
// TestSeedIdentity_MigrationWritesRenamesStatically — записанное значение
// читается В ТЕКСТЕ миграции, а не собирается из переменной.
//
// # ЧЕГО ЭТА ПРОБА НЕ ДОКАЗЫВАЕТ — сказано первым, потому что напрашивается
// обратное
//
// Она НЕ доказывает, что перевод виден разбору посева по цепочке
// (`check.FoldSeededServiceAccounts`). Опыт показал, что не виден: разбор делит
// текст по `;`, фрагмент начинается словом `IF`, и до сверки формы дело не
// доходит — «операторов о таблице 0». Это объявленная граница разбора
// (PL/pgSQL-блоки), а не дефект перевода.
//
// # ЧТО ОНА ДОКАЗЫВАЕТ
//
// Что величина, которую миграция ПИШЕТ, стоит в её тексте литералом. Переменная
// сделала бы запись вторым объявлением той же величины внутри одного файла —
// первое уже стоит в условии исхода, — и разошлись бы они молча: условие
// сравнивало бы с одним написанием, запись клала бы другое.
func TestSeedIdentity_MigrationWritesRenamesStatically(t *testing.T) {
	...
	require.Contains(t, up,
		...
		"перевод имени служебной записи записан не литералом: величина, которую миграция "+
			"пишет, перестала читаться в её тексте")
	require.NotContains(t, up, "SET name = v_",
		"перевод имени пишется переменной: величина объявлена дважды в одном файле — "+
			"условием исхода и записью, — и разойтись они могут молча")
}
