...
// СХЕМЫ: правка строки роли событие даёт, холостой подъём — не даёт, а голая
// запись подтаблицы в обход строки роли не даёт его тоже (§3).
//
// Отсюда же — чего она НЕ ловит и ловить не обязана: наблюдаемость ОТЗЫВА,
// отобранного последствием каталога. Он закрыт #76 явным объявлением из самого
// пути, а не триггером, поэтому свойство схемы осталось прежним; своя проба —
...
func TestIntegration_RoleCompositionIsObservableThroughTheRoleRow(t *testing.T) {
	...
	// ── §3. ГРАНИЦА РЕШЕНИЯ #73 — что СХЕМА не эмитит, и почему ─────────────
	//
	// Запись подтаблицы В ОБХОД строки роли события не даёт: триггеров на трёх
	// подтаблицах роли нет, и §1 выше показала, почему их не заводят — сужение
	// по существу отсекает из подъёма НОЛЬ.
	//
	// ЗДЕСЬ БЫЛ НАЗВАН ОСТАТОК, И ОН ЗАКРЫТ (#76), А УТВЕРЖДЕНИЕ ОСТАЛОСЬ — это
	// не недосмотр, и различие несущее. Остаток говорил: «отзыв последствием
	// каталога подписчику не виден». Закрыт он НЕ триггером — триггер вернул бы
	// ровно тот объём холостых событий, который меряет §1, — а ЯВНЫМ объявлением
	// из единственного пути, который пишет подтаблицу роли в обход её строки
	// (`catalog_consequence_sql.go`, `announceRoleGrantWithdrawalSQL`;
	// применитель зовёт его шагом 8а, в той же транзакции).
	//
	// Значит свойство СХЕМЫ не изменилось и обязано остаться прибитым: заведи
	// кто-нибудь триггер «чтобы уж наверняка», §1 покраснела бы объёмом, а эта
	// половина — дублем события на каждое применение. Наблюдаемость отзыва
	// утверждает СВОЯ проба, по своему предмету:
	// `internal/repo/kaname/pg/role_grant_withdrawal_reaches_the_subscriber_integration_test.go`.
	truncateJournal(t, ctx, pool)
	...
	assert.Zero(t, journalRowsOfKind(t, ctx, pool, "iam_role"),
		"ГОЛАЯ запись подтаблицы события давать НЕ ДОЛЖНА — это решение #73, а не "+
			"находка. Наблюдаемость отзыва держит объявление из пути последствий "+
			"(#76), а не триггер на подтаблице: триггер вернул бы холостой объём, "+
			"измеренный §1")
}
