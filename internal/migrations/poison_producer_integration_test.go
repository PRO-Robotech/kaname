...
// дренажа (`drainer.Config.PermanentPolicy`), а проводки у журнала нет вовсе.
//
// Перечня очередей, которые тот гейт видит, здесь НЕ выписывается (kacho#2480):
// он менялся молча, а у списка в комментарии нет владельца. Предикат рядом:
//
//	git grep -c 'PermanentPolicy' -- cmd/kaname ':!*_test.go'
//
// Значит для журнала эта проба — не вторая линия обороны, а ЕДИНСТВЕННАЯ:
// снимут ограничение — красной станет только она.
