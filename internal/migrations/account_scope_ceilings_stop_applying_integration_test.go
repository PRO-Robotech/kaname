...
//
// # Половина «пятое число переписи» БЫЛА неисполнима — предмет снят в тракте
//
...
// миграции сервер гасил раньше, чем оно уйдёт клиенту. Перепись `П35`, чей
// собственный комментарий обещает «печатается ВСЕГДА», не доходила ни до кого, и
// обработчик уведомлений тут был бессилен: сообщение не отправлялось вовсе.
//
...
// применённую миграцию править запрещено. Починено в ТРАКТЕ НАКАТА
// (`PRO-Robotech/kacho#2560`): применяющее цепочку соединение возвращает себе
// порог перед каждой миграцией — штатный накат в
// `pkg/migratorcli/noticethreshold.go`, здешний харнесс воспроизводит то же
// (`scopeLossChain`), потому что служба доступа тянет платформу пином.
//
// Что от этого НЕ изменилось: доставка уведомлений ОПЕРАТОРУ службы доступа
// по-прежнему ждёт бампа пина (`ПР-11`, `PRO-Robotech/kacho#2544`) — здесь
// утверждается по-прежнему более слабое и проверяемое: сервер это СКАЗАЛ.
//
...
func scopeLossChain(t *testing.T, before int64) (*sql.DB, string, *scopeLossNoticeSink) {
	...
	// ПОРОГ СЕССИИ ВОЗВРАЩАЕТСЯ ПЕРЕД КАЖДОЙ МИГРАЦИЕЙ — ровно тем же способом,
	// каким его возвращает штатный накат (`pkg/migratorcli/noticethreshold.go`,
	// задача `PRO-Robotech/kacho#2560`). Без этого свод глушит `client_min_messages`
	...
	// тот же свод объявляет `SET search_path TO kaname, public`, и `RESET ALL`
	// снял бы его вместе с порогом — то есть сломал бы сам накат.
...
func TestKANQ309AccountScopedCeilingsStopApplying(t *testing.T) {
	...
	//
	// ЗДЕСЬ СТОЯЛО ПОСЛАБЛЕНИЕ, И ЕГО ПРЕДМЕТ СНЯТ. Свод объявляет
	// `SET client_min_messages = warning`, `SET` без `LOCAL` держится до конца
	// сессии, и всякое `RAISE NOTICE` последующих миграций гасил СЕРВЕР — поэтому
	// утверждение переписи было неисполнимо, и ветвь выбиралась измеренным
	// порогом. Причина отпала: применяющее цепочку соединение возвращает себе
	// порог перед каждой миграцией (`scopeLossChain` выше; штатный накат —
	// `pkg/migratorcli/noticethreshold.go`, `PRO-Robotech/kacho#2560`).
	// Послабление, которому больше нечего послаблять, — находка, а не наследство,
	// поэтому оно снято вместе со своим предметом, а не оставлено «на всякий».
	//
	// Порог всё же читается — но как ДИАГНОСТИКА отказа, а не как выбор ветви:
	// вернись он к `warning`, читатель обязан узнать причину из текста падения, а
	// не искать её заново.
	...
	require.Containsf(t, saidByServer, censusLine,
		"пятое число переписи наката не равно двум. Порог сессии "+
			"`client_min_messages` = %q: его объявляет свод `0001_initial.sql`, и "+
			"применяющее цепочку соединение обязано возвращать его перед каждой "+
			"миграцией — иначе `RAISE NOTICE` гаснет на сервере и перепись не "+
			"доезжает ни до кого. Сказано было:\n%s", threshold, saidByServer)
