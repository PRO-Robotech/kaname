...
func TestIntegration_AuditEventChannelHasNoProducerLeft(t *testing.T) {
	...
	// журнала намерений снят вместе с внешним движком отношений, а свёртка
	// прямого факта синхронна (0098) — будить некого. Контроль, потерявший
	// предмет, и есть половина, которая доказывает, что отрицание рядом не
	// вакуумно, поэтому он перепривязан к каналу, чей потребитель ЖИВ и назван
	// прод-кодом (`repo/kacho/pg/reconcile_notify.go`, `LISTEN` на
	// reconcileOutboxChannel). Сам канал снят миграцией 20260829123045, его
	// регрессия — notify_channel_intent_journal_integration_test.go (#1436).
