...
// construction. Здесь — потому что уведомление будит ДРЕНАЖ, а дренажа у этой
// очереди нет и в этой фазе не будет: приёмника аудита в продукте не существует
// (решение и предикат пересмотра — в реестре отступлений iam,
// audit-outbox-has-no-receiver.md). Свернуть два разных основания в одну пробу
// значило бы оставить в дереве одно из них — и следующий читатель применил бы
// не то.
//
// Утверждается ПАРА, и вторая половина обязательна: без неё «ни один триггер не
...
func TestIntegration_AuditEventChannelHasNoProducerLeft(t *testing.T) {
	...
	assert.NotContains(t, channels, "audit_event",
		"канал снят вместе с триггером: он будил дренаж, которого нет и построить "+
			"сегодня нельзя — приёмника аудита в продукте не существует")
