...
-- одного предмета церемонии.
--
-- Путей, снимающих строку, ТРИ — по числу внешних ключей семейства, и все три
-- каскадные НА УДАЛЕНИИ. У каждого ровно один живой писатель вне проб
-- (измерено `grep -rnE 'DELETE FROM (kaname\.)?<таблица>' --include='*.go'`):
--
--   - `token_families_client_fk`  → снятие интерактивного клиента
--     (`internal/repo/kaname/pg/interactive_client_repo.go`);
--   - `token_families_user_fk`    → удаление человека
--     (`internal/repo/kaname/pg/user_repo.go`);
--   - `token_families_session_fk` → уборка записей сессии
--     (`internal/repo/kaname/pg/human_session_repo.go`,
--     `SweepUnservableSessions`, порог поверх снятия либо истечения).
--
-- ЗДЕСЬ СТОЯЛО «снимает ЕДИНСТВЕННЫЙ путь», и квантор был ложью: путей три, и
-- из ложного квантора был выведен весь абзац о пороге. Верно так: строка живёт
-- до РАНЬШЕГО из трёх удалений. Два первых — действия арендатора и расписания
-- не имеют; расписание есть только у третьего, поэтому в штатной работе порог
-- задаёт именно уборка сессий.
--
-- Отсюда действительный порог: НЕ срок кода, который идёт минутами, а срок
-- необслуживаемой сессии плюс порог её уборки. Обновляющий токен, переживающий
-- сессию, этим путём УБИРАЕТСЯ — каскад идёт от строки сессии через семейство,
-- — но живёт до неё, а не до своего срока. Рост, стало быть, ограничен
-- оборотом сессий и порогом уборки, а не числом выдач; своей записи предметом у
-- этих таблиц нет, и долг назван отдельной задачей, а не отложен маркером.
--
-- =============================================================================
...
-- ДОСТУП К ЭТОЙ ТАБЛИЦЕ ИДЁТ ДВУМЯ ПУТЯМИ, и оба измерены по прод-коду:
-- равенством по первичному ключу (`code_digest` — обмен, разбор отказа, замок
-- семейства) и каскадом от семейства по `family_id`, который уже накрыт
-- `authorization_codes_family_uk`.
--
-- ЗДЕСЬ СТОЯЛ ТРЕТИЙ ИНДЕКС — по `user_id`, — и потребителя у него не было ни
-- одного: ни запроса, фильтрующего коды по человеку (их в прод-коде ноль), ни
-- каскада, которому он нужен, — снятие человека приходит сюда ЧЕРЕЗ семейство,
-- то есть по `family_id`. Индекс без потребителя не бесплатен: он платится
-- записью на каждой выдаче и живёт как обещание пути, которого нет.
--
-- Срочный индекс ОСТАВЛЕН, и его предикат снятия назван: сегодня запроса по
-- сроку тоже нет — строку ищут по первичному ключу, а срок проверяют на уже
-- найденной, — он стоит под уборку, которой у этих таблиц пока НЕТ и которая
-- названа долгом в шапке. Закроется долг отказом от уборки — индекс уходит
-- вместе с ним.
