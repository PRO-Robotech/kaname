...
func TestLimitReaderGrant_IsRevokedAndLeavesATrace(t *testing.T) {
	...
	// ── УТВЕРЖДЕНИЕ 3: очередь не несёт НИ ОДНОЙ строки про это отношение ────
	//
	// Несущее, а не опрятность. Строка очереди применяется ПОСЛЕ того, как
	// служба поднимется на модели БЕЗ этого отношения, и движок прав отвергнет
	// её: отношения нет — ни записать кортеж, ни удалить его нельзя. Дренаж
	// считает такой отказ постоянным и травит строку, а переезда отравленных
	// обратно в работу у этой очереди нет by construction: разбор партиций идёт
	// по идентификатору ресурса, которого у неё не бывает. Строка лежит до
	// вмешательства оператора, и с ней лежит вся её партиция.
	//
	// Состояний два, и правильный исход у них РАЗНЫЙ — потому и утверждение
	// одно, а не «строк ровно столько»: на свежей установке строка записи ещё
	// в очереди (её удаляет отзыв, писать было нечего), на живой она давно
	// отправлена и удалена, а кортеж лежит в движке — там отзыв оставляет
	// строку снятия. Общее у обоих: строки, которую отвергнет модель без
	// отношения, не остаётся.
	var queued int
	require.NoError(t, db.QueryRow(
		`SELECT count(*) FROM kaname.fga_outbox WHERE payload ->> 'relation' = $1`,
		limitReaderRelation).Scan(&queued))
	require.Zero(t, queued,
		"очередь несёт строки про снимаемое отношение — на модели без него дренаж "+
			"отравится на первой же, и партиция встанет до вмешательства оператора")

	// ── УТВЕРЖДЕНИЕ 4: след оставлен ────────────────────────────────────────
	...
	t.Logf("перепись: системных выдач осталось %d; выдач права «%s» %d; прямых фактов %d; "+
		"строк ведомости эмитированного %d; строк очереди %d; записей следа %d",
		systemGrants, limitReaderRelation, grants, facts, emitted, queued, trace)
}
