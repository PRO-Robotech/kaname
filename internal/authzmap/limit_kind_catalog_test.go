...
var excludedKinds = map[string]string{
	...
	// ШЕСТЬ ВИДОВ СЛУЖБЫ ДОСТУПА, СНЯТЫХ С КАТАЛОГА ПОТОЛКОВ — задача
	// `PRO-Robotech/kacho#2117`, приёмка `KAN-QUOTA-1`, сценарий `KAN-Q3-04`.
	//
	// Причина у всех шести ОДНА, и она не «пока не сделали»: списывающего
	// триггера не было ни у одного, поэтому величина принималась, сохранялась и
	// НЕ ПРИМЕНЯЛАСЬ НИ РАЗУ. Потолок, под который никто не считает, — это класс
	// «принято-и-проигнорировано» (`api-conventions.md`), и снятие его ЗАКРЫВАЕТ,
	// а не заводит потерю контроля.
	//
	// Отличие от трёх ОСТАВШИХСЯ видов службы (`iam.account`,
	// `iam.user.credential`, `iam.serviceAccount.credential`) — не в имени и не в
	// носителе, а в ПРЕДИКАТЕ: у тех вызов `kacho_quota_count` стоит в применённой
	// миграции, у этих его нет. Поэтому те остались в каталоге и здесь не
	// перечислены.
	//
	// Запись самоистекает штатным образом: заведут этим видам счёт — вид вернётся
	// в каталог, и `TestExclusionsHaveSubject` потребует снять запись.
	"iam.project":        "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
	"iam.user":           "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
	"iam.serviceAccount": "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
	"iam.group":          "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
	"iam.role":           "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
	"iam.accessBinding":  "снят с каталога потолков (#2117): счёта под ним не было ни одного — величина принималась и не применялась ни разу",
}
