...
// объявлять читателя там, где его нет. Замерено: с ним `iam_service_account#user`
// (отношение «пользоваться этой служебной учёткой») выглядел живым, хотя его не
// требует ни каталог, ни модель, ни один вызов.
//
// Литералы берутся РАЗБОРОМ Go, а не поиском по тексту: комментарий и тег поля
// структуры содержат имена отношений по построению. Текстовый предикат на той же
// выборке объявлял читателем `vpc_subnet#use` — по строке `json:"use"` в чужом
// пакете и по перечислению отношений в комментарии.
//
...
var nonVerbWithoutReader = map[relPair]string{
	// `billing_admin` — бронь под домен, которого в продукте НЕТ: доменов семь
	// (IAM, VPC, Compute, Storage, Geo, NLB, Registry), биллинга среди них нет ни
	// одного RPC, ни одной таблицы. Отношение объявлено на кластере и на аккаунте
	// и не написано ни разу за всю жизнь модели.
	//
	// Решением этой задачи оно НЕ снимается: «домена не будет» — продуктовое
	// утверждение, которого никто не делал, а снять бронь и завести её обратно
	// стоит по-разному. Предмет и предикат снятия — задача #1114.
	//
	// ПРЕДИКАТ СНЯТИЯ ЗАПИСИ: домен биллинга заведён и его каталог требует
	// отношение — ЛИБО принято решение, что домена не будет, и отношение снято из
	// модели. В обоих случаях эта запись падает как устаревшая, а не переживает
	// свой предмет.
	{Type: "cluster", Relation: "billing_admin"}: "бронь под домен биллинга, которого в продукте нет (#1114)",
	{Type: "account", Relation: "billing_admin"}: "бронь под домен биллинга, которого в продукте нет (#1114)",

	// Семейство «пользоваться ресурсом»: `use` у подсети и адреса, `user` у
	// служебной учётки. Три объявления об одном замысле — «право взять чужой
	// ресурс в работу, не правя его», — и ни одно не требуется ни каталогом, ни
	// кодом: создание сетевого интерфейса и запуск машины гейтятся `editor` на
	// проекте, а сам ресурс проверяется на существование у владельца, а не на
	// право им пользоваться.
	//
	// Решаются ВМЕСТЕ и не здесь: два из трёх объявлений принадлежат vpc, и
	// закрывать половину замысла в задаче про iam значило бы оставить семейство
	// расщеплённым. Предмет и предикат — задача #1115.
	//
	// ПРЕДИКАТ СНЯТИЯ ЗАПИСИ: отношение начал требовать каталог (право
	// пользоваться отделено от права править) ЛИБО оно снято из модели.
	{Type: "iam_service_account", Relation: "user"}: "семейство «пользоваться ресурсом», решается целиком (#1115)",
	{Type: "vpc_subnet", Relation: "use"}:           "семейство «пользоваться ресурсом», решается целиком (#1115)",
	{Type: "vpc_address", Relation: "use"}:          "семейство «пользоваться ресурсом», решается целиком (#1115)",
}
