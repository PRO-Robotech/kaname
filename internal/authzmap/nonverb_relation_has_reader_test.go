...
//
// Состав берётся у индекса отслеживаемых файлов, а не обходом диска: рядом с
// модулем на всякой машине, где поднимали стенд, лежат распаковки чартов и
...
func prodCodeStringLiterals(t *testing.T, root string) (map[string]bool, int) {
	t.Helper()
	// Обход СОБСТВЕННОГО модуля, а не дерева платформы (#2375).
	//
	// Прежде здесь обходился прод-код ВСЕХ модулей под `services/`. После разреза
	// службы этого дерева рядом с ней нет: полоса стала бы невыводимой, а
	// утверждение «объявленное отношение кто-то читает» замолчало бы — ни
	// красного, ни зелёного.
	//
	// Сужение до своего модуля ЗДЕСЬ ничего не отнимает, и это ЗАМЕРЕНО, а не
	// предположено: из 30 неглагольных отношений, читаемых только прод-кодом,
	// прод-код чужих модулей был ЕДИНСТВЕННЫМ читателем у НУЛЯ (пять читает
	// только служба, двадцать пять — она и соседи). У соседней оси, глагольной,
	// замер дал другое — шесть пар, — и потому там полоса чужих модулей взята из
	// ОБЪЯВЛЕНИЯ фундамента, а не сужена. Две оси, два разных решения, и оба от
	// числа, а не от вкуса.
	//
	// Направление возможной ошибки названо прямо: если завтра единственным
	// читателем неглагольного отношения станет чужой модуль, гейт объявит его
	// мёртвым — то есть даст КРАСНОЕ, а не молчание. Ложная находка видна и
	// чинится записью в ведомость с причиной; молчание не видно ничем.
	...
	require.NoError(t, err, "индекс отслеживаемых файлов собственного модуля")
