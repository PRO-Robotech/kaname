...
func TestVerbRelationsOfType_IsPerTypeAndTotal(t *testing.T) {
	...
	// 25 с заведением `vpc_cidr_group` (именованный набор префиксов): число —
	// перемер, а не правка под результат. Оно обязано меняться ТОЛЬКО вместе с
	// каталогом типов, поэтому расхождение здесь — находка о каталоге.
	require.Equalf(t, 25, full,
		"глагольных типов ожидалось 25, получено %d — число снято перемером против ревизии посадки", full)
}
