...
func TestVerbRelationsOfType_IsPerTypeAndTotal(t *testing.T) {
	...
	// 27 → 28: заведён тип `iam_membership` (IAM-ID-1, S3.1) — объект гейта чтения
	// личности, скоупящий доступ на членство (объект `iam_membership:<mbr-…>`);
	// значение взято замером дерева после его заведения.
	require.Equalf(t, 28, full,
		"глагольных типов ожидалось 28, получено %d — число снято перемером против ревизии посадки", full)
}
