...
var declaredWithoutReader = map[verbPair]string{
	...
	// распоряжающихся глагола без читателя. Кто за это платит — перечислено по
	// замеру, а не по памяти (`GrantedVerbs("iam_user", …)`, наборы посева ролей —
	// миграции 0001/0031):
	//
	//   - `iam.user.edit` (правило `verbs:["update"]`) материализует `v_update` И
	//     `v_delete` (правка влечёт удаление на листе) — оба без читателя, то есть
	//     роль не даёт НИЧЕГО энфорсимого;
	//   - `iam.user.admin` (`verbs:["*"]`) материализует все четыре; `v_get`/`v_list`
	//     читаются, `v_update`/`v_delete` — нет;
	//   - ЛЮБАЯ роль, где автор выбрал глагол `delete` на `iam.user`, — редактор
	//     предлагает его из общего `closed_verbs`, и он тоже не даёт ничего.
	//
	// ЗДЕСЬ СТОЯЛА РОЛЬ `iam.user.delete` — ЕЁ НЕ СУЩЕСТВУЕТ И НЕ СУЩЕСТВОВАЛО.
	// Посев знает по `iam.user` ровно три роли: `admin`, `edit`, `view`; ни одна
	// миграция дерева не заводит роли с глаголом `delete` в имени (предикат:
	// `git grep -n "iam\.user\.delete"` — вне этих двух перечней только событие
	// журнала `iam.user.deleted`, а это другое). Имя было выведено по аналогии с
	...
	// Разница существенна: она и есть причина, по которой исход у обоих перечней
	// один и тот же — словарь ПО РЕСУРСУ, а не вывод роли из обращения.
	//
