...
var declaredWithoutReader = map[verbPair]string{
	...
	// ЗДЕСЬ СТОЯЛА ЗАПИСЬ ПРО `iam_user#v_update` — она СНЯТА ВМЕСТЕ СО СВОИМ
	// ПРЕДМЕТОМ (#1128). Её предикат снятия был назван и выполнен: словарь глаголов
	// стал ПО РЕСУРСУ (`CatalogResource.verbs`), поэтому набор типа сужается, не
	// отнимая глагол у остальных 26, — и глагол снят с типа, а не оставлен мёртвым.
	// Число «26» здесь не пересказ: прогон на снятом глаголе назвал ровно столько
	// пар, и он же теперь зелен.
	//
	...
	//
	// ПОЧЕМУ ОТНОШЕНИЕ ОСТАЛОСЬ ОБЪЯВЛЕННЫМ — и прежний довод здесь БОЛЬШЕ НЕ
	// ДЕЙСТВУЕТ. Он ссылался на общий словарь редактора ролей; словарь стал по
	// ресурсу (#1128), и сужение набора у одного типа больше ничего не отнимает у
	// соседей. Снятие `v_delete` с `iam_user` стало ВОЗМОЖНЫМ и не сделано здесь
	// намеренно: у него свой предмет — снятие ГЛАГОЛА с типа, — и он требует своего
	// изменения канонической модели, своего разбора и своего вердикта. Смешивать его
	// с #1128 значило бы закрыть два предмета одним изменением и сделать вердикт
	// непрослеживаемым.
	//
	// ЦЕНА, КОТОРУЮ ЭТА ЗАПИСЬ УДЕРЖИВАЕТ, названа целиком. Кто за неё платит —
	// перечислено по замеру, а не по памяти (`VerbsOfType("iam_user")` =
	// {delete, get, list}; `GrantedVerbs("iam_user", …)`; наборы посева ролей —
	// миграции 0001/0031):
	//
	//   - `iam.user.admin` (правило `verbs:["*"]`) — подстановка разворачивается в
	//     набор ТИПА, то есть в те же три глагола: `get`/`list` читаются, `delete`
	//     нет. Роль осмысленна и при этом материализует один кортеж впустую;
	//   - ЛЮБАЯ роль, где автор выбрал глагол `delete` на `iam.user`: редактор
	//     предлагает его из словаря САМОГО РЕСУРСА (`CatalogResource.verbs`, #1128),
	//     и он тоже не даёт ничего энфорсимого;
	//   - `iam.user.edit` (`verbs:["update"]`) плательщиком БЫЛА и быть перестала:
	//     её правило материализовало `v_update` И `v_delete` (правка влечёт удаление
	//     на листе) — оба без читателя, то есть роль не давала НИЧЕГО энфорсимого, и
	//     она выведена из обращения вместе со снятием `v_update` с типа (#1128). Тем
	//     же ходом закрылся и сам вывод «правка влечёт удаление»: `GrantedVerbs`
	//     добавляет `delete` только тому, чей `update` прошёл сверку с набором ТИПА,
	//     а в наборе `iam_user` его больше нет.
	//
	// ЗДЕСЬ СТОЯЛА РОЛЬ `iam.user.delete` — ЕЁ НЕ СУЩЕСТВУЕТ И НЕ СУЩЕСТВОВАЛО.
	// Посев знает по `iam.user` ровно три роли: `admin`, `edit` (сегодня выведена) и
	// `view`; ни одна миграция дерева не заводит роли с глаголом `delete` в имени.
	// Предикат: `git grep -n "iam\.user\.delete"` — вне этих двух перечней он даёт
	// событие журнала `iam.user.deleted` (а это другое) и комментарий миграции
	// #1128, унаследовавший тот же ошибочный обиход. Имя было выведено по аналогии с
	// соседом и прочтено как факт: по нему завели #1189 с предметом «роль
	// предлагается и выдаётся», а предлагается не роль — предлагается ГЛАГОЛ.
	// Разница существенна: выводить из обращения нечего, снимать надо глагол с типа.
	//
	// Это не побочный эффект, а следствие решения — и именно поэтому оно записано
	// здесь, а не выведено из молчания.
	//
	// ПРЕДИКАТ СНЯТИЯ (переформулирован вместе с предметом #1189): снять `v_delete` с
	// `iam_user` в канонической модели и из `typeVerbRelations`; обе записи перечней
	// (`declaredWithoutReader` здесь и `knownUnread["v_delete"]` в
	// materialized_relation_has_reader_test.go) уйдут следом — они самоистекающие;
	// проба каталога `TestCatalogResourceVerbs_DescribeTheTypesOwnSets` получит
	// вторую запись суженного набора с причиной. Блокер #1128 в этом дереве уже
	// посажен. Предмет — #1189.
	{Type: "iam_user", Relation: "v_delete"}: "снятие строки спрашивает identity_remover (#1131), исключение из аккаунта — account.member_remover (#1127); снятие глагола delete с типа — #1189",
