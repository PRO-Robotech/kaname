...
//
// ПОЧЕМУ РАВЕНСТВО, А НЕ «ПУСТО». Четыре пары в перечне мёртвы и на сегодня НЕ
// сняты — все четыре у `vpc_address_pool`, где каждый RPC гейтится
// `system_admin@cluster` и пообъектного глагола на самом пуле не спрашивает ни
// один путь запроса. Снятие — отдельное решение со своим разбором; пока оно не
// принято, число обязано быть ЗАФИКСИРОВАНО, чтобы не росло молча. Перечень с
// причиной честнее, чем «ноль находок» из проверки, которая этих пар не
// рассматривает.
//
// ЧИСЛО ЗДЕСЬ УЖЕ ПЕРЕЖИВАЛО СВОЙ ПРЕДМЕТ, и это стоит сказать вслух: строка
// называла ШЕСТЬ пар и перечисляла среди них `storage_snapshot`, которого в
// перечне не было вовсе — то есть была просрочена ещё до снятия `iam_role`
// (#1922). Гейт этого не ловит by construction: он судит МНОЖЕСТВО пар, а не
// прозу о нём, и всякое число, вписанное сюда, держится вниманием. Правя
// перечень, перечитай эту строку — предикат один:
//
//	awk '/^var declaredWithoutReader/,/^}$/' <этот файл> | grep -c '{Type: "'
//
// Голый `grep` по файлу здесь негоден: он считает и пары соседнего перечня
// `readBy` — 10 вместо 4, — то есть меряет файл, а не предмет.
...
var declaredWithoutReader = map[verbPair]string{
	...
	// ЗДЕСЬ СТОЯЛА ЗАПИСЬ ПРО `iam_role#v_get`. ОНА СНЯТА ВМЕСТЕ СО СВОИМ
	// ПРЕДМЕТОМ (#1922): тип больше не объявляет этого отношения, поэтому
	// прощать ему нечего.
	//
	// Предикат снятия был назван и выполнен: читателя у отношения не было ни
	// одного — единичное чтение роли энфорсится ТЕМ ЖЕ предикатом, что страница
	// (`iam_role` → {viewer, v_list} в internal/authzfilter/visibility.go), обе
	// поверхности зовут один резолвер, и ни одна запись каталога прав не
	// называет `v_get` на этом типе. Отношение снято с типа, а не оставлено
	// «на будущее»: право, которого не спрашивает никто, есть обещание права.
	...
	// iam_membership#v_get / #v_list — ТИП ЗАВЕДЁН, ЧИТАТЕЛЬ ПРИДЁТ С КОНТРАКТОМ
	// (IAM-ID-1, S3.1; шаг EXPAND).
	//
	// Тип `iam_membership` — объект гейта чтения личности: членство принадлежит
	// РОВНО ОДНОМУ аккаунту, поэтому `admin from account` на нём не дотягивается
	// до соседнего аккаунта (в отличие от глобальной `iam_user`, у которой
	// предков столько, сколько у человека членств). Сам переезд гейта — смена
	// `scope_extractor` у двух RPC — здесь НЕ сделан, и это граница полосы, а не
	// забывчивость.
	//
	// ЧИТАТЕЛЕЙ СЕГОДНЯ НОЛЬ, И ПРИЧИНА ИЗМЕРЕНА, а не предположена. Запись
	// каталога прав ВЫВОДИТСЯ ИЗ АННОТАЦИИ КОНТРАКТА, а не правится руками:
	// гейт `internal/check` `TestOwnCatalogCopyCoversOwnContract` сверяет копию
	// с `option (corelib.authz.v1.scope_extractor)` и краснеет на расхождении в
	// ЛЮБУЮ сторону. Аннотация сегодня:
	//
	//	proto/kaname/cloud/iam/v1/user_service.proto:31-34
	//	  object_type: "iam_user"  from_request_field: "user_id"
	//
	// Переключить её — работа полосы контракта (`proto-sync`), и она же обязана
	// завести поле `membership_id` в `GetUserRequest` и в запросе
	// `ListOperations`: деривация края читает ОДНО поле ВЕРХНЕГО УРОВНЯ запроса,
	// а такого поля у этих сообщений нет (`user_service.proto:473-476`).
	//
	// ПРЕДИКАТ СНЯТИЯ ЭТОЙ ЗАПИСИ (истекает сама, и гейт это проверяет обратной
	// стороной): аннотация двух RPC переключена на `iam_membership` +
	// `membership_id`, копия каталога пересобрана — у пар появляется читатель, и
	// гейт краснеет на «запись пережила свой предмет», требуя снять эти строки.
	// До тех пор `iam_user#v_get`/`#v_list` ОСТАЮТСЯ читаемыми из каталога и в
	// этот перечень не попадают — осиротеют они ровно шагом CONTRACT, вместе с
	// переключением аннотации.
	{Type: "iam_membership", Relation: "v_get"}:  "тип заведён (S3.1 expand); читатель приходит со сменой scope_extractor у UserService/Get — полоса контракта proto-sync + поле membership_id в GetUserRequest",
	{Type: "iam_membership", Relation: "v_list"}: "тип заведён (S3.1 expand); читатель приходит со сменой scope_extractor у UserService/ListOperations — полоса контракта proto-sync + поле membership_id в запросе",
}
