...
var declaredWithoutReader = map[verbPair]string{
	...
	// намеренно: у него свой предмет — роль `iam.user.delete`, — и он требует своей
	// миграции, своего разбора и своего вердикта. Смешивать его с #1128 значило бы
	// закрыть два предмета одним изменением и сделать вердикт непрослеживаемым.
	//
	// ЦЕНА, КОТОРУЮ ЭТА ЗАПИСЬ УДЕРЖИВАЕТ, названа целиком: правило роли с глаголом
	// `delete` на `iam.user` материализует кортеж, которого никто не спрашивает,
	// поэтому роль `iam.user.delete` не даёт ничего энфорсимого. Соседняя роль
	// `iam.user.edit` по той же причине выведена из обращения (#1128); эта — нет.
	//
	// ПРЕДИКАТ СНЯТИЯ: снять `v_delete` с `iam_user` в канонической модели и из
	// `typeVerbRelations`, вывести роль `iam.user.delete` новой миграцией. Предмет —
	// #1189.
	{Type: "iam_user", Relation: "v_delete"}: "снятие строки спрашивает identity_remover (#1131), исключение из аккаунта — account.member_remover (#1127); снятие глагола и вывод роли iam.user.delete — #1189",
