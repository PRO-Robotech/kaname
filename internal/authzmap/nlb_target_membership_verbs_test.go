...
func TestNLBTargetGroup_DeclaresMembershipVerbs(t *testing.T) {
	...
	// Парный положительный на ту же ось: канонический набор никуда не делся —
	// «объявляет два новых» отличимо от «таблица типа сломана».
	for _, v := range []string{"get", "list", "create", "update", "delete"} {
		require.Truef(t, domain.IsVerbOfType(v, verbs),
			"тип %q потерял канонический глагол %q (набор: %v)", tmType, v, verbs)
	}
	...
	require.NotEmpty(t, peer, "сосед не глагольный — отрицание ниже ничего не утверждает")
	for _, authored := range []string{tmVerbAdd, tmVerbRm} {
