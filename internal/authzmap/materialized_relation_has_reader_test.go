...
var knownUnread = map[string][]string{
	...
	// роль `iam.user.delete` материализует кортеж, которого никто не спрашивает. Аккаунту остаётся читать своих людей (`get`/`list` — читатели
	...
	// возможным и вынесено отдельным предметом — у него своя роль (`iam.user.delete`),
	// своя миграция и свой вердикт: #1189.
