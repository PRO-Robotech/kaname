...
var knownUnread = map[string][]string{
	...
	// `iam_user` в паре с `v_update` — находка #1102, не разрешение. Правку записи
	// личности с этого дня спрашивает `record_writer`, запрет и снятие запрета —
	// `identity_suspender`; ни у того, ни у другого нет источников уровня аккаунта,
	// потому что человек есть ГЛОБАЛЬНАЯ личность и распоряжаться его строкой
	// аккаунт не вправе.
	//
	// ЦЕНА НАЗВАНА, А НЕ ЗАМОЛЧАНА: правило роли с глаголом `update` на `iam.user`
	// продолжает материализовать кортеж, который больше никто не спрашивает.
	// Снять его СЕЙЧАС нельзя не из осторожности, а по замеру: `closed_verbs`
	// каталога — пересечение наборов всех типов, и сужение набора у одного типа
	// вынимает глагол `update` из выпадающего списка редактора ролей у ВСЕХ
	// остальных 26 (прогон на снятом глаголе назвал их поимённо). Разбор и
	// предикат снятия — соседний перечень `declaredWithoutReader`; предмет — #1128.
	"v_update": {"vpc_address_pool", "iam_user"},
	// `iam_user` в паре с `v_delete` — находка #1131, и она ТОГО ЖЕ класса, что
	// строка `v_update` выше, только исход тяжелее: запрет обратим, удаление нет.
	// Снятие строки личности с этого дня спрашивает `identity_remover` (сам человек
	// и надзор облака), у которого источников уровня аккаунта нет ни одного.
	//
	// ЦЕНА НАЗВАНА ПОЛНОСТЬЮ, и она больше, чем у соседа: после #1102 и #1131 у
	// `iam.user` НИ ОДИН из двух распоряжающихся глаголов не имеет читателя, то есть
	// роль `iam.user.edit` (правило `verbs:["update"]`, откуда правкой влечётся и
	// удаление) материализует ДВА кортежа, которых никто не спрашивает, и не даёт
	// ничего энфорсимого; `iam.user.admin` (`verbs:["*"]`) держится на оставшихся
	// `get`/`list`. Аккаунту остаётся читать своих людей (`get`/`list` — читатели
	// есть) и распоряжаться их участием: `RemoveFromAccount` гейтится
	// `account.member_remover` и к глаголам личности отношения не имеет (#1127).
	//
	// РОЛИ `iam.user.delete` ЗДЕСЬ НЕ НАЗЫВАЕТСЯ — её не существует: посев знает по
	// `iam.user` три роли (`admin`/`edit`/`view`), и ни одна миграция дерева не
	// заводит роль с глаголом `delete` в имени. Прежняя редакция называла её рядом
	// с `iam.user.edit`, и по этому имени завели #1189. Предлагается не роль, а
	// ГЛАГОЛ `delete` на `iam.user` — из общего `closed_verbs`; разбор и предикат
	// снятия — соседний перечень `declaredWithoutReader`.
	//
	// Снять глаголы СЕЙЧАС нельзя по тому же замеру, что и у соседа: `closed_verbs`
	// каталога — пересечение наборов всех типов, и сужение набора у одного типа
	// вынимает глагол из выпадающего списка редактора ролей у всех остальных.
	// Предикат снятия — словарь глаголов ПО РЕСУРСУ, предмет — #1128.
	"v_delete": {"vpc_address_pool", "iam_user"},
}
