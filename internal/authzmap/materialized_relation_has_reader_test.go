...
// selfModule — модуль, чей прод-код лежит в ЭТОМ дереве. Своё читается обходом,
// чужое — объявлением фундамента (#2398).
const selfModule = "iam"
...
// ownerServiceRelationLiterals — отношения, которые прод-код модуля-владельца
// ЧИТАЕТ при решении о доступе. Возвращает также объём осмотренного, чтобы
// «ноль находок» было отличимо от «ноль прочитанного».
//
// ИСТОЧНИКОВ ДВА, И РАЗДЕЛЕНЫ ОНИ НЕ ИЗ УДОБСТВА (#2398).
//
// Своё читается ОБХОДОМ собственного состава: у службы её прод-код есть всегда,
// и обход остаётся точным — он видит то, что действительно написано сегодня.
//
// Чужое читается ОБЪЯВЛЕНИЕМ фундамента (`moduleselfgating`), потому что после
// разреза чужого кода у службы нет by construction. Прежняя редакция собирала
// обе половины обходом дерева ПЛАТФОРМЫ, и это утверждение перестало бы
// исполняться в ОБОИХ деревьях: у службы нет чужого кода, у платформы нет
// модели. Тихо — ни красного, ни зелёного, и вопрос «оплачивается ли отношение
// впустую» задать стало бы некому.
//
// Истинность объявления держит СВОЙ гейт на стороне платформы
// (`TestModuleSelfGatingMatchesTheProdCodeOfEachModule`): сверка двусторонняя,
// обе её координаты платформенные, и разрез она переживает. Здесь объявление
// читается, а не проверяется — иначе оно проверялось бы дважды и разошлось.
//
// Состав своего берётся у treecorpus, то есть у ИНДЕКСА ОТСЛЕЖИВАЕМЫХ файлов, а
// не обходом диска. Разница не косметическая: под каталогом службы на всякой
// машине, где поднимали стенд, лежат распаковки чартов и отчёты прогонов, —
// обход диска зачёл бы их в «осмотрено», а найденный в них литерал сошёл бы за
// читателя. Первая редакция ходила по диску, и это поймал tree-wide гейт
...
func ownerServiceRelationLiterals(t *testing.T, root string) (map[string]map[string]bool, int) {
	...
	// ЧУЖИЕ модули — из объявления фундамента. Пусто здесь означало бы, что
	// объявление потеряло предмет либо не доехало пином: и то и другое —
	// находка, а не «чужих читателей нет».
	foreign := moduleselfgating.Modules()
	require.NotEmpty(t, foreign,
		"объявление moduleselfgating пусто: либо оно потеряло предмет, либо не доехало "+
			"пином фундамента. «Чужих читателей нет» такое молчание НЕ означает")
	for _, mod := range foreign {
		if mod == selfModule {
			continue // своё читаем обходом — оно точнее объявления
		}
		rels, ok := moduleselfgating.Relations(mod)
		require.Truef(t, ok, "модуль %q объявлен в перечне и не имеет отношений: "+
			"перечень и содержимое разошлись", mod)
		for _, rel := range rels {
			if out[mod] == nil {
				out[mod] = map[string]bool{}
			}
			out[mod][rel] = true
		}
	}
...
// объявление, а не обход чужого кода, и это утверждается ОТДЕЛЬНО (#2398).
//
// Отдельно — потому что главная проба этого сегодня не проверяет: все 109 пар
// читаются каталогом либо моделью, поэтому третий источник ни на одну из них не
// влияет, и опустошение объявления её не краснит. Замер, из которого росла
// задача, называл шесть пар с ЕДИНСТВЕННЫМ читателем «прод-код чужого модуля»;
// на сегодняшнем дереве таких нет — предмет ушёл, пока задача ждала.
//
// Из этого НЕ следует, что источник лишний: он ловит решения, которых каталог
// выразить не может, и молчит ровно до первой такой пары. Но пока он молчит,
// его подмену нельзя доказать через главную пробу — значит доказывается здесь:
// чужие отношения обязаны ПРИСУТСТВОВАТЬ в результате, и приходить именно из
// объявления, а не из обхода (обход сужен до своего модуля).
func TestForeignReadersComeFromTheFoundationDeclaration(t *testing.T) {
	root := monorepoRootForReaders(t)
	reads, scanned := ownerServiceRelationLiterals(t, root)
	require.Positive(t, scanned, "не осмотрено ни одного прод-файла — предпосылка сломана")

	foreign := moduleselfgating.Modules()
	require.NotEmpty(t, foreign, "объявление фундамента пусто")
	...
	for _, mod := range foreign {
		...
		rels, ok := moduleselfgating.Relations(mod)
		require.Truef(t, ok, "модуль %q объявлен без отношений", mod)
		for _, rel := range rels {
			require.Truef(t, reads[mod][rel],
				"отношение %q модуля %q объявлено фундаментом, но в результат не попало: "+
					"источник чужих читателей подменён или потерян", rel, mod)
			seen++
		}
	}
	require.Positivef(t, seen, "чужих отношений не прочитано ни одного при непустом объявлении")

	// Своё берётся обходом, а не объявлением: обход точнее — он видит написанное
	// сегодня. Утверждаем, что оно тоже есть, иначе сужение обхода срезало бы всё.
	require.NotEmptyf(t, reads[selfModule],
		"свой модуль %q не дал ни одного читателя: обход сужен слишком сильно", selfModule)

	t.Logf("перепись: модулей в объявлении %d · чужих отношений прочитано %d · "+
		"своих отношений %d · прод-файлов своего модуля осмотрено %d",
		len(foreign), seen, len(reads[selfModule]), scanned)
}
