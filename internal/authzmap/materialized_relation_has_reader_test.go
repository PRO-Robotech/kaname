...
var knownUnread = map[string][]string{
	...
	//
	// ЦЕНА НАЗВАНА ПОЛНОСТЬЮ: после #1102 и #1131 у `iam.user` ни один из двух
	// распоряжающихся глаголов не имел читателя. Половина закрыта — `v_update` снят
	// с типа, а роль `iam.user.edit` выведена из обращения (#1128); осталась вторая:
	// `v_delete` материализуется на каждый объект области привязки и не спрашивается
	// никем. Аккаунту остаётся читать своих людей (`get`/`list` — читатели есть) и
	// распоряжаться их участием: `RemoveFromAccount` гейтится
	...
	// заводит роль с глаголом `delete` в имени. Прежняя редакция обоих перечней
	// называла её рядом с `iam.user.edit`, и по этому имени завели #1189.
	// Предлагается не роль, а ГЛАГОЛ `delete` на `iam.user` — редактор берёт его из
	// словаря САМОГО РЕСУРСА (`CatalogResource.verbs`, #1128). Кто за это платит
	// поимённо, разбор и предикат снятия — соседний перечень `declaredWithoutReader`
	// (verb_relation_has_reader_test.go); здесь они не пересказываются, иначе два
	// перечня об одном предмете разойдутся при первой же правке.
	//
	// Прежний довод «снять нельзя» здесь БОЛЬШЕ НЕ ДЕЙСТВУЕТ: он ссылался на общий
	// словарь редактора ролей, а тот стал словарём ПО РЕСУРСУ (#1128). Снятие стало
	// возможным и вынесено отдельным предметом — у него своё изменение канонической
	// модели, свой разбор и свой вердикт: #1189.
