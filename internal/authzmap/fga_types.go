...
// ПОРОЖДЕНИЕ БЕРЁТ ДВА КОРНЯ, И ОНИ РАЗНЫЕ (#61).
//
...
//
// `-root` — корень ПЛАТФОРМЫ, откуда обходятся `services/*/manifest.yaml`. У
// самостоятельного клона фиксированного места рядом нет и быть не может,
// поэтому он берётся из окружения: `go generate` подставляет переменные сам.
// Не задана — команда ОТКАЗЫВАЕТ с названной предпосылкой и кодом 2, а не
// пишет таблицу из одного собственного манифеста. Отказ здесь и есть верное
