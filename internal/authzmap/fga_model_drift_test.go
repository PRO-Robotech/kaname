...
// makes the emitter write dangling-relation tuples → OpenFGA reject →
// drainer.ErrPermanent → poisoned fga_outbox → partial-grant desync.
//
...
//
//   - the Helm ConfigMap `openfga-model-stub-configmap.yaml` (both its `model.fga`
//     and its applied `model.json` block) is GENERATED by
//     `deploy/scripts/gen-openfga-model-configmap.py` (`make openfga-model-json`);
//     `fga_model_configmap_identity_test.go` proves the generated copy still
//     matches the canonical file, so the two can never silently diverge;
//   - this package's Go tables (objectTypes / verbBearingTypes / expandable
//     relations) are asserted against it here.
//
...
// monorepo root. This is THE source; the Helm ConfigMap is generated from it.
...
func canonicalModelPath(t *testing.T) string {
	...
	if _, err := os.Stat(p); err != nil {
		t.Fatalf("canonical authorization model %s is MISSING (%v) — the FGA drift-gate "+
			...
			"match the model that OpenFGA actually enforces. This is a hard failure by "+
			"design: restore the file (it is the artifact `make openfga-model-json` "+
			"generates the openfga-bootstrap ConfigMap from), never disable the gate.",
	...
	for _, ot := range catalogObjectTypes(t) {
		require.Truef(t, f.types[ot],
			"grantable objectTypes value %q is NOT a `type` in %s — the permission catalog "+
				"advertises it and the emitter would write tuples OpenFGA rejects (dangling "+
				"relation → ErrPermanent → poisoned fga_outbox). Either declare the type in the "+
...
// (authzmap.IsExpandableRelation — what ExpandAccess will forward into OpenFGA)
