...
//
// # Четвёртое состояние — недостающая половина
//
// Прежняя редакция проверяла три: запись без предмета → находка, находка вне
// перечня → находка, действующий перечень → тишина. Последнее держалось на
// перечне из пяти живых записей и вместе с ним исчезло. Здесь оно возвращено
// синтетикой и названо прямо: запись, у которой предмет ЕЩЁ ЕСТЬ, обязана
// МОЛЧАТЬ. Без неё сверка, объявляющая устаревшей вообще всякую запись, прошла бы
// оба оставшихся опыта.
func TestNonVerbReaderGate_LedgerExpiresItself(t *testing.T) {
	...
	require.Emptyf(t, stale, "действующая запись названа устаревшей: %v — сверка, объявляющая "+
		"устаревшей всякую запись, прошла бы оба опыта ниже и не измеряла бы ничего", deadPairNames(stale))
