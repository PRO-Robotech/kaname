...
var (
	...
	// ErrPersonalAccountNameUnavailable — личный аккаунт, заводимый как следствие
	// регистрации (Ф4 Р8), столкнулся по `accounts_name_unique`, и свободного имени
	// в зарезервированном пространстве не нашлось перебором против самого
	// ограничения базы (ban #10). Это НЕВЫПОЛНЕНИЕ, а не занятость адреса: имя
	// выбирает СИСТЕМА, не арендатор, поэтому исчерпание — системный отказ, а
	// зарегистрированный АДРЕС при этом свободен, и отказ занятости (400) солгал бы
	// о нём (Р3).
	//
	// # Это СПЕЦИАЛИЗАЦИЯ уже-существует, а не полоса рядом с ней
	//
	// Нарушение уникальности всё же произошло, поэтому `errors.Is(err,
	// ErrAlreadyExists)` на нём остаётся ИСТИНОЙ: потребитель, различающий лишь
	// «уже существует», продолжает работать. Регистрация проверяет ЭТОТ признак
	// ПЕРВЫМ и по нему классифицирует отказ ПО МЕСТУ — из записи ЛИЧНОГО аккаунта,
	// а не из ключа почты (code-authoring §3.1; register.go refuse).
	ErrPersonalAccountNameUnavailable error = specialisedSentinel{general: ErrAlreadyExists}
)
...
// ── Тексты отказа сериализационного конфликта: ОДНО объявление на обе полосы ──
//
// Приёмка: `docs/engineering/acceptance/refusal-text-names-the-lane-that-can-retry.md`
// (задача `PRO-Robotech/kacho#2439`).
//
// # Почему тексты живут ЗДЕСЬ, а не у своих потребителей
//
// Их читают ДВА места: переводчик отказов хранилища (`repo/kaname/pg`), который
// производит синхронный текст, и надстройка над репозиторием операций
// (`apps/kaname/shared`), которая подменяет его терминальным. Объявленные
// литералом у каждого, они разошлись бы при первой же правке — и разошлись бы
// МОЛЧА: на всяком входе, кроме сериализационного конфликта, обе стороны
// отвечают одинаково.
//
// Общее объявление снимает расхождение НЕ проверкой, а построением: источник
// один, и разойтись нечему. Дом выбран этот, потому что обе стороны уже
// импортируют пакет, а обратного ребра между ними нет — то есть цикла не
// заводится.
//
// # Почему текстов ДВА, а не один
//
// Совет «повтори» верен ровно там, где повторить может вызывающий: на
// синхронной полосе он держит соединение, отказ не терминален, повтор того же
// запроса проходит. На терминальном исходе операции повторять некому — исполнитель
// повторяет только терминальную ЗАПИСЬ, тело исполняется один раз, — поэтому
// там текст называет следующий шаг и сверх того факт, который вызывающему нужнее
// совета: изменение НЕ ПРИМЕНЕНО.
const (
	// SerializationConflictSyncText — синхронная полоса. Текст НЕ меняется:
	// он часть контракта и читается существующими клиентами.
	SerializationConflictSyncText = "conflicting concurrent change, retry the request"

	// SerializationConflictTerminalText — терминальный исход операции.
	//
	// Называет ФАКТ прежде совета: сериализационный отказ откатывает транзакцию,
	// поэтому «может быть, применилось» здесь исключено, и сказать это прямо
	// дешевле, чем оставить вызывающего гадать.
	SerializationConflictTerminalText = "conflicting concurrent change, " +
		"the operation was not applied; submit a new request"
)
